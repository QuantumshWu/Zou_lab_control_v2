...
- Seamless/Stepped写值及restore共用的tune检查容许float64单位往返的末位舍入（尺度至少1的8 ULP），保留实际回读值；不把硬件步进、clamping或明显读值偏差当成同一坐标，也不按device/field名称特判。
- ScanAxis的可选`display_unit`随plan与Layout保存，数值仍在端口声明单位中。显式编辑from/to/points时先在选中的单位中等间隔取点，再逐点转换为端口单位；135→247mVpp/10点不得改成dBm等分。只切换显示单位不改已有点，选区更新保留显示单位。Scan控件标签不重复端口单位后缀，内部field identity/单位契约不改。Rigol功率命令以17位有效数字传递，不再由客户端固定六位小数预先量化；实际回读仍来自仪器。

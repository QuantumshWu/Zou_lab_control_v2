...
- 未经authoring的默认投影只由`zlc_plot._kinds.defaults`一张表决定：每种plot kind的`default_spec`、FacetGrid的cell kind选择和「从当前plot要一个grid」都是对同一张表的读取，不得各自维护第二套推断。表按`classify_axes`得到的axis family分组，永不按axis name特判：R（Repeat domain axes）是统计量，只被reduce或被Histogram pool，从不单独成为layout轴；H（Runtime的`primary-index`）同样是统计量，除Rolling自己走它之外只在其它轴都没有结构时作curve最后的x；S（Point domain中的scan axis，slowest first）是位置——最内层是一次sweep走的x，两层是heatmap，最外层是grid的facet，无人认领的scan轴保持可编辑的Reduced；E（Point domain中的`READOUT_EVENT`，如camera frame、survival pair）是子测量的选择——grid给每个event一个cell，无scan的curve沿它走，其它情况显示Latest scope而不得对不同frame取平均；D（Cell-data axes）是内容——声明的picture或两条content轴成image，剩下的一条content在palette能分辨时成group，否则reduce。size为1的degenerate轴是provenance不是结构。`packages/zlc_plot/tests/test_default_roles.py`枚举全表。Limit类display字段（relim与x/y/color范围）声明为non-portable：panel identity改变时它们随semantic/fit一起从新vocabulary重新开始，只有外观字段跨kind携带。
...
- Pulse的API slot值有三层，后面盖前面：pulse文件自己authored的值 → 节点表单本次运行的值 → scan table逐点。保存的值集是`<workspace>/api_values/*.json`（当前集`current.json`，格式串`zlc.pulse.api_values`，封闭语法无版本号；每条`parameter_id → {value, unit}`，另有自由文本`source`记谁写的），与`pulses/`并列，session初始化时seed成空集。**值集是一个显式拉取的来源，不是自动生效的层**：pulse decoder只负责把文件读成PulseSequence，绝不去读第二个文件改写返回值，否则节点表单里的数字在本节点配置中无从解释，同一份layout.json隔天会跑出不同的数。套用只发生在两处手势：Pulse Editor（打开脉冲时套用当前集、On Pulse前重读再套用，因为编辑器是文档所有者、结果直接显示在字段上并点亮dirty星）和节点表单的Load values按钮（只填UI的框）。套用按名字取交集：pulse没声明的id跳过并列出，pulse声明而集合没提的保持原值；单位不符或超出port range则loud拒绝并点名。声明本身不被移除——改的是slot持有什么，不是slot是否存在。节点侧`api_values`是一个`text`字段，只记与pulse不同的项，由scan plan editor的表格编辑，被plan扫到的slot不显示；表格另给出「current.json在哪些参数上与当前值不同」的提示，使未拉取的标定可见而不是静默缺席。跨pulse传播的正路是：在编辑器里套用值集并保存脉冲，之后所有载入该脉冲的节点都拿到新值。
...
- **面板编辑是一个事务，两侧同时回滚**：plot session 拒绝时已回滚自身十六个字段，console 侧必须同步回滚 `binding.state`、Runtime history lease 与 live port 的目标。否则面板永久声称一个从未画过的设置，且下一 shot 仍按被拒的 spec 投影。

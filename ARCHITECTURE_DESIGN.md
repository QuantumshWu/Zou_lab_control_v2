...
- Config tab的Pulse bindings采用与Target一致的对齐Fluent行布局；字段引用可选Default、当前Config文档中的名称或输入自定义名称。换文件和Unload只改变可用值与覆盖状态，不删除已指定名称。真正使用Config覆盖的数值字段采用明显的深绿底白字，未覆盖的Config字段保留白底绿框；不得通过改变控件尺寸或字体区分状态。Calibration两个曝光字段后紧接三个API选择框，共用同一参数表单列宽及草稿通道，资源选择/Refresh沿原入口。
...
- 可编辑数值输入同时遵守四舍五入、真实上下限及实际文本宽度：显示数就是控件与提交草稿的数，不保留用于扩宽后恢复的隐藏尾数。Decimal步进、步长对齐和单位换算继续共用原实现；仅数值/单位/字体/宽度真正变化时量化。同单位族的前缀换算沿公共单位owner做十进制精确缩放，不往返二进制浮点制造长尾。被动布局/投影规范化只同步原草稿通道，不能冒充用户Apply写设备；只读科学数据及设备当前读数不因此被改写。数值和单位变化不得扩宽控件；若合法范围内不存在能在当前宽度显示的数，明确报错、保留原值，不越界、不自动扩宽。
...
- Pulse绑定popup使用两行共同列：Scan标签后的布尔check，以及Source标签后的Default/API/Config互斥三态胶囊开关。三态控件属于公共Fluent家族；名称配置仍只在Config tab，不能把popup变成第二个Config编辑器。

...
- Config tab的Pulse bindings采用与Target一致的对齐Fluent行布局；Config name始终是可编辑文本，空白表示未指定，下拉只建议当前已加载Config文件中的名称，不加入Default、当前自定义名或历史名称。换文件和Unload只改变候选与覆盖状态，不删除已指定文本；未保存的Config草稿不冒充已加载文件的候选。真正使用Config覆盖的数值字段采用低饱和浅绿底与深色文字，未覆盖的Config字段保留白底绿框；不得通过改变控件尺寸或字体区分状态。Calibration两个曝光字段后紧接三个API选择框，共用同一参数表单列宽及草稿通道，资源选择/Refresh沿原入口。

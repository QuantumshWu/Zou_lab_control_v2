...
- 信号的通用运算与衍生只有一个processor：`derive`。它的authoring是一段程序，每行`名字 = 表达式`，操作数是绑定producer的输出（`a.counts`、`a.occupied`……）和上面各行的名字；链式方法`.frame(k)`选帧、`.where(mask)`只改validity不改值、`.sum/.mean/.count/.any/.all("site")`沿site归约，算术/比较运算要求同一几何且单位可算，布尔操作数之间只有`==`/`!=`（两次判决是否一致）与`& | ~`。每行按其名字发布为该节点的一个输出（`@logic/<node>/<名字>`，contract `derive.<名字>`）：descriptor的输出随草稿而定（`declare_outputs`→`outputs_for(values)`），host、layout校验、信号列表都按草稿取，读不出的草稿暂不发布而由schema validator按原文报错。程序先在schema上typing出每行几何再算值，任何不能typing的写法按名拒绝。它读的所有输入都来自同一次publication（bound signal及其sibling输出，`dataset_input_siblings`由程序决定，host据此取sibling），所以对齐由构造保证、因果父唯一；跨两次publication的join（两个panel各自的fit相减）不是它悄悄读别的信号能做的事——那需要Runtime的双父lineage，未做。程序原文进run record与存档lineage。原`occupancy_agreement`节点已退役：它就是三行程序`agree = a.occupied.frame(0) == a.occupied.frame(2)`、`counts = a.counts.frame(1).where(agree)`、`occupied = a.occupied.frame(0).where(agree)`。
...
- Scan正常完成、Stop或失败都默认restore pre-run device values：pre-run值在第一次移动该knob前从device本身读出，并须落在该field当前可命令的bounds内，否则Scan在移动任何knob前就按名拒绝启动（一个放不回去的knob不扫）；restore走同一个verified `tune`，其拒绝在成功的run中就是run的失败、在失败的run中附注在原错误上，不得被SAFE成功掩盖。

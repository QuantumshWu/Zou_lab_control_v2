...
- Histogram除single/bimodal Gaussian外提供single/bimodal Poisson-Gaussian（`histogram_poisson_gaussian`、`bimodal_poisson_gaussian`）：光子计数在整数格点上服从泊松分布、相机读出叠加高斯噪声，模型是二者的精确卷积`A·Σ_k P(k|λ)·exp(-(x-k)²/2σ²)`，x轴必须是光电子单位，负值是读出噪声的正常结果。每类参数A、λ、σ；bimodal以λ_L与δ=λ_R−λ_L参数化，headline为δ（bright−dark contrast）。种子取直方图四分位间的质量加权均值与四分位距（不是矩，热像素尖峰会把矩种子推进rate=0的平谷），σ种子不低于√λ/3（方差的十分之一，目标函数在此有曲率；半光子种子在210光子下只占方差千分之一，求解器看不到梯度就宣告收敛）且不落在任何边界上；σ下限为bin/√12（模型在bin中心取值，读噪比这窄时整数光子的comb在bin中心间不再被抹平——0.03光子的读噪把7光子峰的全部计数塞进两个中心恰好落在整数上的bin；bin/√12正是bin自身的box平滑，此时bin中心值代表bin平均），上限为直方图跨度。bin比读噪宽时σ会吸收bin平滑W/√12，要测读噪须用不宽于σ的bin。**这两个模型只对光电子直方图提供**（`PHOTOELECTRON_AXIS` capability）：产品的光电子读出发布无单位的值，相机原始值带`count`单位（增益未知的ADU，没有光子格点），任何其他单位都不是计数；`fit_models`不列出、显式请求则拒绝并说明原因（MOT ROI像素直方图曾被拟成7 ADU背景峰下0.03光子的comb，该峰比7个光子允许的宽度还窄）。格点和只有一份实现（编译核）：每次目标函数求值先按rate填一张泊松pmf表（在众数处精确、向两侧递推），每bin只做三次exp、格点项用递推读表，且只求和高斯窗（±8σ）与泊松窗（rate−10√rate−10 … rate+12√rate+20，之外项低于众数的e⁻⁵⁰）的交集；SciPy路径的evaluator与overlay都调用同一编译核，冻结anchors用独立算术钉住它。两条路径在相同种子下收敛到同一最优。σ只在低光子数区可辨识（方差=λ+σ²，二十光子时σ²只占3%），高计数下应固定σ或改用Gaussian。

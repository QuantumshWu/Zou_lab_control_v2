...
- ScanAxis的`unit`就是其`values`的单位，Plan、Layout、Seamless/Stepped Dataset及run record保持该单位：135→247mVpp/10点直接存135…247与mVpp，不是存dBm值再附显示偏好。只有bind核端口范围、写设备和编译Pulse的边界才转native unit；Pulse量化后的坐标转回同一author unit，设备轴保持设定值。切单位逐点转换已有列以保持物理扫描，显式编辑范围/点数才在选中单位内等分；旧display_unit路径已删除。Rigol仍用17位有效数字发送，回读仍来自仪器，不以假回读迎合设定值。

...
- Histogram除single/bimodal Gaussian外提供single/bimodal Poisson-Gaussian（`histogram_poisson_gaussian`、`bimodal_poisson_gaussian`）：光子计数在整数格点上服从泊松分布、相机读出叠加高斯噪声，模型是二者的精确卷积`A·Σ_k P(k|λ)·exp(-(x-k)²/2σ²)`，x轴必须是光电子单位，负值是读出噪声的正常结果。每类参数A、λ、σ；bimodal以λ_L与δ=λ_R−λ_L参数化，headline为δ（bright−dark contrast）。种子取直方图的加权中位数与四分位距（不是矩，热像素尖峰会把矩种子推进rate=0的平谷）；宽度下限半个bin、上限为直方图跨度。格点和只有一份实现（编译核）：每次目标函数求值先按rate填一张泊松pmf表（在众数处精确、向两侧递推），每bin只做三次exp、格点项用递推读表，且只求和高斯窗（±8σ）与泊松窗（rate−10√rate−10 … rate+12√rate+20，之外项低于众数的e⁻⁵⁰）的交集；SciPy路径的evaluator与overlay都调用同一编译核，冻结anchors用独立算术钉住它。两条路径在相同种子下收敛到同一最优。σ只在低光子数区可辨识（方差=λ+σ²，二十光子时σ²只占3%），高计数下应固定σ或改用Gaussian。

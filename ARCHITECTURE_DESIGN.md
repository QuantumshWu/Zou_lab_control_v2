...
- Seamless Scan可显式选择一个`Acquisition logic`，只提供其他声明ready的Measurement；不得硬编码Camera。每次Scan Start仅一次`Pulse SAFE → 原Logic Start/Restart → 本次host ready → 启动settle`，随后整个扫描复用该采集运行，manual Continue、device点和repeat均不再重启。Restart仍由composition投递原Logic owner队列；不增加第二生命周期或通过中途重启隐式重置history。settle默认10ms（0.01s），显式值保留：启动等待一次；之后直接跟在每次set device扫描值操作后等待，不另设“实际变化”判断，不在manual、repeat或每次Fire上加等待。初次SAFE在设备写入前完成；每段结束仍确认SAFE。同一Scan的Pulse仅编译和完整load一次，后续Fire复用驻留程序，由原driver重arm扫描bank并恢复SAFE清除的clock mask；不跳过硬件确认。Scan owner在准备时报告Preparing，Fire成功后立即报告Scanning；每个`commit_live`更新全run shot进度。进度只走原ExecutionContext→NodeProgress→Runtime→UI，Stop也沿原owner。

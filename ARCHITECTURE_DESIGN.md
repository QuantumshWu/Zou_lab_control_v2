...
- Pulse的API slot值有一条四层链，后面盖前面：pulse文件自己authored的值 → 工作区当前值集 → 节点表单本次运行的值 → scan table逐点。工作区当前值集是`<workspace>/api_values/current.json`（格式串`zlc.pulse.api_values`，封闭语法，无版本号；每条`parameter_id → {value, unit}`，另有自由文本`source`记谁写的），与`pulses/`并列，session初始化时seed成空集（seed任何数字都会在第一次打开pulse时静默覆盖operator authored的值）。**每个从workspace载入pulse的decoder都套用它**（scan模板与calibration/SLM成像脉冲共两处），所以标定一次、所有pulse都跟着走。套用按名字取交集：pulse没声明的id跳过并列出，pulse声明而集合没提的保持authored值；单位不符或超出port range则loud拒绝并点名。声明本身不被移除——改的是slot持有什么，不是slot是否存在，因此与其后的scan table叠加。Pulse Editor打开时套用当前值集（因此与文件不同则出现dirty星），On Pulse前重新读盘再套用（先值集、后scan table），并有Load values/Save values两个手势；Save values从pulse生成，故手写模板的id与单位不可能凭空捏造。节点侧`api_values`是一个`text`字段，只记与pulse不同的项（存一份等值拷贝会静默压过下一次标定），由scan plan editor的表格编辑，被plan扫到的slot不显示。

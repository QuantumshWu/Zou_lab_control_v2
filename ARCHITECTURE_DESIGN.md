...
- 可编辑数值输入同时遵守四舍五入、真实上下限及实际文本宽度：显示数就是控件与提交草稿的数，不保留用于扩宽后恢复的隐藏尾数。Decimal步进、步长对齐和单位换算继续共用原实现；仅数值/单位/字体/宽度真正变化时量化。被动布局/投影规范化只同步原草稿通道，不能冒充用户Apply写设备；只读科学数据及设备当前读数不因此被改写。整数计数及无法继续舍入的窄边界保留其合法值，并给控件必要的最小可读宽度。

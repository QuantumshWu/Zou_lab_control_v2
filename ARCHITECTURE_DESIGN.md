...
- MEAN uncertainty的唯一owner从中心化一阶/二阶矩恢复样本方差；`mean_of_squares - mean²`若为不大于该次浮点归约与相减前向误差界的正残差，必须归零而不得冒充极小的真实SEM。这个边界只消除数值上无法与零区分的roundoff，不是数据阈值；随后Fit对零/NaN SEM沿用统一的最小真实正SEM规则，不允许某个plot kind或模型另行修权重。

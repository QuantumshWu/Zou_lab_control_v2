...
- **记什么由输出自己声明**：`DatasetOutputDeclaration.recorded` 默认**关**，理由是量出来的——相机的有限输出就是它的原始帧（1200×1920 uint16 = 4.4 MB 一张），一个一千事件的 chunk 是 4.3 GB，在第一个字节落盘之前就堆在内存里。「monitor 不记录」曾被我当成防线，但相机用 `DatasetCoverage` 发布的是同一份帧，那条规则在真机上根本不挡，已删。一个输出属于记录，是因为它累积的东西**就是科学**——occupancy 的 counts/occupied/frame_judged、frame_survival 的 survival、Derive 的用户输出——而只有产出它的节点知道这件事。**scan 的结果不在其中**：store 是一份日志，日志里每个事件必须自足，而扫描事件带着 `cell_origin` 指向 run 的 canonical schema，那个位置在输出上而不在 snapshot 里；记下来会变成一袋没处安放的点。要记它得让 store 也携带 placement，还没做。
- **run 发布的每个事件在发布时就落盘**：`commit_live` 与三条 `commit_processor` 都经 `NodeHost._record` 这一个所有者——只写 `_commit_live` 会把 occupancy、survival 这些**派生量全部漏掉**，而那是一次 run 的绝大部分。**格式与落盘分属两层**，和 `save_npz` 已有的切法一致（zlc_data 不许 import durable 层，`test_import_purity` 就守这条）：`zlc_data.store` 说一个 chunk 是什么（`encode_chunk` 出文档与数组、`chunk_event` 从数组还原事件），`zlc_runtime.publication_store` 说这些字节落在哪、落得多牢。一个 chunk 把一串 schema/validity 形状/有无 sigma 都一致的事件堆成一个 plain `.npy`（不一致就开下一个 chunk——一个 session 本来就会改 schema，拒绝它等于在 run 变得有意思的时候失效），**先 fsync chunk 文件再原子替换 manifest**，manifest 没点名的 chunk 等于没发生，所以中断最多丢当前 chunk 里还在缓冲的事件（默认 1000 个）。反过来写（先点名后写）会留下一个声称拥有它取不出来的事件的 store，比丢掉更糟，因为没有任何东西说出来。**两件事让 chunk 不被自己的元数据吃掉**：事件身份里凡是整个 chunk 相同的都只说一次（block id、stream generation、schema fingerprint——不提出去的话，一个 occupancy shot 的身份是 300 字节 JSON 而数据是 25 字节，manifest 就是文件本身），且提取按「值相等」递归判断、不点名任何字段，明天加一个身份字段自己会被提出去；bool 平面按位打包（occupancy 一个 site 一个 shot 是一个 bit 不是一个 byte），每个事件都重复的平面（没有缺站的 run 的 validity mask）只写一份、读回按需广播。百站实测：一天一百万 shot 的 `occupied` 盘上 **40 MB**（内存里的 bool 是 100 MB），float32 的 `counts` 410 MB、float64 791 MB。读取走 mmap，打开昨天的 run 不等于读完它。**不用 chunk 库**：每个 chunk 就是 `np.load` 能开的文件，装了这个发行版的任何东西都不需要，而 pinned 依赖表不增长；将来要压缩或远端存储再换 Zarr，格式是各自独立的文件，不是一个必须一起动的结构。
- **终态冲刷、shutdown 才关**：一个 host 的代数和它的源一样多（processor 的源结束会被 refuse 成 CANCELLED，standing re-follow 再把同一个 host 起来），在第一个终态就 close 会让第二代起一声不吭地不记录。`self._terminal = True` 原先散在 **14 处**，已收成 `_mark_terminal()`，让「这一代不再产事件」有唯一所有者。
- **Task 记在自己的 run 目录里**：Task 已经为这次 run 分配了编号目录，工件也在里面；旁边再开一个会把一次 run 的证据劈成日期列表里相隔一天的两个文件夹。其余节点在同一个日历天下拿自己的编号目录。
- **记录在哪里由知道 workspace 的那一层说，并且在第一个事件时才分配**：`RunRecorder` 每个已声明输出一个 store（两个输出的 schema 和形状不同，而 chunk 是一串一致事件的堆叠），目录由 console 给（day folder + `unique_path` 编号）；分配推迟到真的有事件——配置了、起了、停了却没发布过的节点没有东西要写，就不建目录。**记录是这次 run 在这个进程里的临时区**：run 退役（停下并放手、重启、Remove、关 console）时 recorder 自己把目录整个删掉，进程退出时（atexit）兜底删；Task 的记录放在它自己 run 目录下的 `published/`，删的只是这一层，artifact 不动。进程各管各的：不扫别的进程留下的东西，被 kill 掉的进程留下什么就是什么。**monitor 不记录**：它按契约只保留最新一个事件，本来就没有历史可写；这条规则也正是它付得起的原因——真实 run 里的 monitor 是相机，一帧八兆，记下来一个 chunk 就是八吉，实测 console 卡 6.9 秒、RSS 冲到 1.6 GB。run 累积什么就记什么，和 plane 累积的是同一批。**记录在自己的线程上写**，也是量出来的：一次 flush 约 17 ms 且几乎全是 fsync——五十个事件和一千个事件一样贵，所以没有哪个 chunk 尺寸躲得掉——而走到终态的是那些轮询 host 的线程，在 console 上就是 Qt 的。`record` 只把事情交出去，队列有界，写不过来的磁盘让生产者等一等而不是让内存长下去；终态只**请求**冲刷不等待（等的是 host 的 shutdown）。**记录失败不失败 run**：第一个失败停掉记录、把原因留在 `recorder.failure` 给人读，实验继续。丢记录不好，丢实验更糟，这个顺序是决定不是偶然，所以写在这里而不是留给一个裸 `except`。

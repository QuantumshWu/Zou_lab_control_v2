...
- Panel标题仅Repeat域显示条件有效数，Point/Cell-data仍显示完整维度。对每个Repeat轴放开自身，其余Repeat/Point坐标由同一次publication最后写入的位置与canonical axis_codes决定；没有incremental placement的完整数据以自身carrier末位置为当前值。不得读取Panel Scope/Focus/Facet/Reduction或独立latest决定计数。Cell-data同时存在多个component且计数不同时显示min–max；计数之积不代表全Dataset有效样本总数。统计只读compact validity，不展开image pixels；一次surface acceptance的标题metadata复用，不重复计算。
...
- Seamless允许仅manual/device轴且Pulse无scan slot。资源选择直接复用严格Pulse reader，不得在资源加载层额外要求slot存在；真实slot语法由reader校验，slot与Plan的覆盖关系仍由原绑定及扫描owner校验。每个Host点复用原采集准备流程，加载普通无slot Pulse（wire rows为空），用Run repeats完成shots_per_point；整轮repeats由原Host外层循环推进，不把它写成无表硬件scan_repeats。数据只包含真实扫描轴，不补虚构slot/轴；有slot时仍使用同一原表执行和精确绑定要求。顶层ScanPlan仍至少有一条真实轴。
...
- 单位化设备边界只有`read_tunable_in_unit`、只读`convert_tunable_value`与`tune_in_unit`，Scan、Device Control和Remote共用；原`tune`与字段ID/canonical metadata保持不变。支持原生单位的driver在原command锁内仅必要时切仪器单位，135mVpp直接按0.135Vpp写入，不绕dBm；不支持的单位在设备边界换算。Rigol用实际负载/波形换算范围，17位有效数字发送，返回真实回读，不修改负载/波形或推断回读必须等于请求；多channel共享功率policy没有唯一电压对应值，拒绝其电压单位而保留dBm/W及None清除。Control和Scan Editor的范围读取、单位换算复用device worker，Qt只接收纯结果；单位换算pending时不能Apply旧单位，晚结果不得覆盖新草稿或访问已删除控件。仅切显示单位不发设备写命令（也不触发Live apply），Refresh不等于Apply；真正Apply发送对应数值+单位。设备epoch是driver事实，Control不从浮点相等自行猜增量；override保存requested_unit，canonical before/after不随显示单位漂移。

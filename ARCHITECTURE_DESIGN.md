...
5. 反馈只做直接强度比例校正，不拟合 Zernike、modal coefficients、hidden aberration、绝对trap depth或连续波前；Editor 中由操作者明确设置的 wavefront coefficients 不属于 feedback 估计。令正 target support 上的逐-site occupied-shot BOX brightness 为 `F_i`，每轮只更新 `w_i <- w_i * (GM(F) / F_i)^0.45`，再归一化 target total、以上一 canonical Pattern/base phase warm-start `solve_phase`、apply、复测并按site spread独立保留 best。best排序不奖励总荧光变大；总荧光相对baseline的90%条件只拒绝把目标site功率送往zero order、background或ghosts造成的效率坍塌，因为相位传播只保证全焦平面总能量守恒，不保证target support内的功率守恒。绝对trap depth未来只能由trap-frequency/light-shift等独立observable校准，本Task不得用亮度变大冒充trap更深、focus更好或像差更小。Task production code不得 import SimulationWorld、读取 hidden trap plane/site intensity或按 simulation seed 分支。

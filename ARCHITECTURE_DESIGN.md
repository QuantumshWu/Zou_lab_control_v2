...
1. SLM Editor 是 `devices/slm` concrete plugin 自己的独立 device window，继续沿用 descriptor lazy `control_factory`、同一 named SLM 与同一 `DeviceUseCoordinator`。默认 `Mask` 页保留 target 与 Final 两张既有 `2x2 = 490 x 357` logical plot viewport；新增相位层不得挤小这两个图。一个共享 `Size` selector 只使用 `zlc_plot.PANEL_SIZE_NAMES`，同时调整两个独立 plot host 以保持等大、对齐；画布行可横向/纵向滚动，因此更大 preset 不重叠、不裁切。DPR 只改变 raster physical pixels，不改变所选 logical preset。`Mask ROI`、常用 `Wavefront` 与 `Advanced` 是分开的、各自可滚动的设置页：常用页只显示 full-raster Steering X/Y、Z4 defocus、Z5/Z6 astigmatism 和一次性 Reset；Advanced 才显示 Z2/Z3 pupil tilt 与 Z7–Z11，并在隐藏项非零时把 active count 写进 tab label。左侧编辑唯一 continuous non-negative target；后台 `solve_phase` 只更新 Mask/base phase。操作者可把 Mask 限制到 authored `x/y/width/height` ROI，并独立设置全 raster carrier X/Y（edge-to-edge waves）以及 Noll Z2–Z11、单位圆瞳内、waves-RMS 的 Zernike coefficients；右侧只显示 `Final = wrap(full-raster carrier + pupil Zernike + ROI Mask)`。target 改动仍由 capacity-one latest-only worker求解，Qt owner不做 FFT；Selectors off 保留画笔，on 开放两图 selector/zoom/pan。Target JSON、Mask NPZ、Final NPZ 的 load/save、8-bit phase-mask image import都不写硬件；phase-mask image只接受二维 8-bit gray并按 `gray * 2π/256` 解释，可为device shape或enabled ROI shape，明确不是 vendor correction/LUT。Load Final 清零 carrier/Zernike并关闭 ROI，从而保持 exact phase roundtrip。只有显式 `Send to SLM` 取得短命 EXCLUSIVE claim后调用 `apply_phase`；Final 若落后于当前 target request必须拒绝。关闭 Editor不apply/restore/blank/关闭SLM，commanded phase原样保留。
2. 第一版粗调的唯一 observable 是现有 qCMOS 拍到的 occupied-atom site brightness。每个 candidate phase 下执行作者写下的真实 cool/load/probe pulse 与 exact grouped camera cycles；相机沿用 calibration 记录的 sensor integration，而 pulse 的 probe gate 保持其 authored API values，两者不得互相覆盖。逐 frame 的 BOX counts/occupied/valid 复用 `OccupancyProcessor` 的唯一判断；每个 site 只对 `occupied=True` 且 valid/finite 的原始 BOX mean 做多-shot平均，empty shots不作为零值进入分母，也不按该 site 的 dark/bright response归一化。这个量与 Calibration 的 multi-frame occupied BOX brightness 是同一口径；loop 内不重新找 site，也不逐 site 做 nonlinear Gaussian fit。
...
6. `Blank`/`Off`/beam-dump 不是 zero phase。X15213 phase transport 已有真实 leaf，但实验室安全光路与 beam-dump command 仍未被证明，因此公共 `SlmAdapter` 不增加虚假的 blank，Editor 也不显示该控件。关闭 Editor 或 session 不擅自写 zero phase；如需安全退役，必须先取得并验收实验室明确的物理 command。

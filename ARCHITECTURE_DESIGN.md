...
- Pulse的API slot值有两层，后面盖前面：pulse文件自己authored的值 → 节点表单本次运行的值（scan table逐点再盖一层）。**没有第二个文件**：pulse的API参数属于pulse文件本身，值集文件那套已整体删除——一个隔壁文件改写loader返回的东西，等于节点表单里的数字在本节点配置中无从解释、同一份layout.json隔天跑出不同的数。节点侧`api_values`是一个`text`字段，只记与pulse不同的项，由scan plan editor的表格编辑，被plan扫到的slot不显示；跨pulse传播的正路是在编辑器里改脉冲并保存，之后所有载入该脉冲的节点都拿到新值。Config参数（如channel delay/DAC bias）的数值集沿`<workspace>/config_values/*.json`（默认`current.json`，格式串`zlc.pulse.config_values`）保存。Save config只导出当前Pulse Editor声明的Config字段实际值和单位，允许离线、不要求On Pulse、不读取或改变设备值；Load config显式把文件作为当前sequencer实例的可选覆盖集。所有执行入口经同一`compile_pulse`按ID取交集覆盖，未匹配保留Pulse原值，额外ID忽略，匹配项仍严格检查数值/单位/字段合法性。Editor只在Open/Load/On Pulse显式投影同一覆盖规则，不在普通refresh或Save时覆盖草稿。current.json只在sequencer初建/替换时自动加载；复用sequencer、添加无关设备不得覆盖operator显式加载的集合。独立客户端各自持有加载集合，不假定同板跨进程自动同步；文件变化须重新Load，不隐式监视。

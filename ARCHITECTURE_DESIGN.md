...
7. 每个 `cooling` rising edge 在 `trap` 为 high 时都是一次新的装载 shot：只对当时 active trap topology 中的每个 trap 独立抽取 base Bernoulli，绝不沿用上一次 pattern，也不让 trap depth 以指数或其他函数提高 loading probability；没有 active trap 的 site 概率严格为零。local trap depth 只进入已装载原子的 qCMOS brightness 与 trap-off release survival。`trap` 关闭的完整实际时长造成 release loss；`emCCD` 窗口触发 qCMOS，而原子荧光只积分 camera working exposure 与 `probe` 窗口的真实交集。`da_bias_x/y/z` 分别表示 MOT 图像 x、y 和光轴磁场：signed `(0,0,0)` 是唯一期望亮度最优点，亮斑位于像素几何中心 `((width-1)/2, (height-1)/2)`；x/y 移动 MOT，三轴离零场的距离共同降低亮度，不得再按 frame ordinal 人为漂移。
...
1. SLM 的唯一公共 capability 是 `slm.phase`。共同设备契约是 runtime-checkable `SlmAdapter`：只暴露稳定 identity、二维 phase shape、`apply_phase(radians)`、immutable last-commanded phase snapshot 和 `close()`。Canonical command 永远是 finite wrapped radians `[0, 2π)`。X15213 的 measured `two_pi_gray`、本机 correction BMP、correction sign/offset、active-raster X offset、orientation、optical settle 以及 USB/DVI transport 全部只在真实 Hamamatsu leaf 内；solver、Editor Final、Workbench 和 virtual contract 都不携带 vendor gray/LUT 语义。
...
3. 对外只有一个 `solve_phase`，同一二维 intensity 仍是唯一 target truth；但 `objective_kind="spots"|"image"|"auto"` 明确描述作者要做离散焦点还是连续图像，而不是让连通块大小偷偷猜业务语义。Grid/checker、单点 Toggle 与稀疏手动画明确用 `spots`；rectangle/ellipse/ring 等连续 flat-top 用 `image`；只有缺少作者语义的旧 artifact/import 才保留 conservative `auto`。solver 的 near-field source 是与 SLM phase 同 shape 的 finite non-negative **amplitude**；Editor 可 materialize 明确的 configured hard ellipse，也可加载同 raster measured intensity、取平方根后裁到同一 ellipse，且必须在 UI 标成 `assumed` 或 `measured`。同一 configured ellipse 同时决定 CGH solve 的 source support 与 Zernike 的物理 unit pupil；measured 文件在 `Apply pupil` 前只是 draft，不得污染已应用的science metadata。求解后裁一块 Mask phase 只是 `CGH crop`，不得再冒充 aperture。`spots` 使用 full-resolution WGS-Kim：先更新、随后固定远场 spot phase，只在非零 support 计算远场点相位，并在规则 Cartesian support 上使用与完整 DFT 数学等价的 separable selected-frequency transform；否则使用不含循环内 shift 的完整 FFT。不得 snap 坐标、裁物理像素、降采样或放宽最终 intensity quality gate。`image` 继续走 full-resolution MRAF。默认 sparse solve 只在**实际返回的 canonical float32 phase**重新传播并精确满足既有 1% simulated support-ratio quality gate 后提前停止，否则跑满上限并如实报告；这是对确定性返回值的验收，不重复执行相同阈值的非物理“三连门”。显式 iteration count 必须精确执行。authoritative warm start 仍只传上一次 accepted Mask canonical phase，不能把已含 carrier/Zernike 的 science phase 再叠加一次；此外，只有在 `spots` objective、非零 support、已应用 pupil context 与 Mask authority 都未被替换时，Editor 才可持有 caller-owned 的短命 optimizer state，复用 fixed far-field spot phases、site weights 与 target amplitudes。该 state 只由 latest matching successful solve 接纳；support/objective 改变、Clear、任何 pupil Apply、Load Mask、Load science phase 或 close 都立即使其失效。它不是设备 truth、不是 phase metadata、不是 artifact/hash，也不得进入 serializer；stale、Stop 或 failed solve 都不能提交它。Target 用 strict JSON 保存；Mask 与 science phase 都复用同一个 `allow_pickle=False` canonical-phase NPZ codec 和 plain metadata，不增加第二 serializer、hash/fingerprint 或 Python object payload。没有 X15213 实测 fringing-field forward model时不得凭空加 blur/crosstalk kernel；当前只报告 focal-plane ghost/background/efficiency，未来取得本机标定后才补 device crosstalk correction。
4. `VirtualSLM` 只更新 installation-owned `SimulationWorld` 的 commanded phase 与 monotonically increasing phase revision。world 以 pupil illumination、8-bit phase quantization、hidden low-order Zernike、illumination decenter/vignetting和同一 FFT 产生唯一 target-plane trap intensity。传播结果只决定 active topology、occupied-atom qCMOS brightness 与 release survival；每次 trap-high cooling rise 的 fresh loading Bernoulli 在所有 active traps 上使用同一个 authored base probability，与 local depth 大小无关。solver 与 Task 都不能读取 hidden aberration 或 hidden trap truth。每次 camera trigger 必须消费最新 phase revision。actual propagated plane 中的新增/移动 peak 按固定 nominal SLM↔qCMOS 仿射关系出现在 raw camera frame；不得从 Editor target 旁路注入 world。所有 trap depth 都直接取同一 propagated plane 的 raw local peak，并共享同一固定 apparatus reference；已校准 nominal anchor 可用较低的 `0.10 x reference` detection gate，blind off-grid candidate 必须通过较强的 `0.20 x reference` dominance gate，二者只区分已知位置与未知 topology 的识别置信度，不得读取 current target/support、重标或归一化 depth。原 nominal 35-site map 仍是 Calibration/Occupancy 的离散 reference；任何 add/remove/move 改变几何后旧 Calibration 都明确 stale，必须重跑检测，不能让 artifact 静默跟随。
5. 现有随机逐-site efficiency 不再是 trap-depth 真相。若保留 site-dependent fluorescence factor，它只能是小、固定的 detector/readout nuisance；qCMOS brightness feedback 可以连同该真实可见 nuisance 一起校平，但不得把它伪装成 trap depth 或在反馈前按各 site 自己的 bright response 除掉。默认 35-site nominal phase 在 hidden coherent plant 下的 `Imax/Imin` 为 `1.8–2.2`，至少 90% 的初始非均匀误差来自可纠正相干项。同一 phase revision 只传播一次；随后多帧只重采 occupancy、photon 与 read noise。
6. 真实 leaf 只支持已明确的 Hamamatsu LCOS-SLM X15213 series：active phase shape `1272 x 1024`，DVI input raster `1280 x 1024 @ ~60 Hz`，也可用官方 USB frame-memory SDK。DVI 在 active_x 位置嵌入 active raster并要求 exact unscaled presenter；USB 写 frame memory、切 display slot并做 byte-exact image readback。`wavelength_nm` 只记录实验 provenance，radians→gray 只服从实验机实测 `two_pi_gray`，不能由型号或波长猜 LUT。correction BMP 保留在实验机本地，可为 `1272 x 1024` active image或 `1280 x 1024` full raster；adapter 在 orientation 后按 authored sign/offset 合成，不要求把文件上传或提交仓库。开发机的 mocked transport/readback 不是 optical proof；实验机可直接 Scan、填写本机路径/实测参数并 bring-up，但最终 orientation、correction sign/offset、settle 与光路效果必须在真机确认后才能宣称 hardware acceptance。

### 3.5 SLM Editor 与 qCMOS 原子荧光直接反馈

1. SLM Editor 是 `devices/slm` concrete plugin 自己的独立 device window，继续沿用 descriptor lazy `control_factory`、同一 named SLM 与同一 `DeviceUseCoordinator`。界面只保留 `Mask` 与 `Wavefront` 两个 permanent tab；不得残留 `Mask ROI`/`Advanced` 历史页。默认 `Mask` 页保留 target 与 `Science phase (pre-correction)` 两张既有 `2x2 = 490 x 357` logical plot viewport；新增相位层不得挤小这两个图。一个共享 `Size` selector 只使用 `zlc_plot.PANEL_SIZE_NAMES`，同时调整两个独立 plot host 和 Wavefront 页的第三个独立 host；各画布可滚动，因此更大 preset 不重叠、不裁切，DPR 只改变 raster physical pixels。主页面直接显示 Input pupil 的 applied/draft status 与 Edit、CGH crop、Zero-order steering、Zernike/system wavefront 和 Vendor correction 的 enable/status/Edit或Load入口。Preset 只用一个 popup：Grid/Checker 可编辑 N、M、spacing Y/X与intensity；Rectangle/Ellipse/Ring显示各自尺寸、edge和intensity；popup draft 不触发 solve，只有 Apply 一次 materialize 唯一 target，无效组合保留draft/target/revision并在状态栏说明错误。Wavefront 页左侧一个可滚动参数面板容纳 full-raster carrier X/Y 与正确 Noll Z2–Z11，右侧显示只含当前 enabled steering+Zernike 的独立 wrapped phase preview；carrier只称 `zero-order separation/steering`，并明确需要真实 Fourier-plane stop/aperture，软件本身不宣称消灭零级。Zernike coordinates服从同一个 authored source pupil；Vendor BMP不进入该预览。
2. 左侧编辑唯一 continuous non-negative target；后台 `solve_phase` 只更新 Mask/base phase。Input pupil 参数或 measured image变化会产生新 pupil amplitude并重新 solve当前 target；CGH crop仅在合成时限制 Mask phase，不重新命名为aperture。右侧 science phase只显示 `wrap(enabled carrier + enabled pupil Zernike + optional CGH crop)`，不含 device-local correction/LUT。target/pupil改动仍由 capacity-one latest-only worker求解，Qt owner不做 FFT；Selectors off 保留画笔，on 开放两张主图 selector/zoom/pan。Target JSON、Mask NPZ、science phase NPZ 的 load/save、8-bit phase-mask image import都不写硬件；phase-mask image只接受二维 8-bit gray并按 `gray * 2π/256` 解释，可为device shape或enabled CGH crop shape，明确不是 vendor correction/LUT。Load science phase清零/关闭composition layers，从而保持 exact phase roundtrip。
3. X15213 vendor correction 仍只由 concrete adapter持有并在 orientation 后、measured radians-to-gray前合成，但 Editor 主页面必须可见显示 `Off` 或 `On · filename`，提供 `Load correction` 与 enable A/B；load/enable只改变下一次 Send 的adapter mapping，不修改Target/Mask/science phase/NPZ，也不立即发送frame。virtual/generic adapter显示 `Unavailable`，不得伪装软件 correction。只有显式 `Send to SLM` 取得短命 EXCLUSIVE claim后调用 `apply_phase`；science phase若落后于当前 target/pupil request必须拒绝。关闭 Editor不apply/restore/blank/关闭SLM，commanded phase原样保留。
4. 第一版粗调的唯一 observable 是现有 qCMOS 拍到的 occupied-atom site brightness。每个 candidate phase 下执行作者写下的真实 cool/load/probe pulse 与 exact grouped camera cycles；相机沿用 calibration 记录的 sensor integration，而 pulse 的 probe gate 保持其 authored API values，两者不得互相覆盖。逐 frame 的 BOX counts/occupied/valid 复用 `OccupancyProcessor` 的唯一判断；每个 site 只对 `occupied=True` 且 valid/finite 的原始 BOX mean 做多-shot平均，empty shots不作为零值进入分母，也不按该 site 的 dark/bright response归一化。这个量与 Calibration 的 multi-frame occupied BOX brightness 是同一口径；loop 内不重新找 site，也不逐 site 做 nonlinear Gaussian fit。
5. 反馈只做直接强度比例校正，不拟合 Zernike、modal coefficients、hidden aberration 或连续波前；Editor 中由操作者明确设置的 wavefront coefficients 不属于 feedback 估计。令正 target support 上的逐-site occupied-shot BOX brightness 为 `F_i`，每轮只更新 `w_i <- w_i * (GM(F) / F_i)^0.45`，再归一化 target total、以上一 canonical Mask phase warm-start `solve_phase`、apply、复测并独立保留 best。Task production code不得 import SimulationWorld、读取 hidden trap plane/site intensity或按 simulation seed 分支。
6. qCMOS 只观测当前 calibration 声明的离散 atom sites。Task 必须如实报告这些 site 的 brightness、missing/saturation、总 shots 与 occupied samples不足的 sites，不得声称它看见 sparse sites 之间或 dense flat-top 的每个 pixel；off-grid peak 可以出现在 raw qCMOS frame，但 add/remove/move 后必须先重跑 Calibration，当前固定 5×7 feedback Task 不得拿旧 boxes 继续闭环。Dense target 仍复用同一 continuous target 与 `solve_phase`；完整 dense interior 只由验收端在 Task terminal 后用 virtual oracle 检查，不能倒流成 Task 输入，也不能用 qCMOS 离散点伪装成全平面测量。
7. Task 开始前冻结 target/resources/calibration并保存incoming phase；每个候选依次 apply -> exact qCMOS shots -> direct fluorescence update -> warm-start solve。候选若missing/saturated/non-finite、总校准荧光相对baseline低于90%或统计证据不足，不得成为best。成功最后明确apply best，保存与device command逐元素相同的no-pickle phase artifact并保留best；Stop或任何失败都尝试恢复incoming，恢复失败必须真实报错。Editor close与Task lifecycle相互独立。

...
- Tick label保持默认对应tick的锚点，不得为防重叠向内移或改变alignment。碰撞判断使用最终实际text/data-frame几何，cell间空白可容纳label，不以虚构cell文字边界限制。只有字号可缩小，达到可读下限后接受重叠；Single/Focus按各Axes及方向分别定价，主轴与辅助轴相撞时先使用辅助轴的字号预算，不无条件连带缩小主轴。真实布局变化时，数字及显式label都从声明字号重新规划；Focus空间恢复时字号必须可恢复，稳定布局不重复规划。
...
- Single、Focus与overview的轴标题统一依据最终数据框、外侧tick文字边界及point间距定位，不使用overview独立的figure比例偏移。普通标题、cell标题及顶部额外标题行共用标题间距；科学计数offset仍使用公共locator的角落位置，不冒充轴名的边界。现有renderer独占文字布局，仅既有geometry/text签名变化时重排，不改Panel外尺寸、Image正方形框、数据或limits。

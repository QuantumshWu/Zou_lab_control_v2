...
- Seamless Plan只需列出实际扫描的轴。未列出的Pulse Scan Slot按当前Pulse字段默认值固定，只从本次执行副本解除其scan binding，复用普通Pulse编译为常量；不改作者Pulse、Plan草稿、默认值或额外发送重复常量列。仅计划中的slot占硬件scan列，manual/device-only计划也可使用带slot的模板。输出Dataset只增加Plan显式轴，已有显式单点轴保留；固定字段由实际执行Pulse/run record保存，不伪造扫描维度或增加点数。未知或重命名后的显式port仍由同一bind_plan拒绝。

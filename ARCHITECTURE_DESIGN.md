...
- Image/Heatmap的每个离散data point是一个正方形screen cell；数据内容框的宽高比只由`columns/rows`决定，canonical scan step只控制tick、selector、overlay与fit的坐标映射，不控制cell长宽。规则grid以lattice index作为唯一painted geometry，canonical↔lattice由同一个transform owner转换；zoom只在固定content box内按统一cell尺度修改viewport，不得通过padding canonical extent或重新layout来维持square。50×50 scan必须完整填满square content，即使两个scan轴步长不同。

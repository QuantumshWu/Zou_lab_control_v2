...
- 普通fit收尾的模型Jacobian是本次owned工作区；自由列选择与权重/robust缩放写入其前缀，不另建N×free矩阵。只有列重排存在写后读覆盖时才使用一行scratch；invalid行归零。秩与协方差由最终参数的同一Jacobian及原稳定分解决定，不读取求解器上一trial的信息矩阵冒充最终导数。

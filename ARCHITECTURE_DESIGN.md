...
- Panel Setting companion frame的header显示固定Panel identity，例如`Setting · panel-3`；不得混入可编辑title、signal或structure。右侧有单击关闭该Setting frame的紧凑`×`，它不删除Panel；拖动、owner stacking和再次点击Setting切换仍由同一个companion owner处理。

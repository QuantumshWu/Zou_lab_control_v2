...
- **同时画的面板要在不同进程里画**：live 面板的 renderer 原先全在一个渲染子进程里，编译核 `nogil` 能逃出 GIL，artist 更新、chrome 绘制和每帧 front 的 pickle 逃不掉，于是四块面板的 Python 一次只跑一块。`RenderProcessPool` 按面板分片，策略只有两条：**先铺开再共用**（成员未满先起新的，满了才挑 host 最少的那个——一上来就「挑最少」会把四块面板全塞进第一个，因为只有它存在时它就是最少的），以及**按需起**（只开一块面板的 console 仍然只有一个子进程，一个子进程在画任何东西之前就是两百多兆）。上限 `default_render_process_count()` = 逻辑核的四分之一、封顶 4：子进程是内存决定的，不是核数。10 Hz 的 display beat 下这一条一个数字都不改（四块面板只用三分之一个核，从没有人在等）；它管的是满速——4 面板 1470×1071 DPR 3 实测 camera 4M 54.5→156.6 fps、facet64 image 35.1→104.6、heatmap 44.3→102.1、curve 65.8→108.2。

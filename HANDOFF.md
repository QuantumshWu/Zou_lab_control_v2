...
SLM server candidate已收敛为Pulse同款入口：apparatus只有`slm.hamamatsu_x15213`一个真实类型，init参数只是server host/port；只有server持有本机Hamamatsu USB adapter、profile与correction。客户只在installation握手、phase apply及unknown outcome恢复时联网；其他Editor state读取本地immutable cache。线上是bounded length-prefix/strict-JSON/canonical-float32 raw command，apply核expected command/mapping revision并loud拒绝stale writer；`bin/slm_server.bat`是本机与trusted LAN的同一入口。该服务不含authentication/TLS，不得暴露public Internet。

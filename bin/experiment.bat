...
rem The checkout itself, so the windows below resolve from wherever this was
rem started -- which is the experiment folder, not bin\.
set "ZLC_HOME=%~dp0.."
...
%ZLC_PY_CMD% -m zou_lab_control_v2 device_manager
...
start "ZLC task console" %ZLC_PY_CMD% -m zou_lab_control_v2 task_console !FORWARD!

echo.
echo [3/3] pulse editor -- edit and fire the sequence beside it.
%ZLC_PY_CMD% -m zou_lab_control_v2 pulse_editor !FORWARD!
set "ZLC_STATUS=!ERRORLEVEL!"
if not "!ZLC_STATUS!"=="0" goto zlc_failed

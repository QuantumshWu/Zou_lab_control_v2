...
def test_task_report_coordinator_closes_failed_prepare_once_and_export_fails_closed(
    session,
    tmp_path,
) -> None:
    """The lifecycle owner never double-closes or releases a live exporter."""

    from threading import Event
    from types import SimpleNamespace

    from zlc_workbench.panel_state import PanelState
    from zlc_workbench.task_reports import (
        TaskReport,
    ...
    node, _snapshot = _one_shot(session, producer="report-probe")
    trigger = node.signal_key("frames")
    publication = session.signal_plane.freeze().publication(trigger)
    ...
    class Page:
        ...
        title = "Page"
        signal = "frames"
        plot_input = publication.value(trigger).snapshot
        spec = object()
        parameters = {}
        facet_thresholds = ()
        fit_options = {}
        ...
        def panel_state(self, *, signal: str, interval_ms: int) -> PanelState:
            return PanelState(signal, "image", "2x2", interval_ms, self.title)

    gate = Event()

    def export(_destination, _surfaces):
        if not gate.wait(2.0):
            raise TimeoutError("test export gate did not open")
        return ("written",)

    export_spec = TaskReportExport("artifact", "probe.v1", export)

    class Registry(TaskReportRegistry):
        def __init__(self, selected_export) -> None:
            super().__init__()
            self.selected_export = selected_export

        def build(self, _spec, _descriptor, selected_publication):
            return TaskReport(
                "probe",
                "Probe report",
    ...
    class Surface:
        def __init__(self, error=None, *, descriptions_ready=True) -> None:
            self.error = error
            self.close_count = 0
            self.descriptions_ready = descriptions_ready
            self.configured = error is None
            self.host = object()
    ...
    def observe(coordinator) -> None:
        coordinator.observe(
            node_id="probe",
            descriptor=SimpleNamespace(),
            report_spec=SimpleNamespace(adapter_key="probe"),
            publication=publication,
            trigger_signal=trigger,
            artifact_results=(
                {
                    "name": "artifact",
                    "contract_id": "probe.v1",
                    "path": str(tmp_path / "probe.json"),
                },
            ),
            signal_for_output=lambda _name: trigger,
            interval_ms=400,
        )

    failed = Surface(RuntimeError("render fault"))
    failed_owner = TaskReportCoordinator(
        Registry(None), surface_factory=lambda *_args, **_kwargs: failed
    )
    observe(failed_owner)
    failed_owner.poll()
    failed_owner.poll()
    assert failed.close_count == 1
    assert failed_owner.take_ready() == ()
    assert len(failed_owner.take_events()) == 1
    failed_owner.close(timeout=0.0)

    export_failure_surface = Surface(descriptions_ready=False)

    def broken_export(_destination, _surfaces):
        raise RuntimeError("disk fault")

    export_failure_owner = TaskReportCoordinator(
        Registry(TaskReportExport("artifact", "probe.v1", broken_export)),
        surface_factory=lambda *_args, **_kwargs: export_failure_surface,
    )
    observe(export_failure_owner)
    with pytest.raises(RuntimeError, match="report export failed before close"):
        export_failure_owner.close(timeout=1.0)
    assert export_failure_surface.close_count == 0
    with pytest.raises(RuntimeError, match="report export failed before close"):
        export_failure_owner.close(timeout=1.0)
    assert export_failure_surface.close_count == 0
    assert any(
        "disk fault" in event.message
        for event in export_failure_owner.take_events()
    )
    export_failure_owner.close(timeout=1.0)
    assert export_failure_surface.close_count == 1

    close_gate = Event()
    close_started = Event()

    def close_export(_destination, _surfaces):
        close_started.set()
        if not close_gate.wait(2.0):
            raise TimeoutError("immediate-close export gate did not open")
        return ("written",)

    pending_surface = Surface(descriptions_ready=False)
    pending_owner = TaskReportCoordinator(
        Registry(TaskReportExport("artifact", "probe.v1", close_export)),
        surface_factory=lambda *_args, **_kwargs: pending_surface,
    )
    observe(pending_owner)
    pending_owner.poll()
    assert pending_owner.take_ready() == ()
    with pytest.raises(TimeoutError, match="report export did not finish"):
        pending_owner.close(timeout=0.0)
    assert close_started.wait(1.0)
    assert pending_surface.close_count == 0
    close_gate.set()
    pending_owner.close(timeout=2.0)
    assert pending_surface.close_count == 1

    mounted = Surface()
    owner = TaskReportCoordinator(
        Registry(export_spec), surface_factory=lambda *_args, **_kwargs: mounted
    )
    observe(owner)
    owner.poll()
    ready = owner.take_ready()
    assert len(ready) == 1
    owner.activate(ready[0], ("panel-report",))
    assert owner.panel_locked("panel-report") is True
    with pytest.raises(RuntimeError, match="while its report is exporting"):
        owner.release_panel("panel-report")
    with pytest.raises(TimeoutError, match="report export did not finish"):
        owner.close(timeout=0.0)
    assert mounted.close_count == 0
    assert owner.panel_locked("panel-report") is True

    gate.set()
    owner.close(timeout=2.0)
    assert owner.panel_locked("panel-report") is False
    # Mounted ownership transferred to PanelBinding; coordinator never closes
    # it, and the ordinary panel owner closes it exactly once.
    assert mounted.close_count == 0
    mounted.close()
    assert mounted.close_count == 1

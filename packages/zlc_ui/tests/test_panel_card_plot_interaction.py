...
def test_what_a_card_says_never_decides_how_wide_it_must_be() -> None:
    """The strip describes; the plot inside is what needs room.

    A QGroupBox title IS a geometry constraint: with the dataset's structure
    put in the title, one card demanded 704 px instead of 149, the board's
    packer could not place it in its slot, cards overlapped, and every
    pointer gesture -- pan, zoom, crosshair, every selector -- landed on a
    card with no canvas under it.  The line is painted now, so it may say
    anything and cost only its own row.
    """
...
STATE = {
    "signal": "@logic/cm/frames",
    "kind": "image",
    "size": "1x1",
    "interval_ms": 100,
    "title": "@logic/cm/frames",
}


def card(summary):
    view = PanelCardView("p", STATE["title"])
    view.set_size_choices(("1x1", "2x2", "4x4"), "1x1")
    view.set_interval_choices((100, 200), 100)
    view.set_panel_projection(dict(STATE), {"data_summary": summary})
    return view


plain = card("")
described = card(
    "repeat 1 x points 3 (frame) x spatial-y 96 x spatial-x 128 "
    "- frame->reduce, spatial-y->facet"
)
assert described.title() == STATE["title"], described.title()
assert described.minimumSizeHint().width() == plain.minimumSizeHint().width()
# One row taller, and only one row: the line has its own room above the plot.
grew = described.minimumSizeHint().height() - plain.minimumSizeHint().height()
assert 0 < grew <= 2 * described.fontMetrics().height(), grew
"""
    )

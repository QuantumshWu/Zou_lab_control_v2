...
// preamble, body and tail three different codes: the body's code must hold
// through every rewind (a rewind that restarted the segment table at segment
...
module tb_loop;
  ...
  zlc_edge_streamer #(.CHANNEL_COUNT(CH)) dut (
    .clk(clk),.reset(reset),.start(start),.prog_count(NE[12:0]),.run_repeat_count(32'd1),
    .loop_start_addr(12'd2),.loop_end_tick(32'd500),.loop_end_coeffs({NS*CW{1'b0}}),
    .loop_count(32'd3),.scan_enable(1'b0),.scan_count(32'd0),.scan_repeat_count(32'd1),
    ...
    .bus_prog_we(bus_prog_we),.bus_prog_bus(2'd0),.bus_prog_addr(bus_prog_addr),
    ...
    .bus_prog_start_value(bus_prog_value),.bus_prog_stop_value(bus_prog_value),
  ...
  endtask
  task prog_hold; input [5:0] a; input [TW-1:0] t0; input [TW-1:0] t1; input [BW-1:0] v; begin
    @(posedge clk); bus_prog_addr=a; bus_prog_start_tick=t0; bus_prog_stop_tick=t1; bus_prog_value=v;
    bus_prog_we = ~bus_prog_we;   // edge -> latch (while reset holds the engine)
  ...
  initial begin
    ...
    prog_hold(6'd2, ticks[5], ticks[7], DAC_TAIL);
    bus_counts[0 +: 7] = 7'd3;

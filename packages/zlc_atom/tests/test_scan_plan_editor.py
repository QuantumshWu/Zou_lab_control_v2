...
The manual row is the one that carries no port and no bounds, so the only
way to know it authors the right document is to build the widget, press the
button, and read what it emitted.
"""
...
def test_a_manual_row_authors_a_name_and_a_point_count_and_no_values() -> None:
    editor = _editor()
    try:
        editor._add_axis()
        ...
        assert len(manual.values) == 4
        # The document holds ordinals until the operator says otherwise; it
        # is a point COUNT that was authored, never a range.
        assert manual.values == (0.0, 1.0, 2.0, 3.0)
        assert not hasattr(row, "start_spin")
    finally:
        editor.deleteLater()

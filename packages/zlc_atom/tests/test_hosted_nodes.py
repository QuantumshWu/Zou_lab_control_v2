...
def test_descriptor_role_is_independent_of_camera_measurement_extent() -> None:
    ...
    derive = descriptors["derive"]
    assert derive.kind is NodeKind.PROCESSOR
    # Its outputs are the names of its program's lines: declared per draft.
    assert derive.outputs == () and derive.declare_outputs is not None
    assert derive.input_specs[0].contract_id is None

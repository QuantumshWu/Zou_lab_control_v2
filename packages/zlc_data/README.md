...
| `selection.py` | Axis-named index/coordinate selections, their tree representation, and selection indexing. |
...
| Selection | `Selection`, `IndexSelection`, `SelectionChange`, `resolve_selection_indices` |
| Persistence | `NPZFormatError`, `save_npz`, `load_npz` |

Some codec and selection functions are intentionally module-scoped; import
them from `zlc_data.codec` or `zlc_data.selection` when their lower-level
contracts are required. The seven validation primitives, including `integer`,
are module-scoped in `zlc_data.validation`. Likewise, `dataset_cell_value`,
...
defined by `_tree.digest`. They bind a revision reference to its schema; they
are not Calibration or Panel Save provenance, a security feature, or authority
to add fingerprints/hashes to those product artifacts. Panel archives can
rebuild the runtime revision reference from the schema they already store
instead of persisting the derived name again.
...
named validity, Value projection, tree codecs, NPZ persistence, and

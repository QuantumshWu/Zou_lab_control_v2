...
class ResolvedPointRows:
    ...
    def __post_init__(self) -> None:
        ...
        # Against a set, not the tuple.  `in` on a tuple is a linear scan, so
        # checking P members against P surviving rows was P^2 comparisons -- and
        # the ungrouped case hands back one group holding every ordinal, which
        # is the worst case and the common one.  It cost 475 ms at 16 000 scan
        # points and 6 s at 50 000, growing with the one thing a scan grows.
        flattened = tuple(item for row in members for item in row)
        distinct = set(flattened)
        if len(distinct) != len(flattened) or not distinct <= set(surviving):

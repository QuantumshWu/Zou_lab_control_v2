...
Live data 使用一条 capacity-one clock lane：owner 冻结 base data revision、display/view authority 和 causal Image overlay authority，analysis worker 在 isolated projection context 中构建 incoming immutable payload；owner 通过 CAS install 提交 data front。若 live fit 已 armed，求解发生在**同一次数据提交事务内**，预算为调用方的刷新间隔减去提交已消耗的时间（`update_data(refresh_interval_ms=...)` 与 live controller 的 commit 共用这一条路径与预算；controller 通过 pacing envelope 携带其真实 cadence）。预算内完成的 overlay 与它描述的数据发布在**同一张 front** 上；超过预算时求解器收到 deadline 信号，data front 立即发布，求解在既有 fit worker 中异步继续——旧 solver 被取消、只求解最新 revision，完成结果必须匹配当前 data revision 与 request generation 才能提交 overlay 和 `FitEvent`。因此数据最多等待一个刷新预算、永远不等待完整求解，慢 fit 不把旧 overlay 画在新 data 上；中间 ingress revision 仍由 latest-only mailbox 合并。selector、viewport、unit 和 resize 不自动启动求解；显式绑定 selector 被删除时，其 request generation 立即退休、solver cancellation event 被置位、overlay 清除。

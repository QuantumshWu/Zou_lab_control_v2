...
def test_each_projection_centres_every_bucket_on_its_own_mean() -> None:
    ...
    # Dense tensor and the explicit NumPy axis reference over the same data.
    ...
    generic = _numpy_projection(
        dense_view, "curve", x, aggregation=Reduction.MEAN, uncertainty=True,
    )
...
def test_far_apart_constant_buckets_have_exactly_zero_sem() -> None:
    ...
    generic = _numpy_projection(
        view, "curve", AxisRef.point("x"), aggregation=Reduction.MEAN, uncertainty=True,
    )

...
def snapshot() -> OwnedSnapshot:
    ...
    return make_snapshot(schema, np.arange(6, dtype=np.float64).reshape(1, 6), revision=0)

def image_snapshot() -> OwnedSnapshot:
    schema = make_dataset_schema(
        repeat_domain(size=1),
...
def test_headless_plot_kinds_have_stable_rgba_goldens(
    ...
    factory = {
        "curve": lambda: curve(selected_snapshot, AxisRef.point("x")),
        "histogram": lambda: histogram(selected_snapshot),
        "image": lambda: image(selected_snapshot, AxisRef.cell_data("x"), AxisRef.cell_data("y")),
    }[kind]
    session = factory()
    try:
        first = session.rgba()

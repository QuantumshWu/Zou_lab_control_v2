...
def _empty_image_session(size: int = 48) -> PlotSession:
    ...
    y_axis = axis("camera_y", values=np.arange(size, dtype=np.int32))
    x_axis = axis("camera_x", values=np.arange(size, dtype=np.int32))
    ...
    spec = ImagePlot(AxisRef.cell_data("camera_x"), AxisRef.cell_data("camera_y"))

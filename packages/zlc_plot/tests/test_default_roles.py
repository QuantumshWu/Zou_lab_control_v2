...
def _scan(dims: dict[str, int], *, repeats: int = 1, data_axes=(), extra_columns=None, roles=None):
    """A Cartesian scan of the named dimensions, with optional cell axes."""

    axes = tuple(Axis.create(name, values=list(range(size)), role=SCAN_POINT) for name, size in dims.items())
    ...
    table = PointTable.from_columns(columns, roles=column_roles)
    return DatasetSchema.create(
        Axis.create("repeat", size=repeats),
        table,
        data_axes=tuple(data_axes),
        point_topology=PointTopology.from_cartesian(axes, point_table=table),
    )


def _cycle(frames: int, *, repeats: int = 1, data_axes=(), role=READOUT_EVENT):
    """One camera cycle: its frames are the point rows, no topology."""

    return DatasetSchema.create(
        Axis.create("repeat", size=repeats),
        PointTable.from_columns({"frame": list(range(frames))}, roles={"frame": role}),
        data_axes=tuple(data_axes),
    )
...
def _sites(count: int = 7):
    return (Axis.create("site", size=count, role=SITE),)


def _history(shots: int, frames: int = 1, *, data_axes=()):
    ...
    return DatasetSchema.create(
        ...
        PointTable.from_columns(columns, roles={"shot": PRIMARY_INDEX, "frame": READOUT_EVENT}),
        data_axes=tuple(data_axes),
    )
...
P = AxisRef.point
D = AxisRef.data
DIM = AxisRef.point_dimension


def test_families_are_read_by_role_never_by_name() -> None:
    families = classify_axes(_scan({"a": 3, "b": 4}, repeats=6, data_axes=_sites(5)))
    assert families.repeat == (AxisRef.repeat(), 6)
    ...
    assert families.topology

    cycle = classify_axes(_cycle(3, repeats=30, data_axes=_picture()))
    assert cycle.events == ((P("frame"), 3),)
    assert cycle.picture == ((D("x"), 6), (D("y"), 4))
    assert cycle.content == ()
    assert not cycle.topology

    history = classify_axes(_history(40, data_axes=_picture()))
...
def test_a_scanned_picture_facets_the_scan_and_reduces_the_repeats() -> None:
    schema = _scan({"a": 3}, repeats=4, data_axes=_picture())
    ...
    schema = _scan({"a": 1, "b": 3}, data_axes=_picture())
    ...
    schema = _scan({"a": 2, "b": 3}, repeats=4, data_axes=_picture())
    assert default_spec(schema, PlotKind.FACET_GRID) == FacetGridPlot(DIM("a"), ImagePlot(D("x"), D("y")))

    # Nothing live to face: repeat is history, not a layout axis.
    schema = _scan({"a": 1}, repeats=4, data_axes=_picture())
...
def test_a_cycle_gives_each_frame_a_cell_and_a_single_image_shows_the_latest() -> None:
    schema = _cycle(2, repeats=30, data_axes=_picture())
    ...
    one = _cycle(1, repeats=30, data_axes=_picture())
...
def test_judged_frames_facet_by_frame_and_walk_the_sites() -> None:
    schema = _cycle(3, repeats=20, data_axes=_sites(35))
    ...
    few = _cycle(3, repeats=20, data_axes=_sites(5))
...
def test_frame_pairs_are_events_like_the_frames_they_came_from() -> None:
    ...
    schema = DatasetSchema.create(
        ...
        PointTable.from_columns({"pair": [0, 1, 2]}, roles={"pair": READOUT_EVENT}),
        data_axes=_sites(35),
    )
...
def test_a_site_resolved_scan_spends_position_before_content() -> None:
    ...
    schema = _scan({"a": 3, "b": 4}, data_axes=_sites(7))
    ...
    three = _scan({"a": 3, "b": 4, "c": 5}, data_axes=_sites(35))
    ...
    one = _scan({"t": 6}, repeats=10, data_axes=_sites(7))
...
def test_a_shot_history_faces_the_shots_and_a_curve_walks_its_own_data() -> None:
    schema = _history(40, data_axes=_picture(40, 500))
...
def test_a_repeated_scalar_at_one_point_is_a_distribution_per_point() -> None:
    schema = DatasetSchema.create(
        Axis.create("repeat", size=20),
        PointTable.from_columns({"x": [0.0]}),
    )
    assert default_spec(schema, PlotKind.FACET_GRID) == FacetGridPlot(P("x"), HistogramPlot())
    flat = DatasetSchema.create(Axis.create("repeat", size=1), PointTable.from_columns({"x": np.arange(4.0)}))
...
def test_bare_scan_columns_nest_by_how_often_their_values_change() -> None:
    """Without a topology the rows say which column is the slow loop."""

    import numpy as np
    from data_factory import Axis, DatasetSchema as Factory, PointTable as Table
    ...
    assert [ref.axis_id for ref, _size in families.scan] == ["row", "x"]
    assert default_spec(schema, PlotKind.CURVE) == CurvePlot(P("x"))
    # Two live scan dimensions of a scalar are a heatmap, bare or not.
    assert default_spec(schema, PlotKind.FACET_GRID) == FacetGridPlot(
        None, ImagePlot(P("x"), P("row"))
    )
    assert fitting_spec(schema, PlotKind.FACET_GRID, cell=PlotKind.CURVE) == (
        FacetGridPlot(P("row"), CurvePlot(P("x")))
    )

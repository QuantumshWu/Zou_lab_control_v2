...
| image 3D bars | 104.2 | 55.2 | 55.9 / 91.5（ROI 41–47） |
...
- **3D 面板仍是最慢的 kind**，但已到本设计的地板：ROI(0.55M) 手势 **41–47 ms**、
  整幅相机(2.3M) **55.9 ms**。逐段查完：compose 分支健康（81 次里 58 次走可复用背景，
  逃生舱不触发）；派生与颜色都已进 numba；采样密度已按柱宽收；缝已按柱宽收。
  **剩下的就是场景本身**——一格一柱＝54.9 万个盒子画进 80 万像素的坐标区，
  即便所有派生平面都命中缓存，扫描线本身也要约 14 ms。**这条地板是裁决**
  （柱数=数据格数），不是没做完的实现。
  唯一还能省的是那块 27 MB 的 `top_rgb` 平面本身（把 `render_height_bars` 的
  `top_rgb` 参数换成 `lut_rgb`，连 rot90 的那次拷贝一起省）——改的是对外签名、
  牵 45 个测试，收益只在 2.3M 那一档，**需要你点头再动**。

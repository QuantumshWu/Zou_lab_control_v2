...
- Post-M6 Goal item 1（2026-08-13）：`figure_viewer` 正式根 launcher 已补齐。根因只是 `zlc_workbench.apps.figure_viewer`、统一 dispatch 与 package dev wrapper 均已存在，而 root `bin` 缺少操作者入口；新增 `bin/figure_viewer.bat` 只调用统一 `_launch.bat figure_viewer %*`，不复制 bootstrap 或创建第二入口，README 同步列出。Windows launcher/CRLF/app smoke 定向证据 `22 passed in 1.99 s`，`git diff --check` 通过；下一步形成该独立里程碑 commit，再进入 Temperature downstream survival，不混入其现有 dirty 文件。

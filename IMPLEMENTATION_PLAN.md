...
- Milestone 6：`REPAIRED / SWEEP COMPLETE / READY TO COMMIT` — server/Context边界保留，Feedback与Simulation物理模型已根修并完成最终残余审计。
...
- Auto-axis repeat policy follow-up：`COMPLETE / READY TO COMMIT` — FacetGrid默认resolver删除三处`AxisRef.repeat()`兜底；自动只选择producer-authored frame/point/scan axis，无非repeat候选时返回None，显式`fate:repeat=facet`路径保留。唯一production owner净删5行，无新增file/class/helper/state；old-red 6项，相关Plot/Workbench 79项、Qt 4项、正式Camera手动repeat-facet 1项green。
- Camera Restart generation follow-up：`IMPLEMENTED / READY FOR ROOT REVIEW` — 旧Monitor取消后不再在其causal generation仍有surface projection在途时poll/retire旧Host；accept/reject后才释放device并activate candidate。正式Camera→auto-preview竞态、queued restart与Qt Producer Restart 3项green；无新增production file/class/state。
- qCMOS SLM Feedback follow-up：`IN PROGRESS` — 用户裁决为单frame多-shot双高斯`bright-dark`、per-site weight/contrast历史控制、用户显式Pulse/exposure、Calibration只供site BOX；当前unit与非hardware lifecycle 34项green，真实virtual闭环仍在完成默认update/validation证据，尚未提交。
...
- Feedback只接受含frozen spots Target的Science Context v2与普通Calibration，并在构造时完成注册，不另选Target artifact；Task自行apply Context起始phase，只更新Pattern并保留pupil/wavefront/system correction。每个candidate复用canonical Camera Measurement sealed Dataset和相同readout projection，以本轮dark-subtracted BOX average直接更新当前Target，不伪造finite value。

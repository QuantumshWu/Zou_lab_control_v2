...
- Active Goal SLM fixed-phase fluorescence feedback root fix（2026-08-16）：`implementation complete; committed without tests at the operator's explicit request`。真实artifact的curve先从`6.49`降到`1.73`，随后在不断变化的phase下回升到`6.26`；根因不是curve renderer，而是Task每轮只执行8轮solve并且只传上一phase，而solver要到第12轮才冻结WGS-Kim远场spot phase，Task也没有跨轮传递`spot_optimizer_state`。因此旧实现每轮都重新选择spot phase，改变了site-amplitude到荧光的干涉映射，并不是论文中的fixed-phase amplitude feedback。当前初始solve明确执行12轮并建立一个只活在本次Task内的caller-owned optimizer state；后续每轮复用同一fixed far-field phase与site weights，只做8轮amplitude update。反馈指数由`0.45`降为`0.25`以适配100-shot噪声；真实run中造成candidate 7空点的90% total gate已删除，total只保留为diagnostic而不参与valid/best。离线同一plant的确定性回放从旧路径的`5.12 -> 2.64 -> 1.37 -> ... -> 4.34 -> 6.03`变为fixed-phase路径的`5.62 -> 2.16 -> 1.39 -> 1.14 -> 1.055 -> ... -> 1.012`。按操作者要求未运行测试；只做diff/文档一致性检查。

- Active Goal SLM feedback observable root fix（2026-08-16）：`implementation complete; committed without a test run at the operator's explicit request`。真实运行的candidate 1中site24是最亮点，BOX occupied-shot mean=`581.909754...`；更新把该点相对权重降至约`0.666`，但candidate 2的coherent trap仍存在、loading probability约`0.478`。Calibration固定BOX threshold=`238.586224...`，而更新后的occupied brightness约`174`，旧Task遂把真实occupied原子误判empty并因`occupied_counts<2`写成missing；candidate invalid后又不更新target/phase，candidate 2起重复同一phase几十轮。当前feedback不再消费occupied threshold：每个valid/finite shot的Calibration BOX feature均进入Welford repeat mean，只减逐sitedark mean，因此observable与操作者`Reduce repeat -> mean`一致，并同时包含depth-dependent loading与probe fluorescence。missing现在只表示finite samples不足，不再表示亮度跨过旧threshold；旧的occupied-shot-only checkpoints均由本条取代。curve仍诚实保存1-based每candidate实际ratio，既不伪造candidate 0，也不carry-forward best；通用renderer未改。

- Active Goal SLM feedback uniformity curve（2026-08-16）：`complete in milestone commit e6ac301; focused old-red 2 failed -> final 3 passed in 0.26 s`。根因是Task已有每个candidate的finite coarse`uniformity_ratio` history并写入NPZ，却只向live plane发布camera average与candidate phase两张image，Monitor没有下降曲线。当前同一plugin owner增加第三个Dataset output `uniformity_history`与普通curve auto-preview；横轴是1-based candidate/update编号，纵轴是该candidate dark-subtracted repeat-mean brightness的`max(F_i)/min(F_i)`，理想方向为下降至1。curve从第一次publication起按`max_updates`预分配固定point schema，未来点为invalid/NaN；每轮递增revision且只填一个新点，因而同一个generic plot session会真正live更新，而不是到terminal才freeze。validation只是同candidate独立复测而不重复点，无效measurement保留candidate坐标并以invalid/NaN呈现；三个outputs共享generation/revision/run record，Stop重发retained candidate时仍保留整段已完成history。descriptor+two-point growth旧红分别因缺output/KeyError为`2 failed`，实现后descriptor/growth/Stop retained三节点为`3 passed in 0.26 s`，且测试守住连续front的schema identity；后续真实run的threshold-censoring根因与当前observable修复以上条最新checkpoint为准。

- Active Goal virtual feedback cooling cadence（2026-08-16）：`complete in this milestone commit; exact cadence node 1 passed in 0.17 s`。操作者当前workspace pulse的`load=0.2 s`使100-shot candidate实测约`26.04 s`；当前把随包imaging template与本机ignored workspace copy的load都明确写成`duration=100, unit=us`，而不是容易误读的秒制小数。其余三帧probe/camera cadence保持不变，完整shot从`260.2 ms`变为`60.3 ms`，因此100-shot coarse authored floor约`6.03 s`；validation若触发再增加约`6.03 s`。产品pulse old-red精确抓到旧`0.002 s`，修改后同一cadence节点`1 passed in 0.17 s`，provenance指向当前checkout。

- Active Goal SLM feedback shot defaults（2026-08-16）：`complete — commit 301c5e0; no test run for a two-value local default change`。Slm Feedback 的旧 authoring defaults 为每个coarse candidate `2000` shots、独立validation `40000` shots；当前只在既有plugin schema owner把两个默认值都改为`100`，操作者显式填写的值、minimum=10、measurement/statistics/quality gates与Task lifecycle全部不变。随后按真实workspace artifact时间定位到操作者当前选择的`workspace/pulses/imaging_template.json`并非随包`62.2 ms`版本：其`load=0.2 s`使完整shot=`260.2 ms`，连续candidate磁盘时间间隔实测约`26.04 s`，与`100*260.2 ms=26.02 s`一致；slow path不是solver、NPZ或preview。实际100 µs pulse修改与验证由上方后续checkpoint记录。

- Active Goal virtual fourfold fluorescence visibility（2026-08-16）：`complete — commit 8dafd57; six focused nodes passed in 12.50 s`。此前low-order plant虽给出约2倍depth spread，却同时把desired-site mean efficiency压低约4.8倍；为把公开荧光调到4倍而只在初始depth区间调Lorentzian会使校正后的深阱穿过共振并反转feedback符号，按current active mean自动重锁probe又会偷读hidden topology并抹掉绝对depth，二者均已old-red否决且无残余。最终plant只在同一physical pupil内增加一项固定、target-independent、`0.186 rad`的panel-row coherent wavefront ripple和有界seed amplitude/phase jitter；它对每个command都走同一8-bit phase quantization、pupil与FFT，不是第二CGH、site gain或per-command normalization。probe固定为`Δ=-1.90 Γ`、Stark slope=`1.15 Γ/U0`，理想uniform nominal depth `d=1`处为`-0.75 Γ`，全部initial/corrected可达depth留在共振同一红侧。seed0 depth=`0.68529–1.36563 U0`、ratio=`1.99279`；移除ripple后ratio=`1.05905`且mean corrected/initial=`0.999881`。共同conditional fluorescence ratio=`3.94012→1.12449`、total保留`93.660%`；public Camera Measurement `20 ms sensor / 5 ms probe / repeat=50 / Reduce mean`的raw BOX ratio=`4.03148→1.57581`、raw total保留`92.368%`、initial BOX与`p_load×F`相关系数=`0.90401`，完整3-frame max=`48763/30764<65535`。直接参数/coherent/nominal-extra/256-seed topology/occupied/public initial+corrected六节点为`6 passed in 12.50 s`，`git diff --check`通过。Feedback best仍只按spread，不最大化总亮度；total brightness现只作为diagnostic，不设baseline门，绝对trap depth反馈留给未来独立frequency/light-shift observable。该simulation切片与checkpoint已收口，不再改物理模型；下一项只由操作者运行Slm Feedback真实验收。

- Active Goal SLM feedback live candidates（2026-08-16）：`complete — commit 3e67c74; focused feedback file 24 passed in 15.60 s`。根因是Task过去在每轮取出qCMOS readout frame后只累计site统计并丢弃整帧，descriptor没有Dataset output/preview，磁盘也只保存terminal artifact；Stop与普通异常共用恢复incoming路径，操作者无法在运行中观察候选，也不能在满意时停下继续使用。当前同一plugin owner为每个实际apply的canonical candidate先原子写`slm_feedback_candidate_####.npz`，写成功后才把它标为durable；随后对真实readout frames做在线repeat mean，并把`readout_average`与同一`candidate_phase`作为同generation/revision/run record的两个image outputs发布，Monitor按既有Task lifecycle自动显示两张transient preview。每轮测量/validation后原子更新同一路径的score/history。Stop重新apply最低有效coarse score候选；若尚无有效项则保留最新完成或当前已durable候选，并重发匹配的camera/phase snapshot，使device、preview与artifact candidate id一致；首存失败且Stop并发到达时恢复incoming，普通异常仍恢复incoming。成功仍保持terminal seal先赢后apply validated candidate并原子更新accepted artifact。旧红覆盖Stop+首存失败会错误保留未落盘phase；完整直接文件当前为`24 passed in 15.60 s`，production净增294行、无新file/class/framework，`git diff --check`通过。下一项只收口约4倍virtual public fluorescence对比；不得混成一个production topic。

- 2026-08-16 commit record：initial-aberration visibility + arbitrary sparse registration 已提交为 `2fce46e`；下条保留本次窄验证与物理数值。

- Active Goal initial-aberration visibility + sparse registration（2026-08-16；其中visibility参数与数字已由上方fourfold checkpoint取代）：`implementation complete in the shared tree; narrow guards green`。（1）旧 `SimulationWorld` 虽然 coherent trap-depth `Imax/Imin=2.034525`，但 `atom_rate=1100 e/s` 使200-count offset/background淹没信号，公开20 ms sensor / 5 ms probe / 50次default `p0=0.5` fresh-load的 `Reduce repeat -> mean` raw BOX ratio旧红只有`1.040363`；该阶段首次接通了按固定cooling期内`Γ∝ω∝sqrt(depth)`的phenomenological finite-capture model与所有sites共同的AC-Stark shift→two-level Lorentzian scattering、50 ms finite bright-state lifetime，不增加hidden per-site gain。当时同一公开路径的depth ratio=`2.034525`、loading ratio=`1.282987`、conditional-fluorescence ratio=`1.492782`，最终raw BOX ratio=`1.876414`，最大raw frame code=`21725 < 65535`；这些visibility参数/数字不再描述当前world，只有模型所有权与测试切口继续有效。（2）feedback不再固定5×7：任意sparse target在已验收的同向x/y、近identity apparatus registration先验下，与当前Calibration SiteMap做确定性的最近一一对应；count mismatch、local ambiguity、unmatched或过大distortion都在Task开始前拒绝，但不把规则对称点集的无标签全局permutation冒充成软件已证明唯一。成功后rows/columns mapping冻结在该Task实例，后续update只复用这份mapping。full-rank shear及horizontal/vertical大倾斜old-red后，任意geometry、拒绝边界及既有measurement/success定向集合为`14 passed in 0.34 s`。Dense interior仍不是qCMOS离散SiteMap observable。

- Active Goal SLM Text readability（2026-08-15）：`complete — commit b5bf00d`。已提交基线的 filled Microsoft YaHei Bold 在约120个site时会把预算消耗在双列粗笔画上，使`ZLab`字距过紧且“神”的中部方格粘连；低分辨率 skeleton 会进一步把直笔画左右扰动，已完全撤销，不得恢复。最终只在现有 Text owner 内优先 filled Microsoft YaHei Regular，使用字体自身 proportional advance、共同 baseline 与一个logical-pixel tracking；`atom_budget`仍只是上限，不抽点凑数，任一可见glyph消失则该字号整体无效。`spacing=4, budget=120`下`ZLab=94 sites`、`神芯=109 sites`，后者无任何 filled `2 x 2` 块；两个精确 Text preset 节点在当前 checkout 为 `2 passed in 0.50 s`。未跑 package/full-tree，X15213 光学验收状态不变；下一项只在实验机观察该 Pattern 的真实光学结果。

- Active Goal SLM device/profile + Pattern closure（2026-08-15）：`complete in this milestone commit; experiment-machine optical acceptance pending`。用户提供的实验机介质`Disc_noinstaller.7z`（SHA-256 `8B88A850D3CFDE031ECEA0626F265836DFD2FF77B8AFE8A7D6B95DDA88EBD283`）静态给出head `LSH0804382`、785 nm readout的256点`phase.csv`、原生`1272 x 1024`多波长correction和官方transport示例；未执行或提交vendor EXE/DLL。新profile只保存serial/readout/default wavelength/full phase curve，不含225常数：852 nm按`2*852/785`目标在curve反插为`224.633404...`并half-up显示`2π gray=225`，同时为全部0–255 phase codes生成nonlinear LUT。真实输出顺序是canonical radians→half-up phase code→native correction add/mod256→nonlinear wavelength LUT；带`NNNnm`的correction做二维unwrap波长换算，不resize/interpolate，`CAL_<serial>`精确验head。USB原生1272宽并byte-readback；DVI左1272、右8列零。Editor最终只有Pattern/Wavefront两页与Input pupil/Zernike/Vendor correction三条主控；Pattern只有exact Grid、几何staggered Checkerboard、Gaussian、Flat Top、中英文Text(min spacing+atom budget)，没有Mask/crop/measured/旧几何残余；Input pupil默认中心圆Gaussian、`1/e²` intensity diameter=height 70%，Off为uniform solver illumination；Steering X/Y与Z4–Z11同属一个Zernike开关。exact grid改变virtual nominal command后只重标world既有hidden low-order plant一行，当时没有改topology/loading；其中旧的equal-loading语义已由上方2026-08-16物理裁决取代。直接证据：X15213文件重新验证`27 passed in 1.49 s`；Editor文件`19 passed`；solver/preset集合`15 passed`；world原两失败节点`2 passed`；按用户要求未再跑package/full-tree。开发机只证明数学、UI与发送bytes，最终orientation、settle、controller drive state和实际光学阵列仍在实验机验收。

- Active Goal SLM software closure（2026-08-15）：`complete; experiment-machine optical acceptance remains external`。两次repository-wide end gate依次得到`1511 passed / 3 failed`与`1514 passed / 1 failed`；四个失败均为clean prior已有的stale catalog expectation、hidden-plant-vs-public-classifier测试假设、Qt popup retirement缺口和异步hidden-plot等待竞态，已分别根修并用其精确节点、`test_console_views.py`全30项、`test_slm_editor.py`全20项验证。用户随后明确要求不再跑完整测试、只需失败节点即可结束，因此第三次full-tree在进行中被主动终止，不伪报全树green。本Goal的软件实现、权威、Checkpoint与分topic commits均已收口；X15213真实orientation、measured gray/LUT、correction BMP sign/offset、settle和光路zero-order separation只可在实验机做最终optical acceptance。

- Active Goal end-gate SLM hidden-plot synchronization（2026-08-15）：`complete in this milestone commit`。第二次repository-wide gate只暴露一个`275ba15`以来的测试等待竞态：Size selector已通过三个public `RasterPlotHost.set_size()`同步发布新front，但测试只等待第三个host的worker-side `logical_size`，未等待隐藏Wavefront widget的独立Qt queued handoff便立即断言旧尺寸。12次production-path probe证明隐藏tab无需切换也会自行更新；修复只把Wavefront widget加入4x4与恢复2x2的既有有界等待，不在Editor手工`setFixedSize`、不绕过plot owner或改变DPR语义。完整SLM Editor文件`20 passed in 8.00 s`。下一步最终重跑repository-wide end gate；不再新增功能。
...
- Active Goal SimulationWorld calibrated topology（2026-08-15）：`complete in this milestone commit; historical loading wording superseded on 2026-08-16`。干净 `4add9b5` 上可复现的4项失败来自两个独立旧基线：hidden coherent plant 的额外 `x1.25` 过调使默认35点 `Imax/Imin` 达到约2.28；同一个current-relative门又同时承担 calibrated nominal anchor 与 blind off-grid peak 的识别，数学上无法既保留checker的弱真阱又拒绝move/default旁瓣。修复只在world owner内删除旧过调，并继续从同一FFT/raw local-peak truth取depth：nominal anchor使用`0.10 x reference` detection gate，未知off-grid candidate使用`0.20 x reference` dominance gate；没有读取target/support、归一化depth或让Calibration/Task读取hidden truth。该切片当时未改fresh loading；上方2026-08-16裁决现明确以同一raw depth驱动finite capture。200-seed A/B screen选择该方案是因为最窄topology margin仍为`+0.00987 x reference`，而备选`.15/.25`方案的checker nominal margin仅`+0.000534`。256 seeds下default/checker/add/remove/move/3 nominal/3 off-grid共七种topology逐点一对一正确，默认ratio=`2.062401295–2.145299673`、可纠正相干比例=`94.680–95.361%`。正式256-seed守卫`1 passed in 5.50 s`；完整virtual physics + feedback + Temperature当前checkout精确集合`50 passed in 37.16 s`，独立review的Calibration/Occupancy/feedback纵向也回到35-site轴。plant重调按预期改变同seed qCMOS science bytes（907 pixels），独立MOT frame仍逐字节相同，未串扰MOT physics/RNG。该结果明确取代下条hot-solve checkpoint末尾记录的4项待修失败；实验机optical acceptance仍是下一项，不得把virtual topology proof冒充真机proof。

- Active Goal SLM exact CPU hot solve（2026-08-15）：`complete in this milestone commit`。没有使用 GPU、裁物理 pupil、降采样、snap target 或放宽 `max/min <= 1.01`。规则 spot array 的 WGS-Kim 现在把 fixed far-field spot phases、site weights 与 target amplitudes作为 Editor-owned、caller-owned 的短命 state；只有同一非空 spots support 与同一已应用 pupil/Pattern authority 可复用，且只由 latest matching successful solve 接纳，support/objective、Clear、任何 pupil Apply、Load science phase、stale/Stop/failure/close 均不会提交它。state约 `2.7 KiB` plain data，只活在Control/request exchange，不进入 metadata、NPZ、serializer或device truth。小 Cartesian envelope（包含 Checkerboard/Text holes）的 forward/inverse 使用对完整 `1024 x 1272` DFT 数学等价的 column-first selected transform；非 Cartesian或成本过大的 envelope回完整FFT。显式`iterations=N`真实执行N次forward/update/inverse；默认停止只以**实际返回的canonical float32 phase**重建full-resolution field并独立传播后的ratio为准，一次精确通过已经证明该确定性返回值满足1%，不再重复非物理三连门。当前checkout provenance实测：cold 5x7为`0.395 s / 22 iterations / ratio 1.009030`；同support ±0.5% target变化 accepted P50/P95=`0.0871/0.0882 s`、2 iterations、ratio=`1.007999`；±2%为`0.114 s / 4 / 1.009298`，±25%仍为`0.280 s / 16 / 1.008503`。约10–15 ms只属于fixed-phase全分辨率核心候选合成，不冒充包含canonical phase重建与exact quality proof的public accepted solve。不得把数值phase速度等同qCMOS/SLM settle后的闭环墙钟。

> 上述 returned-phase 单次精确验收是当前停止规则，并明确取代下方早期 solver/Editor redesign checkpoint 中的“连续三轮”历史记录；旧性能数字只保留为演进证据。

- SLM solver input-pupil seam（2026-08-15）：`complete in this milestone commit`。`solve_phase(..., pupil_amplitude=...)`现在接受与target同shape的full-resolution finite non-negative near-field amplitude，拒绝shape错误、non-finite、负值与全零；`None`精确保留原default pupil。Authored amplitude贯穿selected-DFT/FFT/MRAF每轮pupil projection、final propagation与efficiency，不改1% quality gate，metadata只增加plain `pupil_source=default|provided`，不持久化array/hash。Old-red 3项先因缺参数失败；新增3项与相关solver精确集合`9 passed in 0.47 s`。

- X15213 runtime correction seam（2026-08-15）：`complete in this milestone commit`。Concrete adapter现在以只读`correction_path/name/enabled/available`公开实验机本地map状态，`load_correction()`在不发frame/不改`last_commanded_phase`的前提下完整验证active/full-raster 8-bit BMP后原子替换并默认启用；`set_correction_enabled()`支持明确A/B，无map启用会拒绝。Editor command worker与GUI Load/Enable由leaf-local lock线性化，correction仍只在下一次explicit Send的orientation后、measured gray mapping前合成。Old-red 4项先因缺API失败；完整X15213回归`25 passed in 1.16 s`，与solver pupil联合定向`28 passed in 1.54 s`。

- Active Goal SLM Pattern/UI reconciliation（2026-08-15）：`complete in this milestone commit`。Editor只保留`Pattern`/`Wavefront`两页；Pattern的Target与`Science phase (pre-correction)`仍是两个独立`2x2=490x357`host，Wavefront使用第三个独立host，共享`Size`并各自滚动，DPR只改变physical raster。主页面只有Input pupil、Zernike、Vendor correction三条：Input pupil默认正中心圆Gaussian且`1/e²` intensity diameter为SLM高度70%，中心/X/Y直径可调，Off只把solver illumination改为full-raster uniform；configured circle继续定义Noll unit disk。单一Pattern popup只有exact-spacing Grid、长短行几何交错Checkerboard、Gaussian、Flat Top和中英文Text；Text同时服从minimum atom spacing与atom budget，缺字明确拒绝，不画tofu方框。Steering X/Y与Z4–Z11都在同一个Zernike On/Off内；没有Mask、CGH crop、Measured pupil、Rectangle/Ellipse/Ring或独立steering残余。Correction Load/A-B不发frame、不改science phase，只有下一次explicit Send由X15213 leaf合成。正式real-screen三host不overlap/裁字的既有证据仍有效；本次相关Editor文件`19 passed`，solver/preset精确集合`15 passed`，实验机optical效果仍待bench验收。

- Active Goal SLM solver/Editor redesign（2026-08-15）：`CPU-only exact full-resolution performance slice complete in this milestone commit`。用户明确禁止 GPU、降采样和降质量。旧根因不是 worker deadlock：X15213 `1024 x 1272` sparse WGS80 约 `10.36 s`，仅 5 个相邻 Toggle pixel 又会因 `largest_component > 4` 被误送入约 `42.82 s` 的 MRAF300。现有 solver/Editor owner 内已改为：target 仍是唯一 intensity array，另以 `spots|image|auto` 记录作者的物理 objective；Toggle/Brush/grid/checker 明确是 spots，连续 preset 明确是 image，legacy auto 不再因 5-pixel component 发生算法断崖。spots 使用 WGS-Kim、support-only phase、循环外 shift 与数学等价的 Cartesian selected-frequency DFT/完整 FFT fallback；所有 `1024 x 1272` physical pixels仍参与，不 snap、不 crop、不降采样、不用 GPU。显式 iterations 精确执行；默认仅在 normalized support intensity ratio `<=1.01` 连续三轮后提前停，否则跑满80。layer/ROI/plot-size操作不再覆盖仍在运行的 `Solving latest target…`。旧红先有4个 solver objective/API fail，再补 auto-adjacent 与 Brush objective/solve-status 2 fail；最终 Editor全文件+presets/objective/reference-equivalence/early-stop/solver/artifact精确集合 `19 passed in 4.93 s`。X15213 5x7同40轮：旧 shifted/full-phase `5.136897 s`、ratio `1.07461858`、efficiency `0.58283722`；新 selected-DFT/WGS-Kim `1.346582 s`、ratio `1.00021291`、efficiency `0.57824290`，约 `3.815x` 且保留 `99.21%`效率；默认在 `0.867268 s`、24/80轮达到 final ratio `1.00630867`。下一项进入用户已明确要求的两页 Editor、参数化 preset、可见 correction 与真正 solver-owned aperture；不得把模型ratio冒充真机均匀度，也不重新引入 GPU/降采样。

- Active Goal status（2026-08-14）：`complete in this milestone commit — M5 occupied-shot brightness, Editor selector, and virtual SLM-to-camera causality`。用户已解除此前 finite-shot/dense 裁决阻断，并把产品事实进一步明确为 Calibration 同口径的 **occupied shots site box brightness**；不得再以各 site 自己的 dark/bright response 归一化，也不得把 empty shots 作为零值混入均值。当前只继续 M5，不重开 M2。
- Active Goal X15213 / SimulationWorld reconciliation（2026-08-15，device结论保留；旧equal-loading结论于2026-08-16被取代）：`complete on the development machine; experiment-machine optical acceptance pending`。真实 descriptor 为 `slm.hamamatsu_x15213`；generic `Scan hardware` 枚举官方SDK可读head serial的USB candidate与`1280 x 1024 @ ~60 Hz` DVI display candidate，后者没有可靠EDID identity并由操作者确认。设备profile `LSH0804382`保存serial、785 nm readout与完整256点phase curve，不保存225常数；852 nm从curve反插得到raw `224.633404...`并half-up为只读`2π gray=225`，同一curve生成完整nonlinear LUT。输出顺序固定为canonical phase→phase code→native correction add/mod256→nonlinear wavelength LUT；显式波长correction做二维unwrap wavelength conversion，禁止resize/interpolation。USB发送原生`1272 x 1024`并byte-readback，DVI固定左1272列、右8列零。相关X15213文件`27 passed`。SimulationWorld每个trap-high cooling rise仍逐site独立fresh draw，但概率现在由同一coherent local depth经`p_i=1-(1-p0)^sqrt(d_i)`决定，不再是所有active sites相同的base Bernoulli；exact-grid的topology gate不变。开发机mock/readback只证明frame bytes，不是optical proof；最终orientation、settle、controller drive state与光路效果仍待实验机验收。M2继续关闭，不得重开。
- Active Goal M5 observable correction（2026-08-14）：`complete in this milestone commit`。`SlmFeedbackTask` 现在固定消费 Calibration `BOX` feature，由同一个 `OccupancyProcessor` 给出逐 frame/site `counts/occupied/valid`；每个 site 用独立 O(1) Welford 状态只累计 `occupied=True` 的原始 box feature，empty shots不进分母，invalid/non-finite 与 occupied样本不足仍让候选失败。旧红用 empty count=999、occupied count=20..54 证明旧 normalized/all-shot公式给出错误均值；修后直接 feedback 文件 `13 passed in 14.83 s`，模块来源均为当前 checkout。下一项只做 Editor Selector 开关，然后核实/修通 explicit Send 后 virtual trap geometry/loading/qCMOS 的因果链。
- Active Goal M5 Editor Selector（2026-08-14）：`complete in this milestone commit`。同一个 plugin-local `SlmEditorControl` 增加可见 `Selectors` switch，默认 off 保留 Toggle/Brush/Erase；on 时停止 painting、禁用两个画笔参数，并通过两个现有 `RasterPlotHost.set_interaction_enabled()` 同时开放 target/phase 的 selector、wheel zoom 与 pan，target event filter 完全放行而不会误改 target。真实 Qt pointer/wheel 旧红先因开关不存在失败；修后从 off click paint、on AREA selector、target bytes不变、viewport zoom、off恢复画笔的纵向守卫通过，Editor 全文件 `7 passed in 3.20 s`。下一项只处理 explicit Send 到 virtual trap/loading/qCMOS 的断链。
- Active Goal M5 SLM-to-qCMOS causality（2026-08-14）：`complete in this milestone commit`。Editor 的 candidate phase 现在携带其 target request revision；target 一变即 stale，matching solve 完成前 `Send to SLM` 明确拒绝，不能再把旧/incoming phase 误发给 device。`SimulationWorld.render_frame()` 在每个 trigger 上消费最新 commanded phase revision；原 calibration 35 sites 的 occupied BOX brightness 按同一 coherent plane 的 local trap depth变化，实际 propagated plane 上达到物理装载门的 off-grid peak 则按 nominal SLM↔qCMOS 仿射投影成新增 camera spot，loading/survival/PSF 都在 world owner 内完成。新增/删除/移动 site 后旧 Calibration 明确 stale，public occupancy仍只投影原 nominal 35-site calibration skeleton。旧红分别证明 stale Send 会错误接受、同 occupied 的不同 trap depth 原先生成 byte-identical frame、移动 spot 原先永远不出现在 qCMOS；修后三个直接文件合跑 `37 passed in 20.89 s`，既有 Camera→Occupancy 与 auto-Calibration 纵向 `2 passed in 26.24 s`。正式可见验收只用公开 `zlc_ui.capture_window`、Windows real-screen 与真实 `ExperimentGuiFlow` virtual session：Editor target/actual 都是 `1024 x 577` logical，DPR 3 crop `3072 x 1731`、desktop canvas `3840 x 2400`，Selectors可见、solver idle、hardware unchanged，结束后三个窗口、session、control、claim全部归零；没有 computer-use、offscreen、direct grab或手工缩放。
...
- Active Goal Milestone 4：`complete in this milestone commit; physics wording superseded by the 2026-08-16 checkpoint above`。唯一 `slm.phase`/窄 `SlmAdapter`、唯一 continuous non-negative target、自动选择 weighted GS/MRAF 的唯一 `solve_phase`、strict target JSON/no-pickle canonical phase NPZ、VirtualSLM 与 `SimulationWorld` 中唯一 commanded phase/revision 已落地；hidden coherent optics（8-bit phase、低阶像差、偏心/渐晕）产生 local trap intensity，同 revision 的 qCMOS render 只传播一次，detector nuisance 限于约 1.02×。当前物理裁决让同一raw local depth同时进入active topology、harmonic-frequency finite capture、occupied AC-Stark/Lorentzian fluorescence与release survival；不存在所有active traps等概率的旁路。该 milestone 的 solver/profile 与边界证据仍是历史记录；真实 X15213 leaf 的当前状态只以上方最新 reconciliation 为准。不得重开 M2。
- Active Goal Milestone 5：`in progress — repeat-mean qCMOS brightness feedback + plugin-local layered SLM Editor`。当前唯一observable是Calibration同口径、逐site dark-subtracted的all-shot BOX repeat mean，不再使用会随trap brightness失效的occupied threshold。初始12轮WGS-Kim建立并冻结far-field spot phase；后续在同一caller-owned optimizer state内执行 `w_i <- w_i * (GM(F)/F_i)^0.25`，只调site amplitudes。禁止feedback拟合Zernike/modal/hidden aberration；Editor中operator-authored Zernike只是显式wavefront layer。更早occupied-shot、`0.45`、每轮重新选spot phase及90% total gate均由上方2026-08-16最新checkpoint取代。
- Active Goal Milestone 5 latest-HEAD reconciliation（2026-08-13，observable wording superseded on 2026-08-14）：sensor exposure 与 authored probe gate 的分工、`OccupancyProcessor.evaluate()` 的唯一 occupied/valid 判断及 terminal lifecycle 证据继续有效；其中“dark/bright detector normalization”已被用户裁决废止，当前 loop 只累计 occupied-shot raw BOX brightness。旧测试计数只描述当时 HEAD，不可代替当前 layered Editor、X15213 和 SimulationWorld 合并验证。
- Active Goal Milestone 5 finite-shot/performance gate（2026-08-13，旧science数字superseded）：每个qCMOS shot走公共finite sequencer、exact grouped frames且不保存全部frames的lifecycle/performance事实继续有效；当时特定loading/release proxy的收敛率、shot下界和observable比较不能再引用，但这不否定2026-08-16当前的harmonic-frequency depth-dependent capture law。当前raw occupied BOX observable的finite-shot/multi-seed结果等待最新实现验证，不降低1% criterion、不读hidden truth，也不预填数字。
- Active Goal Milestone 5 terminal ordering（2026-08-13）：feedback 在每个 finite chunk 的 `fire()` 前、每个 shot、每轮 update 与 validation apply 边界都检查 Stop；最终不可逆的 best apply/save 使用 `NodeExecutionContext.seal_terminal()` 与 Host Stop 在同一 `_start_lock` 原子排序。Stop 先赢会拒绝 seal，并经同一异常路径恢复 incoming、零 artifact、Host cancelled；seal 先赢后到达的 Stop 不再重标，apply/save成功保留 best/artifact，二者任一异常仍恢复 incoming并由Host真实标 failed。相关Task+runtime terminal/cancel集合 `31 passed in 26.42 s`；没有新增 lifecycle class/file或check-then-act假边界。
- Active Goal Milestone 5 observable screens（2026-08-13，superseded）：旧loading/release proxy与paired-ratio数字不再构成science evidence或shot预算；被废止的是当时具体proxy和数字，不是2026-08-16当前的finite-capture depth law。paired ratio没有进入Task；当前产品只使用用户裁决的occupied-shot raw BOX brightness。是否达到1%以及需要多少shots只由最新exact qCMOS finite-shot验证决定，仍禁止fit、空间平滑、simulation fast-forward或hidden truth回流。
- Active Goal Milestone 6 qCMOS owner profile（2026-08-13）：两个实测热点均只在原 owner 内优化且保持 science bytes。`SimulationWorld.render_frame` 只预计算固定 site detector PSF 几何 spot，并保留每帧旧 scale 的原左结合顺序；256 frames `1.869 -> 0.223 s`（约 `8.39x`），`6,291,456` 输出 bytes 的 SHA-256 `2c214a93…50cc7` 与逐帧旧算法完全相同。`extract_psf_signals` 只向量化完整等尺寸 PSF cut 与 annulus median，最终每 site `np.sum` 顺序不变，384-frame extraction `0.241 -> 0.129 s`（约 `1.87x`）并对 uint16/float32/float64、padding 0/1/3、NaN ring/cut 逐字节等价。没有新增 class/file/cache manager；这些提速降低软件开销，不改变 logical pulse duration 或 finite-shot统计下界。
- Active Goal Milestone 6 outer/solver profile（2026-08-13）：优化后的真实 128-shot `_measure` 为 `7.976–7.988 s`，其中 authored pulse logical wait `7.405 s / 92.7%`，virtual render/fire `0.366 s / 4.58%`，Occupancy（含 PSF）`0.174 s / 2.18%`，publication/dataset 约 `0.029 s / 0.37%`，两次 WGS8 solve 与 apply/restore 合计 `0.020 s / 0.25%`；384-frame raw buffer `9.44 MB`、tracemalloc peak `10.43 MB`，online statistics 仅 560 B。该结果说明软件优化已不能缩短 operator-authored physical cadence，禁止通过 simulation 快进、hidden truth 或减少 exact qCMOS publications 伪造 M5 墙钟。128² solver 维持现实现：WGS8 p95 `11.24 ms`、WGS80 p95 `82.73 ms`、uncontended MRAF300 约 `0.36 s`；FFT 只占约 8–9%，workspace/array-reuse prototypes 没有稳定收益且部分更慢/更占内存，因此不新增 cache/workspace/backend/GPU。warm start 虽仅约 2% latency收益，但 WGS8 RMS error 改善约 71%，继续作为 science/convergence seam 保留。
- Active Goal Milestone 6 Editor owner profile（2026-08-13）：target/toggle/brush/preset/set-phase 等 Qt owner handler p95 均约 `0.5–2.9 ms`，solver completion slot约 `0.07 ms`，latest-only/capacity-one/warm-start均已直接验证；唯一阻断是旧 `Send to SLM` 在 Qt owner 内同步调用 `apply_phase`，80 ms adapter 会冻结 owner `81.35 ms`且没有 heartbeat。现已在同一 plugin class 内增加独立 `slm-command` 单 worker：点击时冻结 canonical phase，worker完整执行短命 claim -> apply -> exact confirm -> release，queued Qt completion只投影结果，active command 时 close 非阻塞拒绝并由既有 timer 重试；不新增 class/file/framework。20次80 ms adapter seam 的 Send-return p95 `1.10 ms`、5 ms heartbeat gap p95 `8.78 ms`，solve/command/Qt为三条不同线程；Editor全文件 `6 passed`、Workbench同session close-refusal/retry `1 passed`。下一项只处理 profile 发现的 MemoryRegisterTransport 诊断日志无界增长（若全树 consumer 允许在原 transport owner 内有界化），随后进入 M6 里程碑定向回归；不重复 solver/profile搜索。
- Active Goal Milestone 6 final owner corrections（2026-08-13，旧science-byte公式于2026-08-16 superseded）：World仍只缓存固定几何`spot`并逐site累加；当前电子期望值改为`atom_rate * finite_bright_time * common_Stark_Lorentzian_scale * detector_gain * base_area / (sigma_x*sigma_y) * spot`，因此旧`atom_rate * probe * ...`字节hash只保留为历史性能证据，不能约束新物理。`MemoryRegisterTransport.write_batches/read_log`经全树核对只有诊断测试消费者、不是transport protocol或产品audit API；为保持公开source compatibility，默认仍是完整`list`历史及原slicing/equality/index/clear语义，仅由三条长期产品owner（VirtualPulseStreamer、remote memory backend、Workbench virtual dial）显式`record_history=False`，10,000次关闭模式读写后两份历史仍为空而寄存器状态正确。定向consumer/owner `10 passed`；transport结论不受本次world物理更新影响。
- Active Goal Milestone 6：`complete — commit 7a5cd61; sparse-geometry gap superseded on 2026-08-16`。合并回归暴露并修正了两条真实时序/数值边界：feedback 的 raw-moment variance 在常量 normalized strength 上产生 `6.28e-9` 假 SEM，现改为 O(35) vectorized Welford、零 shot 保存；runtime Stop、worker-return 与显式 terminal seal 现由同一 `_start_lock` 排序，worker结束后的 late Stop 保持 done，Stop先发生再退出保持 cancelled且 LIVE outputs 在terminal前detach。优化后的单次128-shot `_measure=7.96854 s`，只比 `7.96160 s` authored floor 多 `6.94 ms`；最新 Editor/feedback/PSF/finite scan/sequencer/transport/runtime/Workbench exact-session 与 archive/overlay 合并回归为当前树 `118 passed in 31.66 s`，同一 commit 的干净 detached worktree 再验为 `118 passed in 37.23 s`，模块路径全部位于该 worktree，验证目录随后已移除。M6 不再有 owner-thread/device-command、产品无界 diagnostic state、science-byte或有证据的 FFT/workspace blocker。当时记录的“Task current5x7-only”缺口已由上方任意sparse trusted-prior registration取代；仍未被qCMOS离散SiteMap覆盖的是dense flat-top interior terminal验收，足量finite-shot sparse 1%证据也仍需按最新物理重验。不得把ideal dense solver、proxy或hidden sparse screen冒充这两项完成。
- Post-M6 Goal item 1（2026-08-13）：`complete — commit d0e83e5`。`figure_viewer` 正式根 launcher 已补齐。根因只是 `zlc_workbench.apps.figure_viewer`、统一 dispatch 与 package dev wrapper 均已存在，而 root `bin` 缺少操作者入口；新增 `bin/figure_viewer.bat` 只调用统一 `_launch.bat figure_viewer %*`，不复制 bootstrap 或创建第二入口，README 同步列出。Windows launcher/CRLF/app smoke 定向证据 `22 passed in 1.99 s`，`git diff --check` 通过。
- Post-M6 Goal item 2（2026-08-13）：`complete — commit 8df10ee`。Temperature 已收敛为 Occupancy 下游的直接 survival 数据产品。Occupancy 仍只拥有逐 frame/site `counts/occupied/valid`；Temperature 每个 exact cycle 复用 `OccupancyProcessor.evaluate()`，唯一 `_survival_validity()` 规定仅 before/after 都 valid 且 before occupied 才形成 survival 事实，值、pooling 与 Dataset validity 不再各自推断。`survival` Dataset 的 unknown cell 由 COMPONENT validity 标记，`survival_rate` 只对 repeats/sites 中的有效 loaded pairs pooling并继续声明普通 curve preview；artifact 与 Final result 已删除 1/e crossing、release-time model、temperature/fit结论，只保存 trap-off、loaded/recaptured 与 survival rate。真实 virtual Calibration -> Temperature -> FINAL/artifact/curve 纵向 `1 passed in 17.87 s`，共享 snapshot helper 的 schema identity/zero-copy直接守卫 `1 passed in 0.29 s`，`git diff --check`通过。
- Post-M6 Goal item 3 design（2026-08-13，`complete as requested design only — commit 751d98e; not implemented, pending user review`）：最小 uncertainty 机制不建立 sibling signal，而是在同一个 `OwnedSnapshot/DataBlock` 中携带 optional、同 physical shape、immutable `<f8` `standard_error`，复用主值 validity 与现有 generation/revision；`ValueSchema`、`SignalValue`、PanelState 和 fit API 均不增加第二身份。Temperature 只为 pooled `survival_rate=k/n` 计算 plug-in 1σ `sqrt(p*(1-p)/n)`，逐 trial `survival` 不带 error。zlc_plot Curve 在一个 source contributor 时沿同一 selector/单位差值 scale 投影 SE，并以每 series 一个复用 `LineCollection` 自动画无 cap 的 1σ whisker；聚合 contributor >1 时保留 central value但不猜独立性传播，fit 永远不把 measurement SE 当权重。无 SE 的 archive 继续 strict v1；有 SE 才写 strict v2，Workbench 仍只保存主 snapshot。明确不设计 CI/Wilson/Jeffreys、band、x error、covariance、weighted fit、image uncertainty、开关、manager 或 registry。本项全程未编辑生产/测试、未运行测试；未经用户裁决不得实施。
...
- Post-goal board gravity correction (2026-08-12)：`f98ce8d` 删除把二维 drop intent 压成 trial insertion order 的 `drop_index`。唯一规则现在是：raw top-left 先选 settled siblings 的二维最近 anchor，dropped card 固定在该 anchor，其他 cards 保持相对 order 并使用同一个 north-west packer 收敛。由此靠近左下格点得到 `1,2 / 3`，靠近左上格点才得到 `3,1,2`；窄屏只 clamp 显示投影，重新放宽恢复 authored anchor。无新 production file/class；相关 zlc_ui 两文件 36 passed。

- Read-only audits (2026-08-12, 未修，证据全部实测)：
  1. **fit 用哪段数据**：裁决单源 `_fit_projection.py:847-865`（显式 selector_kind > AREA > X_RANGE > viewport > 全量），但应用抄三份（curve `:934-964` / histogram `:1004-1029` / image `:1193-1222`）。缺陷：(a) curve/histogram 的 viewport 与 AREA **连纵轴一起裁**（`:959-961` 观测值、`:1026` 计数）→ 纯上下缩放把峰从拟合里删掉（实测幅度 4.95→0.82）；(b) 同一次 facet batch 内各 cell 域不同（cell0=selector/5、cell1=viewport/4）且无人报告；(c) `_fit_facet_batch`（`_session_fit.py:376/427/793`）丢掉 `selector_kind` 而 `:1396-1399` 会带 → 解算域≠报告域；(d) THRESHOLD 在 `_FIT_SELECTOR_KINDS` 却不在 `_DEFAULT_FIT_SELECTOR_PRIORITY` → 缩放赢过画出的阈值线；(e) 分组 curve 只拟合 series[0]；(f) 拖动中的候选框参与拟合域；(g) 只有一条 curve 的优先级测试，image/facet 作用域零覆盖。
  2. **指数模型无 x offset**：`fit.py:1929` `offset + amplitude*exp(-x/decay_time)` 锚死 x=0；rolling 的 x 是绝对 shot 序号，框选第 1200 发 → 种子 A=1.485e+27、该 τ 下 exp(-x/τ)∈[1.6e-28,3.2e-27]、τ 下界处 1e-279 → 数值退化。`damped_sine` 同病。修法：**拟合域平移到窗口起点**（`x - x_ref`），不要加可拟合 `x0`（与 A 简并）；先看 `FitModelSpec.coordinate_relations` 能否承载。
  3. **取消/撤下的 fit 仍占发布位**：fit 通道没有撤销事件（`selection_bridge.py:447-457` vs 选择器侧 `:732-739`），`PlotSession.clear_fit()` 不通知观察者（`_session_fit.py:1524-1574`），console 撤下 `apply_panel_fit → host.clear_fit()`（`panel_state.py:163-165`）到 plot 层就停。实测：un-arm 后十条 fit 信号逐字节仍在、名字仍被占（`already owned by 'panel-1:fit:2'`）、**Outputs 里 fit 开关消失（16→6）使操作者无法撤掉**；取消在飞的求解不发部分结果但**留下的旧答案标记 valid**（FAILED 则写 NaN/invalid，这个不对称需一并定）。干净路径：关该输出开关 / 换另一个模型 / 改投信号或关面板。
  4. **seamless 的 Stop 停不掉 = 一行**：`nodes/scan/seamless.py:206 self.sequencer.wait_done(None)` —— 无超时、无 cancel 检查、返回值还被丢弃；按定义就是"等整张表播完"。Stop 本身只置一个协作标志（`host.py:440 _stop_event.set()`），**cancel 路径上没有任何地方命令板子停下**。实测（虚拟台架，240 cycle）：Stop@4.018s → terminal@8.469s，**4.451s 死等**，worker 停在 `devices/simulation/sequencer.py:92 time.sleep(remaining)`，结束时刻正好等于表的逻辑播放时长（240×35.05ms+0.055=8.47s）。虚拟台架上还有第二处：`seamless.py:183 sequencer.fire()` 会把整张表同步播完（`simulation/sequencer.py:74 → _fire_world`），Stop 打在 600 cycle 表的 2.0s 处 → STOP→terminal 3.88s，Stop 时播了 210/600、结束时 600/600，**Stop 之后又播了 390 个 cycle**；1200→10.1s、2400→24.6s（≈ 剩余行数 ÷ 89 行/s）。这**不是安全等待**：安全是 `finally` 里的 `sequencer.safe()`（`seamless.py:209`，SAFE_TIMEOUT=5.0，实测 0.000s，且不排空脉冲）。**修法仓里已有**：兄弟节点 `stepped_scan/measurement.py:300-308` 是 `while True: _check_cancelled(); wait_done(0.1)`，`calibration/task.py:684` 也传有限超时；三种 streamer 都支持有限超时并提前返回（virtual `:89-90`、local `device.py:455`、remote `remote.py:987-991`），**只有 seamless 传 None**。真机上这一行同样无界：帧节奏使 Stop 多半落在循环里（~0.1s），但**板子若不报 DONE 就永远不返回**（实测 safe() 已中止程序后仍在轮询 3s）。附带：相机 monitor 在 worker 退出后还会在 GUI 线程阻塞 2.004s（`devices/simulation/camera.py:378 read_frame_records`）。

- Facet fit / cell-kind faults (2026-08-12, 实验机报，**全部在 HEAD 5ac38b2 复现**，未修)：
  **共病一句话**：面板记录经由多条路径投到 host 上，而**只有部分路径把记录按"这个 host 真有的词表"过滤**——`semantic`/`display` 在多数点过了 `_declared_only`，`fit` 一次都没过，而换 cell kind 的**重建路径用的还是上一种 kind 的 parameter_surface**。
  1. **fit 模型名 / incompatible**：模型清单单源 `builtin_fit_models()`，但"某个图提供哪些"由 KindHandler 的 `fit_target` 决定，facet 取自 **cell spec** → 换 cell kind 就换了合法模型集。console 把 `state.fit` **整份**交给 `apply_panel_fit`；其内的"只用被提供的模型"守卫**只在调用方传 `models=` 时生效，而 7 个调用点里只有 1 个传**（换 cell kind 的重建、editor、Save Fig 都没传）。于是陈旧 model id 被重新 arm → `_fit_projection.py:434-441` 抛 `fit model 'lorentzian' is not authored for histogram plots` → facet batch 捕获成 `FitOverlay(success=False, diagnostic=…)` → 每个 cell 把它当 fit 文本画出来（`rendering.py:4158` 总览截 24 字符、`:4071` 焦点/单图 72 字符）。"名字不统一"是两件事：下拉把不可用模型用**裸 model_id 当标签**重新插回（一行 `lorentzian` 挨着一行 `Bimodal Gaussian`），且注册表里同一个高斯**真的存了两份**——`gaussian_offset`/"Gaussian with offset"（series）与 `histogram_gaussian`/"Single Gaussian"（histogram），offset=0 时数值等价。
...
2. 每个 panel 建立唯一 Workbench-owned `PanelState`，包含 signal/size/update interval/fixed kind/semantic/display/fit。Setting frame、Panel Edit 和 monitor panel 都订阅它，不保留独立 config 副本。Monitor board 的瞬时几何由 `zlc_ui` 唯一拥有：松手先选 raw top-left 最近的二维 gravity anchor，再固定 dropped card 并让其余 cards 向西北 pack；不得把二维落点降成一维 insertion order，也不得在 Workbench 复制几何算法。

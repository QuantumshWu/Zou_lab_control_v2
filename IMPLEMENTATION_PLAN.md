...
- Active Goal Milestone 4：`complete in this milestone commit`。唯一 `slm.phase`/窄 `SlmAdapter`、唯一 continuous non-negative target、自动选择 weighted GS/MRAF 的唯一 `solve_phase`、strict target JSON/no-pickle canonical phase NPZ、VirtualSLM 与 `SimulationWorld` 中唯一 commanded phase/revision 已落地；hidden coherent optics（8-bit phase、低阶像差、偏心/渐晕）是约 2×初始 site 非均匀的主要来源，局部 trap intensity 同时进入 loading 与 release survival，同 revision 的 qCMOS render 只传播一次，detector nuisance 限于约 1.02×。256 个固定 hidden seeds 的初始 `Imax/Imin=1.961–2.040`，最低可纠正比例 `96.9%`；128² CPU 直接 profile 为 sparse WGS80 `0.077 s`、dense MRAF300 `0.358–0.369 s`，重复 render 零新增 propagation。直接证据：virtual physics `12 passed`；catalog/config 与 capability `5 passed`；foundation/import boundary `4 passed`；Camera/Occupancy 与 auto-calibration 纵向 `2 passed`；Temperature 真 Task `1 passed`。phase 边界/幂等/不可变、continuous graded intensity、strict malformed artifacts、duplicate world alias、close 不 blank、真实 load/loss 调用路径均有守卫。真实 Hamamatsu adapter 继续由实验机 SDK/model/orientation/LUT/settle/safety 资料 gate，不创建伪 stub。下一步进入 M5 的 SLM editor 与 qCMOS 原子荧光粗调闭环；不得重开 M2。
- Active Goal Milestone 5：`in progress — direct qCMOS fluorescence feedback + plugin-local SLM Editor`。用户最新裁决已经收口为唯一简单闭环：qCMOS 只看 calibration 的离散 atom sites；Task 用 fixed site/PSF 对 exact grouped images 做背景与逐-site detector response 归一化，形成多-shot occupancy-weighted fluorescence `F_i`，再直接执行 `w_i <- w_i * (GM(F)/F_i)^0.45`、归一化、warm-start `solve_phase`、apply/复测/保留 best。禁止 Zernike/modal/像差拟合、hidden truth读取和连续波前反演；qCMOS 不被宣称看见未采样 pixel。无限-shot proxy 在未调参的 7 个 hidden seeds 上以 inner solve 8、outer <=120 全部达到 hidden site `Imax/Imin <=1.01`，首次为 62–73 轮，最坏 best `1.009756`；这只是算法方向证据，不能替代 exact qCMOS finite-shot验收。当前实现证据：Calibration artifact 已持久化 strict site-aligned dark/bright response，直接 oracle/codec 边界 `5 passed`；concrete `slm_feedback` Task 只消费 camera publication、fixed PSF 与这份归一化，粗调与独立 validation 分别 authored shot 数，camera 每次最多缓存 `128 x 3` frames且 Task用在线均值/SEM而不保存全部样本；validation 只接受独立 point ratio `<=1.01` 且35个site最大相对SEM `<=0.5%`，不再把逐site `1.96*SEM`虚称为35-site联合95%结论。Task还在fire前核对 frozen camera identity/geometry/readout，saturation只看PSF与annulus实际support，完整CameraCycleSource核对adapter terminal proof；直接 science/lifecycle + cross-node owner守卫 `7 passed`。真实 virtual Calibration 后，同一 camera/sequencer/SLM 连续执行 `2 candidates x 40 shots x 3 exact frames`，两轮 qCMOS ratio 均进入 progress，因 shot/轮数不足诚实失败并恢复 incoming、camera terminal、零 artifact（包含在本轮`7 passed`, `24.62 s`）。plugin-local Editor 已具备同一 continuous target 的 toggle/brush/erase/clear/preset/import、JSON/NPZ、capacity-one latest-only solve、短命 EXCLUSIVE Send 与 close-preserves-phase；关闭不再在 Qt owner 等待 FFT/plot worker，直接 lifecycle `4 passed`。Workbench 同一 experiment session 的 SLM control close-refusal/retry 守卫已通过：第一次 shutdown 如实保留 session，Editor worker退役后第二次完成且 phase 不变（`1 passed`, `3.16 s`）。正式 Windows real-screen 验收只使用公开 `zlc_ui.capture_window`，从 `ExperimentGuiFlow` 初始化 virtual session、按需打开同一 named SLM 的 plugin Editor；`ratio=0.8` 下 target/actual 为 `1024 x 577` logical，DPR 3 抓图为 `3072 x 1731`，desktop canvas `3840 x 2400`，两张 plot 完整呈现，关闭前后 phase 不变、claims idle、session/control 清空且 visible top-level 为零；此前空白或 offscreen/direct-grab 图全部作废。下一步只做文档/受影响定向回归与足量-shot/multi-seed性能门；该性能门属于进入 M6 前必须如实保留的未完成证据。
- Active Goal Milestone 5 latest-HEAD reconciliation（2026-08-13，取代上一条中旧的定向测试计数）：HEAD `9860b95` 将 Calibration frame contract 的 exposure 明确为 sensor 实际积分时间，因此 feedback 已停止用它覆盖 pulse probe gate；operator-authored pulse API values 原样编译，Camera 仍使用 calibration 的 sensor integration。逐 frame counts/occupied/valid 现在只由 `OccupancyProcessor.evaluate()` 判断，feedback 仅负责 dark/bright detector normalization、在线均值/SEM与直接 intensity-ratio update；任一 invalid/non-finite site 精确列为 missing，不能静默当 empty。最新定向证据为 feedback science/lifecycle/cross-owner `6 passed in 0.39 s`，真实 virtual Calibration 后同 camera/sequencer/SLM 的 `2 candidates x 40 shots x 3 frames` 失败恢复纵向 `1 passed in 22.59 s`；Editor 的 binary Toggle、当前 `image` transform、空 metadata、latest-only cleanup 与 lazy boundary 为 `5 passed in 2.32 s`，Workbench 同 session shutdown refusal/retry 为 `1 passed in 3.30 s`。M5 仍未 complete：足量 finite-shot、多 seed 收敛与 M6 性能门尚未执行，且本 milestone 尚未 commit。
- Active Goal Milestone 5 finite-shot/performance gate（2026-08-13）：每个 qCMOS shot 现在只走公共 finite sequencer 路径：对 zero-slot pulse 写 `((),)` scan row，并以 `sweeps=chunk` 表达独立 outer shots；没有 `RepeatRegion`、simulation 分支或 forever cadence。Memory transport 与正式 virtual installation 的两个直接守卫证明 `5 sweeps -> 5 world fires/load events -> 15 frames`，且每组三帧属于同一 shot（`2 passed in 0.88 s`）。真实当前 feedback `_measure` 的 128-shot cProfile 为 `1.963 s`，其中 `OccupancyProcessor.evaluate/extract_psf_signals` 占 `1.425 s`，virtual render/fire 约 `0.477 s`；M6 只能在这些实测 owner 内优化。现行 `2000`-shot coarse 只有约数百分点 site 统计波动，要求 raw 35-site ratio 先跨 `1.01` 才 validation，因而不能作为可达的完成门；`40000`-shot validation 的 35-site extrema 也仍没有足够余量。只用 qCMOS Bernoulli aggregate 的预注册数值试验中，fixed exponent 与 Robbins-Monro 在 `2000/8000 shots x 1000 updates`、六个未见 seeds 上均未稳健通过 `1.01`（最佳 8000-shot方案也只有 `1/6` 通过 P95/P5、`0/6` 通过 max/min，且每 run 已是 8M shots）。不得把该门降级、用 hidden truth 回流或将这一结果写成 M5 complete；下一项只评估同一直接强度反馈在 operator-authored trap-off/release pulse 下能否以更少 shots 提高可观测灵敏度，仍禁止 fit/aberration/model inversion。
...
- Active Goal Milestone 5 direct-observable screen（2026-08-13）：不含 trap-off 的 loading observable 即使用比完整 qCMOS 更乐观的 Bernoulli aggregate，`1M shots/update x 80` 才在一组 48 unseen seeds 上刚达到 hidden P95/P5 最坏约 `1.00985`，物理下界约 171 h；固定局部平均也已否决，因为 `[1,2,1]` 会引入约 10% 空间偏差，cross kernel 不稳定，identity 到 `20k shots x 120` 仍 `0/36` 通过。operator-authored release observable 在 `t_off≈10–12 us` 可提高直接强度灵敏度且仍不需要任何 fit：当前乐观 proxy 中 `50k shots x 120`、12 unseen plant seeds × 3 shot RNG 为 `36/36` hidden P95/P5 通过，最坏 `1.008959`，power 保留 `99.3–99.6%`；但按真实 `35.012 ms/shot` 仍约 58.4 h。不得把 proxy 当 virtual qCMOS验收；下一项只检查同一两帧 qCMOS 的 pre/post 直接比值能否进一步减少 shot 数，仍不读 hidden、不拟合、不做空间平滑。
- Active Goal Milestone 5 paired-release decision（2026-08-13）：两帧 pre/post 直接比值已经否决，不进入 Task。它把同样携带 trap-depth 信息的 loading probability 消掉并额外引入 pre-frame 噪声；即使用比真实 qCMOS 更乐观的 exact occupied-pair oracle，`50k shots x 120` 在 12 unseen plant seeds × 3 shot RNG 上也只有 `1/36` 达到 hidden P95/P5 `<=1.01`，真实 noisy ratio 为 `0/36`，200k 仍不稳健。release 的单 post-frame observable 是三者中较好的直接强度量，但稳健 screen 仍需约 `40M` adaptation shots + `4–10M` independent validation（约 `94–107 h`），现行 `2k/40k` 无法科学支持 1%。因此不实施 ratio/拟合/平滑捷径，不把 M5 写成 complete；当前可执行 Task 保持 operator-authored pulse 与直接 qCMOS fluorescence，同时将 1% finite-shot 门如实保留为物理统计 blocker，继续 M6 只优化实测 owner，不能用 simulation 加速或 hidden truth 替代。

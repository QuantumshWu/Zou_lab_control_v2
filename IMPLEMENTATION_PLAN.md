...
- 可编辑数字共用Decimal定点写法和可见宽度舍入，删除自动科学计数兜底；越界到精确边界，边界放不下时保留原值/单位/范围/宽度并报错。Form与Scan在原Spin提交入口一起接受最终unit/range/value，不再让新bounds先作用于旧单位。Scan的限幅结果写回实际点列；等距判断仅容纳浮点ULP尾差，真实非均匀内点不重排，Values银行保持独立。原步长/单位机制及被动草稿通知不变，不触发设备Apply。14个直接用例与正式TaskConsole单位循环通过，数值框始终105px；输入0得到真实下限20ns，长小数的显示/控件/Plan数值相同，无custom误标。Viewer直接测试仍用finish_close后处理DeferredDelete，避免close被忽略而遗留窗口。

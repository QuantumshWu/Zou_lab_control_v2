...
- Histogram新增`histogram_poisson_gaussian`/`bimodal_poisson_gaussian`（泊松光子计数⊛高斯读出噪声的精确卷积，格点和只有编译核一份实现，SciPy路径与overlay调用同一核；种子取四分位间加权均值与四分位距，σ种子≥√λ/3且不落边界，下限在总宽度上）。合成Poisson+Gaussian直方图（64 bins、5000样本）参数恢复：0.1光子、半数样本为负时λ=0.0998±0.006、σ=0.301±0.004；60光子σ=3给2.59±0.26；双态60/210 σ=3给σ_L 3.15±0.30、σ_R 3.21±0.95、δ 150±0.33；两百光子以上读噪只占方差百分之几，误差棒如实变大，λ与δ和Gaussian的中心/劈裂同样精确。用户报告的「误差很大、不如Gaussian」根因两条：σ种子落在半bin下限上且rate种子偏一个bin时TRF把σ冻在边界（deviance 1.75对1.22）；亮态σ种子半光子在210光子下无梯度、求解器原地宣告收敛（0.47±0.06对真值3）。用户报告的「MOT ROI像素直方图两个fit全错」：single Gaussian的答案（中心−35、σ30的宽坡）是单高斯对「尖峰+长尾」在Poisson deviance下的真最大似然（坡 975 对尖峰 2200，每个尖峰盖不住的尾部计数代价约50），要拟尖峰须用X-range/Area selector限定拟合域或用bimodal Gaussian（尖峰σ1.82+宽尾σ33，正确）；bimodal Poisson-Gaussian则是真bug——σ下限被我改成总宽度后跌到0.03形成comb走样（改为bin/√12），且该直方图是`count`单位的ADU、没有光子格点（现在不提供并拒绝说明）。`run_fit_models`同一直方图feed（两态8±2/30±4，读出宽度2–4格点，对该模型偏宽）：bimodal 3.91 ms对Gaussian bimodal 3.50 ms，single 5.45 ms（单态盖双态的misfit，宽度跑到12格点）对3.04 ms；光子计数直方图（0.7/6光子、读噪0.45、64 bins）编译求解0.7/1.0 ms对Gaussian 0.6/0.7 ms，两路径最优一致到3e-15。`run_mot_roi_chain --panel3 histogram`（新增：40个源索引Histogram cell，逐cell live bimodal fit，像素值故读出宽度数十格点=最差区）：fit_total 22.3对18.6 ms，numeric_fit_batch 13.0对13.7 ms，四面板临界路径97.2对94.4 ms、7.6对7.7 fps；首版259 ms的根因是NumPy孪生evaluator给40个overlay逐项求和，删孪生改调编译核后消失。engine测试的Poisson行用0.3光子读噪、四分之一光子bin（comb可见，宽度可辨识）与十倍amplitude（峰值约70 counts），二十光子的宽度是方差的3%、两求解器会停在不同点。

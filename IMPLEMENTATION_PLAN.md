...
- Auto-axis repeat policy follow-up：`COMPLETE / COMMITTED bf3133e` — FacetGrid默认resolver删除三处`AxisRef.repeat()`兜底；自动只选择producer-authored frame/point/scan axis，无非repeat候选时返回None，显式`fate:repeat=facet`路径保留。唯一production owner净删5行，无新增file/class/helper/state；old-red 6项，相关Plot/Workbench 79项、Qt 4项、正式Camera手动repeat-facet 1项green。
- Camera Restart generation follow-up：`COMPLETE / READY TO COMMIT` — 旧Monitor取消后不再在其causal generation仍有surface projection在途时poll/retire旧Host；accept/reject后才释放device并activate candidate。正式Camera→auto-preview竞态、queued restart与Qt Producer Restart 3项green；无新增production file/class/state。

...
- Active Goal SLM exact CPU hot solve（2026-08-15）：`complete in this milestone commit`。没有使用 GPU、裁物理 pupil、降采样、snap target 或放宽 `max/min <= 1.01`。规则 spot array 的 WGS-Kim 现在把 fixed far-field spot phases、site weights 与 target amplitudes作为 Editor-owned、caller-owned 的短命 state；只有同一非空 spots support 与同一已应用 pupil/Mask authority 可复用，且只由 latest matching successful solve 接纳，support/objective、Clear、任何 pupil Apply、Load Mask/science phase、stale/Stop/failure/close 均不会提交它。state约 `2.7 KiB` plain data，只活在Control/request exchange，不进入 metadata、NPZ、serializer或device truth。Cartesian forward/inverse 均使用对完整 `1024 x 1272` DFT 在35个目标频点上的精确 column-first separable selected transform；irregular support仍回完整FFT。显式`iterations=N`真实执行N次forward/update/inverse；默认停止只以**实际返回的canonical float32 phase**重建full-resolution field并独立传播后的ratio为准，一次精确通过已经证明该确定性返回值满足1%，不再重复非物理三连门。当前checkout provenance实测：cold 5x7为`0.395 s / 22 iterations / ratio 1.009030`；同support ±0.5% target变化 accepted P50/P95=`0.0871/0.0882 s`、2 iterations、ratio=`1.007999`；±2%为`0.114 s / 4 / 1.009298`，±25%仍为`0.280 s / 16 / 1.008503`。约10–15 ms只属于fixed-phase全分辨率核心候选合成，不冒充包含canonical phase重建与exact quality proof的public accepted solve。Editor+solver/pupil/objective/reference/artifact精确集合`34 passed in 8.29 s`；X15213/catalog/import边界另`35 passed in 1.81 s`；`git diff --check`通过。完整`test_virtual_physics.py`仍有干净HEAD即可复现的4项既有SimulationWorld topology/plant失败（初始ratio 2.27775、nominal ghost peak、checker弱site失活、base camera extra site），feedback纵向同受该extra site影响；这些不是本次solver回归，也不得在本条伪装为全树green。下一项应另起单一topic收掉这4项既有virtual-plant基线，再做实验机optical acceptance；不得把数值phase速度等同qCMOS/SLM settle后的闭环墙钟。
...
- Active Goal SLM two-page optical UI（2026-08-15）：`complete in this milestone commit`。Editor只保留`Mask`/`Wavefront`两页且无`Mask ROI`/`Advanced`残留；Mask的Target与`Science phase (pre-correction)`仍是两个独立`2x2=490x357`host，Wavefront使用第三个独立host，共享`Size`并各自滚动，DPR只改变physical raster。主页面可见Input pupil applied/draft、CGH crop、zero-order steering、Zernike/system与Vendor correction；pupil只允许configured ellipse或同raster measured intensity的平方根再裁到该ellipse，只有Apply改变solver source与science metadata，Zernike复用同一support。单一Preset popup参数draft只在Apply时materialize target，无效组合保留popup/target/revision并显示错误。Correction Load/A-B不发frame、不改science phase，只有下一次explicit Send由X15213 leaf合成。Editor旧红与完整文件最终`16 passed in 6.30 s`；Editor/X15213/pupil/solver/catalog/import组合`53 passed in 7.61 s`；`git diff --check`通过。正式`zlc_ui.capture_window`走同一`ExperimentGuiFlow`与`slm`control factory，在Windows real-screen分别抓Mask/Wavefront：target=actual=`1024x577`logical、DPR3 crop=`3072x1731`、desktop=`3840x2400`；人工复核三图独立、无overlap/裁字，未Send且hardware phase不变，session/control/claim/top-level最终归零。实验机vendor BMP、实际orientation/gray/correction/optical stop效果仍属于明确的bench optical acceptance，不冒充开发机已验收。
...
- Active Goal X15213 / SimulationWorld reconciliation（2026-08-14，取代本文更早的“真实 SLM 资料不足”与“depth 提高 loading”描述）：`complete on the development machine; experiment-machine optical acceptance pending`。真实 descriptor 为 `slm.hamamatsu_x15213`；generic `Scan hardware` 枚举官方SDK可读head serial的USB candidate与`1280 x 1024 @ ~60 Hz` DVI display candidate，后者没有可靠EDID identity并由操作者确认。USB/DVI共同使用`1272 x 1024` active phase、authored active-X/orientation、measured `two_pi_gray`与settle；vendor correction BMP只在实验机本地，由adapter按authored sign/offset叠加，不进入solver/artifact。SimulationWorld每个trap-high cooling rise都对active topology逐trap重新抽独立base Bernoulli；depth只决定active/inactive、occupied brightness与release survival，missing trap不能保留或复活atom。该日合并X15213/旧Editor/SimulationWorld/session/qCMOS/Temperature精确集合为`66 passed in 40.53 s`；其中旧Editor页面与截图描述已被上方2026-08-15 two-page checkpoint完整取代，不再是当前UI规格。开发机mock/readback不是optical proof；最终orientation/correction/gray/settle/光路效果仍待实验机验收。M2继续关闭，不得重开。

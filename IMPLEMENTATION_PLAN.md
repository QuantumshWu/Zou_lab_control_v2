...
- Current phase：`新增的Simulation物理/MOT camera/Panel标题/全局10 Hz显示时钟/Calibration PSF kernel grid要求正在实施；此前机器验证基线继续有效但不再代表只剩人工验收。`
- Last completed action：`连续采集共同buffer已从4 cycles增至16 cycles并提交ff2a3b4；frames_per_cycle=3时实际为48帧，完整cycle凑齐即发布而不等待buffer填满。Panel Setting与Panel Edit共享完整semantic/display/fit surface的切片已提交22fd0eb；已完成的Sitemap/explicit overlay/site ordinal与透明度、Distribution独立classifier、Calibration运行中preview和Setting顶层闪窗不得重做。`
...
- Next action：`先在既有SimulationWorld/device_types/templates/tests内做最短纵向修改：deterministic约2倍site效率跨度、per-site椭圆/旋转/偏斜PSF、随机装载与trap-off loss、复用VirtualCamera的mot_camera；不新建物理类或文件。再修Panel默认标题和100 ms全局display tick，最后用现有zlc_plot FacetGrid[Image]补PSF kernel report并做profiling。`

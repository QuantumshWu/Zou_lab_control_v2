...
- **3D 面板仍是最慢的 kind**：ROI 手势 **42–50 ms**、整幅相机 **53.3 ms**，
  ...
  **下一步已定位但未做**：`_materialize` 的逐像素常数。它需要在内核层面查
  （numba 没法逐行 profile），不能靠猜着改——那正是「补丁式修复」。乐观估计
  场景 16.9→8 ms、整帧 42→33 ms，仍到不了其它 kind 的 6–10 ms：那道差距是结构性的，
  heatmap 每帧只是贴一张已算好的 RGBA，3D 每动一次都要重新投影整个场景。

...
- Calibration显示相机曝光（2026-09-24，用户提议）：calibration本来就是开一个camera measurement、用reference窗当曝光武装相机，只是表单上看不见。框架加派生字段`AuthoringField(derived=True)`（`_defaults_from_resources`对派生字段无条件取钩子值、`project_logic_schema`用description做禁用原因、不可required/enabled_when）；calibration加派生字段`camera_exposure_seconds`=`camera_exposure_seconds(reference, readout)`（task.py一处规则），`CalibrationRequest`多一个字段并校验不短于reference窗，measurement按它武装，`to_dict`记录；`_build`按同一规则算而不读表单值，所以绕过表单直接实例化（notebook/测试）也一样。`test_the_form_and_the_request_agree…`在0f8fcd4e就红（`project_values`先对空的必填API字段报Enter，验证器没机会跑），不是这轮的。测试：zlc_atom钩子/派生声明/请求拒绝，console投影显示0.02、禁用原因、改reference后跟着变、草稿不持有。
- Occupancy逐帧行不跟相机的frames_per_cycle变（2026-09-24）：根因=`update_logic_draft`只重投影被改节点自己的编辑器和面板的producer投影，读它信号的节点（占用节点读相机草稿的帧数）的编辑器没人重投影；上一轮的测试读的是`logic_editor_projection`的新鲜计算而不是view里打开的编辑器，所以绿着漏了。修：`_refresh_consumer_editors`——生产者草稿一变就重投影所有以它为源的节点的编辑器，与面板的`_refresh_producer_projections`同一关系；测试改读view里的编辑器投影并加3→2→4→1往返。用户随后拍板去掉「已命名的帧保留其行」的特判：行就是周期的帧，`_reconcile_frame_artifacts`在每次finalization前把超出周期的逐帧路径从草稿里删掉（藏着会让Start被一个看不见的行拒绝），新帧从空开始，单帧周期不留逐帧路径；`project_artifact_inputs(named=)`删除。
- Pulse时长估算没算Bracket（2026-09-24）：编辑器条上方的总时长和预览标题都是Σperiod（一遍），loop完全没算。根修：loop算术（`LoopNode/loop_tree/frame_ticks/frame_visits/bracket_iterations/loop_nesting_depth`）从compile搬到新模块`zlc_pulse.loops`（model与compile之下，一套算法）；model加`PulseSequence.loops`（行loop表，空Bracket不入表）和`played_nanoseconds()`（authored时长精确ns按loop表展开），compile的loop表改从`sequence.loops`取；编辑器总时长与预览标题改用它，tooltip在有Bracket时另报一遍pass的长度；时间轴仍画一遍。测试：model「×3=3倍、嵌套与compile的`duration_seconds`相等、空Bracket不计」、编辑器「加Bracket后总时长×3、tooltip两句、预览候选同数」。
- 手动定过colormap上下限的面板打不开Edit、Save报「'color_min'」（2026-09-24）：根因=`PlotSession._set_configuration_values`在有效relim为fixed时向本次patch索要两端（`prepared[low_name]`），而由存储状态新建的host（Edit tab、Save的临时host、重开console时的面板）在构造期的configure里patch根本没有上下限，于是抛裸`KeyError('color_min')`，raster host以「failed to start: 'color_min'」拒绝一切；活的面板不受影响因为它是先tight后才被写成fixed。修：过渡值取有效值`prepared.get(name, previous.values[name])`，与上一行「有效mode而非patch的mode」同一原则。回归：zlc_plot `test_fixed_limits_at_start`（直接用fixed对构造session）、presenter `test_a_colour_range_set_by_hand_still_mounts_edit_and_saves`（表单定上下限→Edit挂上→Save写出）。探针实测三条路（表单、colorbar手势、先换显示单位再手势）在修前都复现同一句。
- Bracket次数允许1（2026-09-24，用户要求便于调试）：`MINIMUM_BRACKET_COUNT` 2→1（model单源；compile的报错句改从常量取；wire层只拒0，因为引擎把剩余0/1都当最后一遍，0会被播成1）；UI默认下限1（默认新建仍为2）；RTL不改：`w_fin = rem<=1`本就把1当作播一遍。测试：zlc_pulse新增「×1与无Bracket逐拍相同」、非法值表去掉1；pulse editor两条测试改成拒0收1；README/设计文档同步。
- 四个小细节的三处修正（2026-09-23 晚）：①occupancy 编辑器报「internal error … missing calibration path [1]…」，根因=投影给表单的 `artifact_values` 只带草稿里已有的键，而真表单 `reconcile` 要求键集恰好等于表单键（上一轮的 console 测试用假 view，从未跑到 reconcile）→ 投影按表单键逐个取值（缺的给空串），草稿里已命名的帧即使超过当前周期帧数也保留其行以便看见并清掉（`project_artifact_inputs(named=)`），新增真 `LogicEditorView` 测试走 `update_projection`→`reconcile`；②预览 loop 的深度原来=marker 在列表里的序号，两个不嵌套的 Bracket 被画成两层→`_loop_depths` 按包含关系算（不相交同高、包住别人的比里面最深者高一层、同跨度按调用方先内后外），底部/顶部预留空间也按最深层而非 marker 数，渲染器与 presenter 的注释同步改写；③逐帧 calibration 瘦身：processor 去掉 `_place/_model_of/_model_for/_models_by_frame/_runtime*`，只剩一张 `_placed` 表（0=兜底）、`model` 属性（仍是 `readout.select_model` 同一对象）、`_verdict_tables` 按帧堆行索引；run record 只在有逐帧路径时才写 `calibration_paths_by_frame`，否则记录与以前逐字相同（上一轮无条件写入把 `test_hosting_a_processor_on_a_finished_signal_derives_once` 弄红了，现复绿）。行数实话：上一轮的 +1179 是四件事合计；逐帧 calibration 本体源码从约 +590 降到 +486/−97（9 个文件），其测试 +433（含本轮新增的真视图测试与更严的 console 测试）。验证：zlc_atom 逐帧 5+run record 1 绿，邻近 5 个模块的 8 条红与父提交 18ed7c7b 同红（临时 worktree 逐条对照）；workbench 逐帧 4+console logic 40（`test_close_keeps` master 同红）；UI 真视图 1；plot `_loop_depths` 3+raster 2；真屏：不嵌套的两个 Bracket 同高、Run 高一层；occupancy 编辑器四行 picker、frame 2 显示自己的文件、无 internal error。
- 四个小细节（2026-09-23）：①Bracket post标题pill写「Bracket N」（model序号，最外层先），标题字与边框用该Bracket的调色板墨色（`zlc_plot.bracket_color`单一来源，`BracketVM.ordinal/color`由presenter算），预览loop按`PulseLoopMarker.series`用同一墨色、Run固定`pulse_run`灰；调色板`bracket_cycle`改为橙/蓝/紫/绿。②预览loop标签只写「×N」/「×∞」。③Occupancy逐帧calibration：`ArtifactInputSpec(per_frame=True)`、草稿键`calibration_path[k]`、finalization同文件一次解码、build参数`calibration_by_frame`、表单按源周期帧数出「· frame k」行（生产者草稿`frames_per_cycle`优先）、layout保存逐帧键；processor按readout-event轴定每个cell的帧号，逐帧阈值/usable表一次向量化判定，site_ids不一致/帧号越界/无帧轴都拒绝，run record记`calibration_paths_by_frame`。④Calibration三个API字段默认=pulse按period顺序前三个API参数：新框架钩子`LogicNodeDescriptor.resolve_defaults`（资源先解析、只填空字段、每次finalization重算、草稿不写入，表单显示生效值）。验证：UI pulse views 31、workbench pulse editor 120（`test_a_dot_binds`master同红）、plot pulse 14、zlc_atom新增5项+occupancy邻近16项（2条derivation_boundary master同红）、workbench新增4项+console logic 36项（`test_close_keeps`master同红，`test_calibration_pulse_is_a_workspace_file_picker`由此复绿）；真屏：两层Bracket的post标题/边框颜色与预览loop颜色一致，标签只剩×∞/×2/×10000。
- 周期表 + 多层命名Bracket合入master（2026-09-23）：分支`claude/20260914-fieldmon`（host周期表模型/RTL/编辑器/文档/xsim/引擎瘦身/估算器校准，10个提交）与master（spacer种类、选中语义、预览子进程、post定宽、`api:`扫描轴、restart根修等20余个提交）合并。文本冲突5处：BracketPost取分支的item key加master按「10000」量出的宽度、tooltip带bracket id、选中改为按key的填充；presenter的`_schedule_vm`保留spacer计数并改走`brackets=`；seamless保留master的`outer_rows`/`api:`轴逻辑并删掉分支已废除的`slot_tick_scales`（`_program_for`签名、compile调用、重编译调用三处）；设计文档同一区域两边条目合成一份。语义核对：预览的loop marker由分支按`brackets`+`bracket_bounds`从内到外生成，master的括号脚/虚线边界/hatch/按点堆叠标签按marker深度画，直接适用；`insert_spacer`走分支的`_apply_item_order`（按id重算bounds）；codec同时带`kind`与`brackets`列表，`migrate_pulses`把单`bracket`迁成`brackets[bracket_1]`；两条master新测试改到周期表API（`StreamerParams(max_rows=8)`、`BracketPost("b", "end")`）。验证：zlc_pulse 172 passed（3条`test_config_values_on_the_board`红在master同红）；UI pulse views 30 passed；workbench pulse editor 119 passed（`test_a_dot_binds…`master同红）；zlc_atom scan/seamless/board-config/architecture 50 passed（7条seamless + 2条architecture红在master同红）；task console app 24 passed（3条master同红，`generic_device_tune`全量里偶发、单跑绿）。真屏（capture_window开产品编辑器窗口）：无选择Add Bracket包整个Pulse→选中short再Add Bracket得嵌套bracket2→选中long_before后Add Spacer→bracket2的end post输入10000（post 75 px、无narrow拒绝）→切到Preview 90 ms出首帧：三层括号（Run×∞/Bracket×2/Bracket×10000）按深度堆叠、脚在两端、spacer斜线hatch、period边界虚线与最内层括号立柱同高。自查补两处：三个测试fixture pulse经codec重写补上`kind`（spacer那轮在master把`test_pulse_resolver_uses_the_project_json_document`弄红了，现复绿）；merge没碰`fpga/`、`wire.py`、`fpga.py`，与已build的分支顶点逐字节相同，不需要rebuild。**差分仿真**（新增`fpga/pulse_streamer/sim/diff/`，`run_diff.py --old <ttl25边沿表build 0f7dd53c> --new <周期表build>`）：15个变体（一拍行夹长行、三路DAC的edge与Bresenham ramp、整段Bracket×3+Run 2、起于普通行/edge行/ramp行的局部Bracket、TTL 3与1 tick+DAC总线2 tick延迟跨run接缝、一拍行burst×3带与不带Bracket、三点scan表驱动duration slot与DAC slot单独/带Bracket/带延迟）在两边各自的真实top+BRAM IP模型上播放，逐时钟比较25 TTL+4×10 DAC+4 da_clk：10个逐拍相同（含scan——旧的base/delta×scale编码与新的绝对slot值播出同样引脚；FIRE到运行首拍延迟相同）；5个只在「Bracket最后一行以ramp结尾」时该ramp的DAC总线从回绕拍起不同：旧引擎不输出ramp目标值、停在倒数第二步（v4 384 vs 312，v10 910 vs 912并使下一轮ramp起点偏低），而旧引擎自己的延迟路径（v6）在同一回绕拍输出了目标值——旧引擎自相矛盾，新引擎直接/回绕/延迟三条路径一致；TTL与da_clk全部相同。全量套件：zlc_ui 176 passed/5 failed、zlc_atom 1106/20、zlc_workbench 554/34+1 error，红的逐条在合并前的master（505d9505）同名同红。bitstream仍未烧板。
...
- 真实200点×35-site survival Figure的Lorentzian错误已在公共SEM owner根修：旧全series reference让常量bucket相减留下`6.9e-18–1.39e-17`正roundoff，min-positive sigma floor遂产生`7.6e-10` SEM与`1.315e9`权重，使compiled/SciPy、Gaussian/Lorentzian、manual/live都正确优化到错误零线。现全部九条投影路径先取每bucket mean，再以一次second pass同时累积`E[d]`/`E[d²]`；局部前向误差界只归零不可辨识roundoff，不用固定阈值或模型/plot-kind特判。保存selector下Lorentzian由`center/FWHM/A/B=96.62156/.42488/≈0/≈0`恢复为`96.70214/.26766/.81829/.04067`，covariance有效；真实PlotSession live revision 120/140/160/180/200中心为`96.6832/96.7042/96.7022/96.7021/96.7021 MHz`。`0±1e-3`与`1e6±1e-3`的dense/generic/factored/axis/Facet/regular+irregular Rolling均保留真spread，远隔常量bucket精确零；2.04M dense/factored/repeat Rolling为`3.67→4.07/4.44→5.27/7.01→7.60 ms`，仅一份compiled signature。`e52a00f3`只修TRF终止判据，未触碰SEM/权重，既不是来源也不可能修正错误objective。
- 同源数值审查继续删除四处同类历史残余：Rolling trailing从远端`N/S/Q`前缀相减改为逐shot `(n,mean,M2)`与现有Numba owner内的span-block Chan/Welford，100k history的span10/100k热态均约`5.35/5.69 ms`（NumPy逐span原型的100k为2.7s，未保留），整shot gap及`n=1` sample sigma保留；Derive `std`为每个输出bucket选真实样本作锚点的两遍局部归约；Calibration常量population不再产生约`1e16`假separation，exact constant bimodal明确无split；SLM split-half在各半内先锚定真实log样本，完全均匀时严格`variance=error=0`。没有在Fit加epsilon/quantile floor，也没有新增模型、production类或文件。
...
- ROI撤销恢复首次联动前对应字段的草稿，删除applied_selection_values与Camera readback回填接口；唯一Logic草稿继续供Start冻结，执行中的run不变。现有authoring owner仅持每字段的撤销值，手工改写取消该字段撤销，多Panel不复活旧patch。补齐ee8811cc遗漏：真实Scan控件异步舍入曾被当成手改清除撤销权；普通Form、Scan Range/API的normalized来源现沿同一draft patch保留。Curve Area撤销按公共绘制kind同步清除，不能用x_range数值语义删除另一surface。既有真实Plot+Qt Scan重投影例先红后绿；Camera Live/Frozen实际Qt空白点击、取消恢复、Restart工作点及多Panel/手改直接例通过，测试窗口已关闭。
...
- Rolling保留范围恢复为仅window；删除window+trailing−1扩容，以及先全历史平滑再裁输出的旧路径。公共snapshot裁剪在DataView归约前限制本Panel范围，static Repeat与live primary-index同语义；window变化重建view，trailing只改payload。既有用例验证真实Plane的10/20两lease互不改变W10平滑结果、左端partial mean、SEM、双Repeat物理行坐标及参数变化；没有新history/cache/kernel。
...
- Config与Manual Data表格共用2000行阈值：以内展开、整个Tab纵向滚动，以上保留有界虚拟viewport。正式Viewer验证2000/2001行边界，100k行仍只读取可见格子。Config bindings使用Target式retained Fluent行；Config name是始终可编辑的文本，下拉只列已加载文件的真实名称，删除Default/空项/当前自定义名称的伪候选，清空表示不绑定。候选由同一active文件事实投影，不读取未保存编辑表冒充已加载文件；换文件/Unload保留文本，拒绝回显及Enter后Tab不重复提交。已保存120、未保存300仍执行120，Pulse默认值10/20不变。Config覆盖使用低饱和浅绿底深字，不改变控件几何。
- Calibration两个曝光字段与其后的三个API选择共用同一参数表单，资源Refresh不变；正式TaskConsole核对五行同列及真实选择写回。Rolling数值与标题留在图框上方的原固定顶部预算内，删除Rolling专属加高margin，使用原字号及已有compact padding；2x2 Image/Rolling均490×357逻辑像素，DPR3均1470×1071，实际文字bounds无裁切/重叠。Frame不靠Qt裁切掩盖，数值/标题/数据框和外尺寸同时满足原契约。截图/探针仅在ignored research。
- 可编辑数字共用Decimal定点舍入，先按真实范围限幅，再取显示精度网格内最近的合法数；删除“碰到写不下的边界就拒绝”的提前退出。只有范围确无可表示数才报错，不用科学计数或扩宽。设备真实范围更新不会被格式失败恢复成旧范围；该输入明确无效并由原Form读取边界阻止Apply。主动显示单位请求保持整份接纳/拒绝；Scan在控件接受前不提前更新设备port描述，显示与保存点列同源。原步长、Decimal单位与被动草稿通道保留，不触发设备Apply；浮点ULP尾差不误标custom，非线性点列不重排，Range/Values互不填充。Viewer直接测试仍用finish_close后处理DeferredDelete，避免close被忽略而遗留窗口。
- 同源只读数值整改在公共Fluent owner完成：Double/Integer Spin的disabled/read-only几何变化保留完整文本，保留canonical value、清除display-only `numericError`且不发normalization；重新editable才恢复原可见精度规则。Numeric LineEdit的宽度规范化屏蔽普通`textChanged`，只发`valueNormalized`，状态切换清理并重算旧宽度错误。Panel linked Scan producer因此可直接显示ROI写入的长端点，不再把`The numeric field is too narrow…`闪到axis状态，也不依赖另一Logic editor碰巧舍入回写。Editable真实窄范围拒绝、units/range clamp/custom values/normalized undo保持原语义；隐藏控件切换为只读也立即清除旧显示错误；无新增timer、轮询、全表扫描、projection revision或consumer特判。
- Slot弹窗使用现有anchor toggle/NoMouseReplay，删除总是重新show的旁路；共同列显示Scan、紧凑三态Source与实际binding的只读Config name，删除“Names: Config tab”。公共TriSwitch每段按自身文字定宽，绘制、点击与动画共享同一几何，不扩展填满弹窗。公共popup chrome计算在content就是popup时停止，不能误累计整个窗口祖先的边距；Slot自然宽度不再被无关窗口布局撑开。正式Pulse Qt窗口验证再点slot关闭、Escape/外部关闭、Scan/Source切换保持展开；截图只留ignored research。
...
- Repeat标题改为同一publication当前Repeat/Point坐标下的条件写入数量。Runtime复用原写入覆盖记录，每次commit只固定几个整数；标题不再选site/pixel或读取科学validity，旧publication和Stop后的结果不被后来进度改写。删除原末site判定及重复Workbench坐标解析，Point/Cell完整维度、三组格式和Scope独立性不变。
- Scan公共schema的轴name改为`repeat`、`shots per point`；只改名称，不改AxisId、坐标、排序或Pulse执行参数。真实Camera→Occupancy→FrameSurvival→Seamless自动图在1/401/801 shots显示1×1/2×1/3×1，科学eligibility变化不再影响标题；另20000次实际Runtime提交全部到50×1，标题metadata投影P50/P95/max为0.0019/0.0022/0.021ms（不含Qt绘图）。截图/原始数据只在ignored research。
...
- 动态Overlay从exact publication读取主图实际使用的范围，由主图`DataBlock.window`决定canonical prefix或相同indexed window；不再独立采用companion的全局history需求。状态再走公共Scope/Last和facet定位，Last是声明顺序末coordinate而非最后valid值。仅Repeat/Point定位采集cell，site向量保持原子完整；Mean或pool没有独立Boolean判决，多个cells不能伪造共识圈。invalid或不能唯一选择时隐藏，与原same-shot等待规则一致。

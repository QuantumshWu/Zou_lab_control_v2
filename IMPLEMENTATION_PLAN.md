...
- 公共Panel Setting frame现以固定identity（`Setting · panel-N`）作为可拖header，不读取可编辑title/signal/structure，并在右上角提供只关闭Setting的紧凑`×`；TaskConsole与FigureViewer因复用PanelCard同时获得该行为，Panel删除仍是card header的受保护命令。

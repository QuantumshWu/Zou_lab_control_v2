...
- **3D 面板仍是最慢的 kind**，但已到本设计的地板：ROI(0.55M) 手势 **42.1–42.6 ms**、
  整幅相机(2.3M) **53.3 ms**，live 帧 **94.5 ms**（本轮起点 181）。逐段查完：
  compose 分支健康（81 次里 58 次走可复用背景，逃生舱不触发）；派生、颜色都已进
  numba 的同一次 walk；`render_height_bars` 已改成收 256 行色表而不是逐格颜色平面
  （连 fold 那次拷贝一起省）；采样密度按柱宽收；缝按柱宽收。
  **剩下的就是场景本身**——一格一柱＝54.9 万个盒子画进 80 万像素的坐标区，
  即便所有派生平面都命中缓存，扫描线本身也要约 14 ms。**这条地板是裁决**
  （柱数=数据格数），不是没做完的实现：用旧 LOD（约 1 万根柱）时同一手势是 33 ms。

...
- Goal status：`active — formal visible-button acceptance on HEAD 632abe7 reopened four defects: panel-derived ROI/Fit are misattributed to Camera Measurement in the Logic row, Fit signal membership flickers, an open Setting popup is not resized after the first signal schema arrives, and the real Qt wheel route has not yet proved producer ROI updates with Area selector priority`
...
- Current phase：`用户在正式可见按钮路径推翻了上一 checkpoint 的完成判断。当前树根因已确认两项：(1) ConsolePresenter._show_logic 把 source_name 指向 Logic direct output 的所有派生信号也并入 Logic 行，错误归属 panel ROI/Fit；Logic 行必须只显示 descriptor/host 的 direct outputs，现有 signal_groups 继续按 panel bridge_id 分组。(2) SelectionBridge 在 source revision 先于下一批 Fit event 到达时，把 _StaleFit 当失败并 withdraw 当前 fit processor，真实改变 SignalDataPlane membership；应保留上一批 accepted Fit publication，直到同一 panel 的下一批 Fit 原位更新。仍在闭合可见 Setting popup 的 schema-change resize 与真实 Qt wheel 产品路径。`
- Last completed action：`只读根因审计完成并记录到本 Checkpoint；本轮尚未改生产代码、尚未启动 Python/GUI。此前 commit 482c2ce 的 no-echo 投影语义保留，不重做。`
- Last verified tests：`上一 checkpoint 的 private-host zoom test 与一次 Qt projection test不足以代表正式产品路径，现被用户直接反证；本轮必须先让既有测试在当前生产上精确旧红，再仅跑直接相关新绿。`
- Pending acceptance gates：`Logic 行只保留直接输出且 panel 分组稳定拥有 ROI/Fit；连续 source revisions 期间 Fit signal catalog 不增减；Setting 空白时零无意义结构刷新、首次绑定 signal 时可见 popup 当场重算宽度且右缘不裁切；真实 Qt wheel 自动更新 producer ROI，Area selector 存在时 Area 优先、删除后 viewport 接管。`
- Next action：`扩写既有 SelectionBridge/Console/QTest 守卫得到四个旧红；随后只在现有 selection_bridge.py、console.py、panel_card_view.py 及必要的现有 Qt event wiring 中做最小修复，不新增文件/类/抽象。`

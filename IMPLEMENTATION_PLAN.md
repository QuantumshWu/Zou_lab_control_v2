...
- Registered SLM Feedback：`COMPLETE / FOLLOW-UP SWEEP COMPLETE` — Target→camera BOX注册与Task自行apply Context保留；observable为single-frame多shot双高斯`bright-dark`。Calibration只供site BOX geometry；Pulse/exposure显式；per-site weight/contrast/fit历史决定update/hold/1.4x bounded bootstrap/disappearance rollback。默认500-shot coarse、`0.25`基准增益、12 updates、3000-shot/300秒validation。
...
- qCMOS SLM Feedback follow-up：`COMPLETE / SWEEP COMPLETE` — 当前mode每cycle一张frame，以本run BOX shot双高斯的`bright-dark`闭环；Calibration只供geometry，Pulse/exposure显式，history随Candidate Context续用且mode/Pulse/exposure变化时reset。产品默认virtual missing-site链valid coarse为`2.092→1.971→1.687→1.474→1.331→1.243→1.169→1.128→1.090`；validation 2000/3000 shots、84.0秒，`1.09126 [1.08293,1.09989]` accepted。Feedback 35、physics 45、Atom 325 passed；35-site×500-shot fit+BIC中位约0.714秒/candidate。

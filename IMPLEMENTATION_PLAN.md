...
- Render coherence后的正确语义性能cut已在同一真实Windows DPR3、2×2、MOT 40-shot四Panel链验证：Curve critical path从本cut前`166.6/185.8/202.1 ms`（P50/P90/max）降至重复真实run的`85.77–127.28/92.93–143.24 ms`，best`76.19–95.03 ms`；Windows混合核调度产生run间波动，帧序列无单调恶化，不用单次较好run冒充稳定值。Curve/SEM直接消费共享prepared state，grouped line+SEM一次批量transform；后者isolated render `24.48→12.56 ms`且与通用路径0差异像素。Fit动态数值不再触发第二次MathText grammar layout，overview以Matplotlib自己的最终MathText mask批量compose，完整公式、下标和抗锯齿保留。16-logical-core机器上4-thread窗优于8-thread窗且Curve/Image fit、4M Camera、dense 3D的isolated 4-thread均不慢，bootstrap因此默认最多4并保留显式环境override；全局fit/render gate实测更慢并已删除。旧`7dce795`的`67.78 ms`依赖错误的SEM列envelope与plain glyph atlas，不能作为正确画面的等价下界。Image-Fit重复真实run为`112.77–139.80/156.05–163.01 ms`（P50/P90，best`86.88–109.23 ms`）；isolated正确cold-proxy＋full refinement约`28 ms fit + 19 ms render`，不得为复现旧`80.16 ms`而恢复warm跳过cold。

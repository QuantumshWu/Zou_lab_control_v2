...
- Pulse的API slot值链已落地（worktree `zlc_v2_pulseslot`，分支`pulse-api-slot`，5个commit）：`zlc.pulse.api_values`封闭语法+`apply_api_values`取交集覆盖（zlc_pulse）；`zlc_atom/pulse_values.py`拥有`<workspace>/api_values/current.json`的位置与读写，两个pulse decoder各套用一次，因此calibration/slm_feedback/stepped/seamless/temperature五个节点全覆盖；Workspace新增`api_values`属性并seed空集；Pulse Editor打开即套用、On Pulse前重读、Load values/Save values两手势；三个scan节点新增`api_values` text字段，由scan plan editor的表格编辑（只记与pulse不同的项，被plan扫到的slot不显示）。四层顺序：authored → 当前值集 → 节点表单 → scan table。

...
- Active Goal Milestone 5：`in progress — direct occupied-site qCMOS brightness feedback + plugin-local layered SLM Editor`。当前唯一 observable 是 Calibration 同口径的逐-site BOX feature：只累计 `OccupancyProcessor` 判定 occupied 且 valid/finite 的原始值，empty shots 不进分母，也不做 per-site dark/bright response normalization；随后直接执行 `w_i <- w_i * (GM(F)/F_i)^0.45`。禁止 feedback 拟合 Zernike/modal/hidden aberration；Editor 中 operator-authored Zernike 只是显式 wavefront layer。更早 proxy、Editor lifecycle 与 close-preserves-phase 证据属于历史实现记录；layered Editor、depth-dependent fresh loading 与最终 finite-shot行为只以上方2026-08-16 checkpoint和当前待汇总验证为准，不预填新的完成数字。

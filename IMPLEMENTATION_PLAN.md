...
- Active Goal Milestone 6：`complete in this milestone commit`。合并回归暴露并修正了两条真实时序/数值边界：feedback 的 raw-moment variance 在常量 normalized strength 上产生 `6.28e-9` 假 SEM，现改为 O(35) vectorized Welford、零 shot 保存；runtime Stop、worker-return 与显式 terminal seal 现由同一 `_start_lock` 排序，worker结束后的 late Stop 保持 done，Stop先发生再退出保持 cancelled且 LIVE outputs 在terminal前detach。优化后的单次128-shot `_measure=7.96854 s`，只比 `7.96160 s` authored floor 多 `6.94 ms`；最新 Editor/feedback/PSF/finite scan/sequencer/transport/runtime/Workbench exact-session 与 archive/overlay 合并回归为 `118 passed in 31.66 s`。M6 不再有 owner-thread/device-command、产品无界 diagnostic state、science-byte或有证据的 FFT/workspace blocker；下一步提交这一大里程碑。M5 仍明确 `in progress`，且有两个独立验收缺口：足量 finite-shot 的 sparse 1% qCMOS证据，以及 Goal要求但当前5x7-only Task尚未覆盖的 dense flat-top hidden/interior约1% terminal验收。不得把功能实现、ideal dense solver、proxy、多seed hidden sparse screen或M6提速写成这两项完成。

...
- 真实200点×35-site survival Figure的Lorentzian错误已在公共SEM owner根修：中心化二阶矩对全常数bucket相减时留下`6.9e-18–1.39e-17`正roundoff，旧min-positive sigma floor把它变成`7.6e-10` SEM与`1.315e9`权重，使compiled/SciPy、Gaussian/Lorentzian、manual/live都正确优化到错误零线。`_sem_from_moments`现按本次归约/平方/相减的float64前向误差界将不可辨识残差归零，不用固定数据阈值或模型/plot kind特判，并复用原三块临时数组。保存selector下Lorentzian由`96.62156/.42488/H≈0/B≈0`恢复为`96.70214/.26766/.81829/.04067`；真实PlotSession live在真峰进入范围后的revision 120/140/160/180/200中心为`96.6832/96.7042/96.7022/96.7021/96.7021 MHz`且始终current。相关SEM/boolean/dense-generic/sigma fit原文件`15 passed`。`e52a00f3`只修TRF终止判据，未触碰SEM/权重，既不是本问题来源也不可能修正错误objective。

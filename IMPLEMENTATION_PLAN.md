...
- Active Goal SimulationWorld calibrated topology（2026-08-15）：`complete in this milestone commit`。干净 `4add9b5` 上可复现的4项失败来自两个独立旧基线：hidden coherent plant 的额外 `x1.25` 过调使默认35点 `Imax/Imin` 达到约2.28；同一个current-relative门又同时承担 calibrated nominal anchor 与 blind off-grid peak 的识别，数学上无法既保留checker的弱真阱又拒绝move/default旁瓣。修复只在world owner内删除旧过调，并继续从同一FFT/raw local-peak truth取depth：nominal anchor使用`0.10 x reference` detection gate，未知off-grid candidate使用`0.20 x reference` dominance gate；没有读取target/support、改动fresh base Bernoulli、归一化depth或让Calibration/Task读取hidden truth。200-seed A/B screen选择该方案是因为最窄topology margin仍为`+0.00987 x reference`，而备选`.15/.25`方案的checker nominal margin仅`+0.000534`。256 seeds下default/checker/add/remove/move/3 nominal/3 off-grid共七种topology逐点一对一正确，默认ratio=`2.062401295–2.145299673`、可纠正相干比例=`94.680–95.361%`。正式256-seed守卫`1 passed in 5.50 s`；完整virtual physics + feedback + Temperature当前checkout精确集合`50 passed in 37.16 s`，独立review的Calibration/Occupancy/feedback纵向也回到35-site轴。plant重调按预期改变同seed qCMOS science bytes（907 pixels），独立MOT frame仍逐字节相同，未串扰MOT physics/RNG。该结果明确取代下条hot-solve checkpoint末尾记录的4项待修失败；实验机optical acceptance仍是下一项，不得把virtual topology proof冒充真机proof。
...
- Active Goal Milestone 4：`complete in this milestone commit; physics wording superseded by the 2026-08-14 reconciliation above`。唯一 `slm.phase`/窄 `SlmAdapter`、唯一 continuous non-negative target、自动选择 weighted GS/MRAF 的唯一 `solve_phase`、strict target JSON/no-pickle canonical phase NPZ、VirtualSLM 与 `SimulationWorld` 中唯一 commanded phase/revision 已落地；hidden coherent optics（8-bit phase、低阶像差、偏心/渐晕）产生 local trap intensity，同 revision 的 qCMOS render 只传播一次，detector nuisance 限于约 1.02×。最新物理裁决规定 intensity 只进入 active topology、occupied brightness 与 release survival，所有 active traps 的 fresh loading 使用同一 base probability；本条旧的 depth-dependent loading 解释不得恢复。该 milestone 的 solver/profile 与边界证据仍是历史记录；真实 X15213 leaf 的当前状态只以上方最新 reconciliation 为准。不得重开 M2。
- Active Goal Milestone 5：`in progress — direct occupied-site qCMOS brightness feedback + plugin-local layered SLM Editor`。当前唯一 observable 是 Calibration 同口径的逐-site BOX feature：只累计 `OccupancyProcessor` 判定 occupied 且 valid/finite 的原始值，empty shots 不进分母，也不做 per-site dark/bright response normalization；随后直接执行 `w_i <- w_i * (GM(F)/F_i)^0.45`。禁止 feedback 拟合 Zernike/modal/hidden aberration；Editor 中 operator-authored Zernike 只是显式 wavefront layer。更早 proxy、Editor lifecycle 与 close-preserves-phase 证据属于历史实现记录；layered Editor、fresh loading 与最终 finite-shot行为只以上方 2026-08-14 reconciliation 和当前待汇总验证为准，不预填新的完成数字。
...
- Active Goal Milestone 5 finite-shot/performance gate（2026-08-13，science conclusion superseded）：每个 qCMOS shot 走公共 finite sequencer、exact grouped frames且不保存全部 frames 的 lifecycle/performance事实继续有效；基于 depth-dependent Bernoulli loading 的收敛率、shot 下界和候选 observable 比较已被 2026-08-14 模拟物理裁决作废，不能再引用。当前 raw occupied BOX observable 的 finite-shot/multi-seed结果等待最新实现验证，不降低 1% criterion、不读 hidden truth，也不预填数字。
...
- Active Goal Milestone 5 observable screens（2026-08-13，superseded）：旧 loading/release proxy 与 paired-ratio 数字依赖现已废止的 depth-dependent loading 模型，不再构成 science evidence 或 shot 预算。paired ratio 没有进入 Task；当前产品只使用用户裁决的 occupied-shot raw BOX brightness。是否达到 1% 以及需要多少 shots 只由最新 exact qCMOS finite-shot验证决定，仍禁止 fit、空间平滑、simulation fast-forward 或 hidden truth 回流。
...
- Active Goal Milestone 6 final owner corrections（2026-08-13）：World 缓存只保存固定几何 `spot`，每帧仍按旧 `atom_rate * probe * gain * base_area / (sigma_x * sigma_y) * spot` 的原左结合表达式与逐 site 顺序累加，避免优化引入任意参数下的 IEEE 重结合差异。`MemoryRegisterTransport.write_batches/read_log` 经全树核对只有诊断测试消费者、不是 transport protocol 或产品 audit API；为保持公开 source compatibility，默认仍是完整 `list` 历史及原 slicing/equality/index/clear 语义，仅由三条长期产品 owner（VirtualPulseStreamer、remote memory backend、Workbench virtual dial）显式 `record_history=False`，10,000 次关闭模式读写后两份历史仍为空而寄存器状态正确。定向 consumer/owner `10 passed`，最终只读审查对 Editor claim/close、World science bytes、Calibration fast/fallback、solver no-change、finite outer与diagnostic兼容均为 no-blocker。
- Active Goal Milestone 6：`complete — commit 7a5cd61`。合并回归暴露并修正了两条真实时序/数值边界：feedback 的 raw-moment variance 在常量 normalized strength 上产生 `6.28e-9` 假 SEM，现改为 O(35) vectorized Welford、零 shot 保存；runtime Stop、worker-return 与显式 terminal seal 现由同一 `_start_lock` 排序，worker结束后的 late Stop 保持 done，Stop先发生再退出保持 cancelled且 LIVE outputs 在terminal前detach。优化后的单次128-shot `_measure=7.96854 s`，只比 `7.96160 s` authored floor 多 `6.94 ms`；最新 Editor/feedback/PSF/finite scan/sequencer/transport/runtime/Workbench exact-session 与 archive/overlay 合并回归为当前树 `118 passed in 31.66 s`，同一 commit 的干净 detached worktree 再验为 `118 passed in 37.23 s`，模块路径全部位于该 worktree，验证目录随后已移除。M6 不再有 owner-thread/device-command、产品无界 diagnostic state、science-byte或有证据的 FFT/workspace blocker。M5 仍明确 `in progress`，且有两个独立验收缺口：足量 finite-shot 的 sparse 1% qCMOS证据，以及 Goal要求但当前5x7-only Task尚未覆盖的 dense flat-top hidden/interior约1% terminal验收。不得把功能实现、ideal dense solver、proxy、多seed hidden sparse screen或M6提速写成这两项完成。

...
- Loading-edge Simulation follow-up：`IMPLEMENTED / CORRECTED` — 删除错误的grid-resonant phase ripple及未提交的强像差/Stark调参；恢复与SLM command/Target/grid无关的原固定defocus/astigmatism/coma。默认nominal depth改为620 µK，固定20 µK cooling下500 µK为sharp loading edge，默认35-site中自然有4个zero-loading site；`bright-dark` probe参数保持原值，旧测试手工`weight=0.1`已删除。

...
- Active Goal SLM two-page optical UI（2026-08-15）：`in progress — authority migrated before production edits`。用户要求删除拥挤/overlap的Zernike布局、保留旧版两张大图、增加同页wavefront预览、参数化presets、主页面可见aperture/ROI/wavefront/correction入口。最终只保留Mask/Wavefront两页；Mask的Target与`Science phase (hardware correction excluded)`仍是两个独立`2x2=490x357`host；Wavefront页用第三个独立host。Input pupil是solver near-field amplitude，可由assumed ellipse/rectangle或同raster measured intensity产生并重求解；旧Mask ROI改名CGH crop且只做composition。Preset popup draft只在Apply时一次materialize target。X15213 concrete adapter增加runtime correction load/enable/status，Editor明确显示On/Off/Unavailable，correction仍不进science phase/NPZ且只在explicit Send时由adapter合成。carrier只称zero-order separation/steering并说明需要物理Fourier stop。预计现有8文件内完成、无新file/class；old-red、实现、real-screen capture与commit待完成。
...
- Active Goal X15213 / layered Editor / SimulationWorld reconciliation（2026-08-14，取代本文更早的“真实 SLM 资料不足”与“depth 提高 loading”描述）：`complete in this milestone commit on the development machine; experiment-machine optical acceptance pending`。真实 descriptor 为 `slm.hamamatsu_x15213`，`Scan hardware` 通过同一 generic descriptor seam 枚举 USB serial candidate 与 `1280 x 1024 @ ~60 Hz` DVI display candidate；DVI 没有可靠 EDID identity，必须由操作者确认。USB 使用实验机官方 `hpkSLMdaLV.dll`/`hpkSLMda.dll`，DVI 使用 exact unscaled full raster；两者都以 `1272 x 1024` active phase、authored active-X、orientation、measured `two_pi_gray` 与 settle 工作。vendor correction BMP 只保留在实验机本地路径，adapter 接受 active/full-raster 8-bit grayscale并按 authored sign/offset叠加；不上传、不进入 solver/Editor artifact，也不猜 wavelength LUT。Editor 默认 `Mask` 页保留两张旧版 `2x2 = 490 x 357` logical 大图；共享 `Size` selector 同时驱动两个独立 host 的 standing `zlc_plot` preset，超出窗口时画布行双向滚动而不重叠/裁切。Mask ROI、常用 Wavefront（Steering X/Y、Z4–Z6、Reset）和带 active-count 的 Advanced（Z2/Z3、Z7–Z11）分别放在可滚动设置页。Target→Mask、ROI、carrier/Zernike 与 Final composite仍是一条相位链；Load/Save target、Mask、Final及 raw 8-bit phase-mask import均不写硬件，只有 explicit Send apply Final。SimulationWorld 每个 cooling rise 在 trap high 时对 active topology 内每个 trap 重新做独立 base Bernoulli；depth 不再改变 loading probability，只决定 active/inactive、occupied brightness 与 release survival，missing trap 不能保留或复活 atom。开发机只有 deterministic/mocked transport evidence，不能写成真实 X15213 optical proof；实验机可直接 Scan/configure/Send bring-up，最终 orientation、correction、gray mapping、settle、光路与 lifecycle 证据等待实机填写。合并后的 X15213、Editor、SimulationWorld、session control、qCMOS feedback 与 Temperature 精确集合为 `66 passed in 40.53 s`。正式 `zlc_ui.capture_window` 在 Windows real-screen 后端验收 Mask/Wavefront/Advanced：target/actual 都是 `1024 x 577` logical、DPR 3 crop `3072 x 1731`、desktop canvas `3840 x 2400`；默认两张 plot 都是 `490 x 357` logical，切到 `4x4` 后两者都是 `826 x 609` 且双向 scrollbar 可达完整画布。未 Send，hardware phase逐元素不变，claim/session/control/top-level窗口最终归零。未使用 computer-use、offscreen、direct grab或手工缩放。M2 继续关闭，不得重开。

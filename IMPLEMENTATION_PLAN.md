...
- Current phase：`Simulation物理与MOT camera生产修改已完成focused验证，正在运行完整Atom回归；Panel标题/全局10 Hz显示时钟/Calibration PSF kernel grid仍待后续独立切片。`
- Last completed action：`未增加文件或类，在既有SimulationWorld内加入seed-deterministic约2倍site efficiency跨度、逐site sigma-x/y/angle/skew椭圆PSF、每shot随机loading和trap-low survival；既有VirtualPulseStreamer从compiled program投影首个camera window前的trap-low时间；同一VirtualCamera类新增camera.virtual_mot descriptor和1200x1920 broad MOT renderer，virtual template现含mot_camera。旧生产5个目标先分别以AttributeError/两种奇偶pattern/缺trap_off参数/缺schema红；修改后目标5 passed，相关simulation/camera/device/install 29 passed。`
...
- Next action：`运行完整zlc_atom并修正唯一template/device manifest消费者；通过后提交Simulation切片。随后独立修Panel默认标题和100 ms全局display tick，最后用现有zlc_plot FacetGrid[Image]补PSF kernel report并做profiling。`

...
- Current phase：`Simulation物理/MOT camera切片a5d600b、Panel标题/10 Hz display tick cf9e0a6、64-cycle continuous buffer ded3c48、Calibration同一result六张report 34afae5均已独立提交。当前收口通用Axis/display-label：Facet title/fit row已消费coordinate display label，但grouped Curve仍无条件删除legend。`
- Last completed action：`Calibration plugin仍只在完成一次分析后保存JSON并把同一个CalibrationRunResult直接交给公共zlc_plot。site_map.png、fidelity.png、box.png、psf.png、uniform_psf.png、psf_kernels.png六张图已由同一result和公共plot API生成；完整Atom 146 passed、Workbench 334 passed。`
- Last verified tests：`六图Calibration旧生产精确红为仅四张；修后Atom 146 passed、Workbench 334 passed且真实TaskConsole offscreen链通过。当前grouped Curve的coordinate display-label测试在旧生产红为legend is None，修正后目标1 passed，完整zlc_plot 206 passed in 13.93s；导入路径均指向当前v2树。`
- Pending acceptance gates：`先验证并独立提交grouped Curve legend的通用display-label修正；随后继续derived signals、live-fit/performance及UI剩余目标；最终重跑full/detached和由用户执行可见bin\\experiment.bat验收。`
- Next action：`旧生产已精确证明grouped Curve legend为None，目标测试修正后1 passed；现跑完整zlc_plot回归、diff-check并独立提交该两行生产语义。`

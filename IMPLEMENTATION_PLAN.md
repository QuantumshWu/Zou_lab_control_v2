...
- 当前worktree继续收口数据/历史投影：整体snapshot唯一schema/ref，内部仅不可变三plane与数值布局，不逐记录恢复public snapshot。显式整数组与Plot共用DataBlock原始打包，sigma仍按需；缩小窗口和关闭history清理不再适用的carry。传输依赖改为整数列，新增planes共享类型/单元描述成批编码，接收由原串行owner按序安装；删除重复查找表及child身份补偿缓存。源schema形状/大小只首次计算。真实Qt再次验证Fate、Frozen与保存精确数据，窗口及进程已关闭；首次恢复已消除上一阶段退化，具体性能和剩余必要成本仅记ignored报告，不宣称硬件极限。本轮不合并master。

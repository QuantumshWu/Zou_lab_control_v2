...
- Current phase：`fa4a9b9已完成viewport/zoom producer联动；按用户新证据复核ROI/fit derived signal真实Logic Start/binding路径。`
- Last completed action：`fa4a9b9：zlc_plot此前只对外发selector commit，viewport虽有canonical转换却没有callback，Workbench因此不知道zoom/pan。现有PlotSession/RasterPlotHost增加普通viewport callback，不增加event类；既有selection translator把canonical viewport交给同一个descriptor selection_patch，并把display viewport镜像到live/Edit。viewport在plot owner裁到实际data coordinate bounds，Matplotlib aspect留白不再产生负ROI。真实wheel gesture旧红为producer draft不变；修后producer ROI改变且两张surface viewport相同，1 passed。权威文档同步删除旧“zoom/pan不改producer”陈述。`
- Last verified tests：`selector+viewport正式live/Edit/retarget/Refresh/wheel/producer纵向路径1 passed；按执行纪律未跑package/full。`
- Pending acceptance gates：`用户重新报告ROI/fit signal未实际进入Logic Node；live fit必须复用现有fit artist而非每revision重建；Setting改为内容驱动最窄完整宽度、短文案和label-position Auto/Manual switch；随后重大phase/full/detached重验，以及同一最终HEAD的正式可见按钮复验与零残留。`
- Next action：`从SelectionBridge发布的roi/fit contract，经SignalDataPlane、Console source choice、LogicDraft source_signal、start_logic finalizer到NodeHost build逐段走一条真实路径；已有chooser测试不能代替实际binding/Start。只修首个实际断点，不新增registry/framework/file。`

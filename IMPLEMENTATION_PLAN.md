...
- Current phase：`Phase 1 Setting 与 Phase 2 Refresh/Edit-live 已完成，进入 Phase 3：让 fit publication 永远独立于 Selectors gate，ROI selector关闭只撤回ROI outputs，不清 fit；缩短fit signal leaf names并走真实 Logic input。`
- Last completed action：`Phase 2：同一当前 publication 的 Refresh 直接no-op；同一 signal generation的新revision在现有 editor host上调用update_data/update_image_frame并重绑frozen selection context，不再替换host/widget；generation/signal边界仍走原有replacement。Edit committed selection 的live QWidget front现被既有纵向测试显式验证。只改console.py与既有test，无新文件/类。待本Checkpoint同阶段提交后记录commit。`
- Last verified tests：`Phase 2 既有panel-editor纵向测试在原生产精确红于editor host identity；修改后1 passed in 2.21s，并同时断言Edit commit后live Qt raster front sequence真实前进。额外非持久化正式virtual repeat=0连续publication探针确认同一editor host/widget下front 1→2且frozen publication更新。每个进程先打印root/console路径指向当前v2；未跑package/full。`
- Pending acceptance gates：`上述三个缺陷的窄范围旧红/新绿、各阶段提交；Goal 末尾一次受影响全量验证。正式可见按钮最终验收仍由用户本人执行。`
- Next action：`提交 Phase 2 exact files；随后修改既有SelectionBridge/Console测试先证明Selectors默认OFF时fit缺失和ROI removed误清fit，再只改现有selection bridge与console._apply_deriving。`

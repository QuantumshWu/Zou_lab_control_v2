...
- Active Goal SLM feedback live candidates（2026-08-16）：`implementation complete in the shared tree; focused feedback file 24 passed in 15.60 s; commit pending`。根因是Task过去在每轮取出qCMOS readout frame后只累计site统计并丢弃整帧，descriptor没有Dataset output/preview，磁盘也只保存terminal artifact；Stop与普通异常共用恢复incoming路径，操作者无法在运行中观察候选，也不能在满意时停下继续使用。当前同一plugin owner为每个实际apply的canonical candidate先原子写`slm_feedback_candidate_####.npz`，写成功后才把它标为durable；随后对真实readout frames做在线repeat mean，并把`readout_average`与同一`candidate_phase`作为同generation/revision/run record的两个image outputs发布，Monitor按既有Task lifecycle自动显示两张transient preview。每轮测量/validation后原子更新同一路径的score/history。Stop重新apply最低有效coarse score候选；若尚无有效项则保留最新完成或当前已durable候选，并重发匹配的camera/phase snapshot，使device、preview与artifact candidate id一致；首存失败且Stop并发到达时恢复incoming，普通异常仍恢复incoming。成功仍保持terminal seal先赢后apply validated candidate并原子更新accepted artifact。旧红覆盖Stop+首存失败会错误保留未落盘phase；完整直接文件当前为`24 passed in 15.60 s`，production净增294行、无新file/class/framework，`git diff --check`通过。下一项只提交本feedback切片，再单独收口约4倍virtual public fluorescence对比；不得混成一个production topic。

- 2026-08-16 commit record：initial-aberration visibility + arbitrary sparse registration 已提交为 `2fce46e`；下条保留本次窄验证与物理数值。

- Active Goal initial-aberration visibility + sparse registration（2026-08-16）：`implementation complete in the shared tree; narrow guards green`。（1）旧 `SimulationWorld` 虽然 coherent trap-depth `Imax/Imin=2.034525`，但 `atom_rate=1100 e/s` 使200-count offset/background淹没信号，公开20 ms sensor / 5 ms probe / 50次default `p0=0.5` fresh-load的 `Reduce repeat -> mean` raw BOX ratio旧红只有`1.040363`；现有world owner按固定cooling期内`Γ∝ω∝sqrt(depth)`的phenomenological finite-capture model令`p_i=1-(1-p0)^sqrt(d_i)`（严格`p(1)=p0`），并用所有sites共同的AC-Stark shift→two-level Lorentzian scattering与50 ms finite bright-state lifetime生成occupied fluorescence，不增加hidden per-site gain。当前同一公开路径的depth ratio=`2.034525`、loading ratio=`1.282987`、conditional-fluorescence ratio=`1.492782`，最终raw BOX ratio=`1.876414`，最大raw frame code=`21725 < 65535`；参数finite/positive守卫、coherent correction后loading/fluorescence relative spread同步收窄、fresh-load随机性与reference-window scaling合并精确`5 passed in 9.88 s`，最终核心三节点再验`3 passed in 4.02 s`。（2）feedback不再固定5×7：任意sparse target在已验收的同向x/y、近identity apparatus registration先验下，与当前Calibration SiteMap做确定性的最近一一对应；count mismatch、local ambiguity、unmatched或过大distortion都在Task开始前拒绝，但不把规则对称点集的无标签全局permutation冒充成软件已证明唯一。成功后rows/columns mapping冻结在该Task实例，后续update只复用这份mapping。full-rank shear及horizontal/vertical大倾斜old-red后，任意geometry、拒绝边界及既有measurement/success定向集合为`14 passed in 0.34 s`。Dense interior仍不是qCMOS离散SiteMap observable。

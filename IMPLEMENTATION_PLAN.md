...
- Current phase：`4f0d963已完成ROI/fit derived Dataset进入真实Logic input与Start；开始核对live fit solver和artist更新路径。`
- Last completed action：`4f0d963：旧测试只造了假consumer。真实断点一是Occupancy把动态Calibration frame compatibility误写成固定camera.frames contract，二是finished/frozen图的fit仍强行attach live processor。现有DatasetInputSpec增加显式source-neutral语义，Occupancy按实际Calibration/schema校验；SelectionBridge复用已有reserve_frozen_processor/publish_terminal_processor发布terminal fit/ROI，不新增类或文件。真实产品测试确认ROI和fit都进入Occupancy source choices，随后用ROI artifact启动实际Occupancy，NodeHost.source_signal为该ROI signal。`
- Last verified tests：`真实panel selector+fit -> data plane -> Occupancy chooser/draft/finalizer/Start/NodeHost纵向路径1 passed；按执行纪律未跑package/full。`
- Pending acceptance gates：`live fit必须在同一fit模型/artist存在时更新现有artists，只有fit模型或artist结构改变才重建；Setting改为内容驱动最窄完整宽度、短文案和label-position Auto/Manual switch；随后重大phase/full/detached重验，以及同一最终HEAD的正式可见按钮复验与零残留。`
- Next action：`先profile/静态追RasterPlotHost.update_data -> PlotSession live-fit solver -> fit scene/render artist，区分solver是否应重算与artist是否原位更新；只在现有zlc_plot owner内修首个重复fit/重建断点，不增加文件、类或第二fit lane。`

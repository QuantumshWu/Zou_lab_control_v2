...
- Active Goal status（2026-08-13）：`blocked pending user product decision — M5 strict sparse finite-shot + dense observability`。同一阻断已连续三次 Goal turn 复核：35-site sparse 的诚实约1% qCMOS门需要约 `40M adaptation + 4–10M validation shots`（当前 authored cadence约 `94–107 h`），而35个离散校准site没有可证明完整 dense interior 约1%的公开观测量。继续必须由用户二选一：保留原严格口径并批准长时 sparse 采集且指定 dense 观测路径；或将 M5 产品验收限定为35-site sparse、dense只保留M4 ideal-solver验收。不得由执行者自行降阈值、读hidden、加拟合/空间平滑或越过“先完成M3–M6”顺序提前开始最终全仓审计。

...
用户执行边界：M7暂停；当前只根修真实SLM Feedback回归，不访问真实SLM或其它hardware。
...
- Milestone 6：`REPAIRED / SWEEP COMPLETE / COMMITTED b39f66c` — server/Context边界保留，Feedback与Simulation物理模型已根修；当前bright-dark follow-up见下。
...
- SLM server owner：`IMPLEMENTED / REPAIRED` — 仿照Pulse，apparatus只有`slm.hamamatsu_x15213`一个真实类型并只保存server host/port；server默认使用恢复的DVI exact-raster presenter，USB仅显式可选，profile/correction固定在server端，expected command/mapping revision拒绝陈旧写入。删除`slm.remote`用户分叉；默认路径不加载vendor DLL。
...
- Registered SLM Feedback：`COMPLETE / FOLLOW-UP SWEEP COMPLETE` — Target→camera BOX注册与Task自行apply Context保留；每candidate严格一批authored shots。单/双高斯有限fit均valid；归一化份额分配保证dark-only按用户boost绝对增加、invalid不变、loaded只相对配平，并以per-site dark/loaded bracket冻结loading floor。默认100 shots、`0.03` single boost、`0.25` feedback gain、`0.5`普通单步上限、12 updates，无retry/validation采集。
- Simulation物理根修：删除逐site gain/ellipse/angle/skew、target-specific 21-cycle ripple与nominal/extra双状态。唯一dynamic trap roster来自commanded phase经共同pupil、共同low-order wavefront aberration和FFT得到的dominant peaks；统一Fourier→camera affine与一个shared非对称imaging PSF。当前probe red detuning与正trap shift幅度组合为进一步红移，occupied bright-dark随depth单调下降、loading随depth上升；Camera产生真实dark/bright mixture。
...
- Camera/Occupancy live geometry follow-up：`COMPLETE / SWEEP COMPLETE` — Runtime不再由“Processor + Monitor”隐式制造`primary-index`；只有输出契约显式`index_by_source`的Fit/selection-derived信号具备history能力。`frames_per_cycle=3`的Camera与Occupancy分别保持`1×3×Y×X`和`1×3×SITE`，overlay使用同一cycle leading geometry。Runtime+Logic 133项、Workbench Presenter 56项、Camera/Occupancy/保存跨包48项及最终exact 23项通过；7个changed Python文件AST与diff-check green。
- Live history/clim/generation follow-up：`COMPLETE / SWEEP COMPLETE` — `index_by_source`降为capability，Runtime只在真实Panel window lease期间从current event开始保留、按active最大window限界并在最后lease关闭时归零；standing host提交complete target+authored delta，Fixed→Tight/Normal清除旧clim且coalescing不丢相邻编辑；window edit只重投影host-owned snapshot，不再向Runtime索取retired ROI/Fit publication。正式Camera→ROI→Histogram证明Panel前多轮无prehistory、window=1只留current，ROI generation替换期间1→100不再卡死或报generation mismatch。最终Runtime `103 passed`、Plot `429 passed`、Workbench Presenter `57 passed`、Workbench跨包（排除既有launcher换行项）`389 passed`、exact `5 passed`；8个changed Python文件AST与diff-check green。未过滤Workbench只剩3个本修改未触碰的既有`.bat` bare-LF失败。
- Auto-axis repeat policy follow-up：`COMPLETE / COMMITTED bf3133e + ace4a19` — FacetGrid默认resolver删除三处`AxisRef.repeat()`兜底；自动只选择producer-authored frame/point/scan axis，无非repeat候选时返回None，显式`fate:repeat=facet`路径保留。Residual补齐repeated scalar的point-facet Histogram seed而未恢复repeat兜底；Plot全包428、Workbench排除既有batch CRLF参数项后392项完成无失败。
- Camera Restart generation follow-up：`COMPLETE / COMMITTED 78a71fd + ace4a19` — 旧Monitor取消后不再在其causal generation仍有surface projection在途时poll/retire旧Host；accept/reject后才释放device并activate candidate。正式Camera→auto-preview竞态、queued restart与Qt Producer Restart 3项及两个完整Host-contract lifecycle residual green；无新增production file/class/state。
- qCMOS SLM Feedback follow-up：`COMPLETE / SWEEP COMPLETE` — phase在shot前发布；真实全site ratio与显式`observable_uniformity_history`分离。用户真实Calibration 23→Target 35 site的100-shot链以`23→26→29→32→32→33→35`恢复loading，candidate 7测得全阵列ratio `1.098487`；Task不按magic ratio提前停，而在全部authored updates后选择最佳candidate。
- Virtual Camera decoupling follow-up：`IMPLEMENTED` — 删除single-consumer `CameraCaptureSpec`及CameraCycleSource对Pulse window/cadence的解析；VirtualCamera不再按Pulse source time与exposure skip trigger，source ordinal只按实际采集frame连续编号。普通`imaging_template + 0.1 s`不再制造虚假ordinal gap。
- Loading-edge Simulation follow-up：`IMPLEMENTED / CORRECTED` — 删除错误的grid-resonant phase ripple及未提交的强像差/Stark调参；恢复与SLM command/Target/grid无关的原固定defocus/astigmatism/coma。默认nominal depth固定为520 µK，固定20 µK cooling下500 µK为sharp loading edge；5×7、9×9、4×4、7×5、3×3默认grid均有至少10% zero-loading sites。`bright-dark` probe参数保持原值，旧测试手工`weight=0.1`已删除。
- 本follow-up Gate 17/18冻结：12 files `+1103/-819`（净+284）；production 3 files `+519/-450`（净+69），tests 3 files `+505/-296`（净+209），active docs 6 files `+79/-73`（净+6）。无新增file/class/lane/stored lifecycle state；production helper由删除`_censored_sites/_boost_target`、新增双consumer `_fit_contrasts`净减1。Tests净增1 function，新增两个多consumer sample fixtures；全部KEEP，旧Calibration-dark、three-frame、all-shot-average、2x bootstrap与whole-candidate rollback assertions已就地改写，无重复/obsolete/white-box-only test待删。Active old-name/API/docs scan、6 changed Python AST与diff-check green。
...
| 6 | Server-owned SLM与robust Feedback | COMPLETE / REPAIRED | DVI-default，USB显式可选；见§11 |
...
- SLM Feedback每candidate使用稳定`<task>/camera`的一次canonical Camera Measurement `repeat=N` generation；Estimator与preview共用唯一frame coordinate。phase在shot前发布，shot后分别更新真实all-site history与显式observable-site progress；Stop只接受完整测量candidate，真实失败恢复incoming。
...
- `display_interval`只控制Surface deadline；显式声明只表示history能力，Runtime仅在consumer window lease期间保存source primary index及validity，普通Monitor/Occupancy与无lease派生信号仍为latest cycle。Surface同一same-shot group只保持一个active完整front，忙时只保留Plane latest和admission debt，中间indices invalid且不排完整frame。
...
- 普通Pulse Stop/FIRE wire priority、Camera/Remote/FPGA归M5；SLM transport/context/feedback归M6；single distribution/fresh install/notebook与final docs归M7。M4未访问hardware，也不把offscreen/virtual证据冒充实验机验收。
...
- Runtime新增capability+lease的中立`primary-index` indexed-derived Dataset：需要history的Fit/selection-derived输出只有在真实consumer lease起点之后才对每个Measurement source index写value或invalid，普通Monitor、Occupancy及无lease派生信号仍latest；active leases取最大window并受64 MiB/100k硬上限，最后lease关闭即清空。所有Plot读取同一OwnedSnapshot；standing Plot对window/fate编辑重投影自己已接受的snapshot，扩大window只影响未来保留，绝不向Runtime复活retired generation；Save冻结同一Dataset。
...
- Load在任何program write前核target ABI、clock、geometry、uint32 count与TTL/DAC delay FIFO capacity。Camera Measurement按自己authored cardinality消费actual frames，不解析program windows/cadence；Virtual按compiled wall cadence逐cycle但不以camera exposure skip Pulse frame event或制造ordinal gap。
...
- 正式X15213 server默认DVI exact-raster presenter，USB frame memory仅显式可选。真实adapter初始command为unknown；DVI presenter ack或USB write/display/readback再加profile settle后才是known-new。失败为unknown（USB可根据readback区分known-old）；command与mapping revision、profile/model/serial/wavelength/orientation/correction/settle和outcome进入immutable receipt。
...
- Feedback只接受含frozen spots Target的Science Context v2与普通Calibration，并在构造时完成注册，不另选Target artifact；Task自行apply Context起始phase，只更新Pattern并保留pupil/wavefront/system correction。当前follow-up的每个candidate复用single-frame canonical Camera Measurement sealed Dataset，对本轮raw BOX shots双高斯拟合bright-dark并用per-site历史更新Target；Calibration只供BOX geometry，不伪造dark/bright sample或finite value。
...
- Frozen-tree residual：functional candidate 21 files `+3396/-1958`（净+1438）；production净+1084、tests净+329、active product docs净+22、profile净+3。加入三份Checkpoint状态文档后最终24 files净+1453。后续根据实验机DLL失败证据，DVI exact-raster presenter已恢复为server默认transport；客户侧仍无双device type或双owner。
...
- 明确未验收：本地无真实1280×1024 SLM display或官方Hamamatsu header/DLL；真实DVI geometry/orientation、可选USB controller/ABI、profile曲线来源、correction编码/方向与optical settle必须按Atom README实验机runbook记录。

Milestone 6原commit：`Converge USB SLM context, feedback, and simulation truth`；其USB-only决策已在实验机证据后根修为DVI-default server owner。

...
- Active Goal SLM feedback uniformity curve（2026-08-16）：`complete in this milestone commit; focused old-red 2 failed -> final 3 passed in 0.26 s`。根因是Task已有每个candidate的finite coarse`uniformity_ratio` history并写入NPZ，却只向live plane发布camera average与candidate phase两张image，Monitor没有下降曲线。当前同一plugin owner增加第三个Dataset output `uniformity_history`与普通curve auto-preview；横轴是1-based candidate/update编号，纵轴是该candidate occupied-site brightness的`max(F_i)/min(F_i)`，理想方向为下降至1。curve从第一次publication起按`max_updates`预分配固定point schema，未来点为invalid/NaN；每轮递增revision且只填一个新点，因而同一个generic plot session会真正live更新，而不是到terminal才freeze。validation只是同candidate独立复测而不重复点，无效measurement保留candidate坐标并以invalid/NaN呈现；三个outputs共享generation/revision/run record，Stop重发retained candidate时仍保留整段已完成history。descriptor+two-point growth旧红分别因缺output/KeyError为`2 failed`，实现后descriptor/growth/Stop retained三节点为`3 passed in 0.26 s`，且测试守住连续front的schema identity；provenance指向当前checkout。

...
- **3D 面板仍是最慢的 kind**：ROI 手势 **42–50 ms**、整幅相机 **53.3 ms**，
  ...
  **地板的来源**：DPR 3 的 4x4 卡片是 1630×1630 = 266 万设备像素，场景必须在这个
  尺寸上光栅化（20.7 ms），matplotlib 还要在同尺寸上合成（9.7 ms）。同一块面板作为
  **heatmap** 时手势帧是 11.6 ms——差额就是 3D 场景光栅与它的 chrome，而它存在是因为
  **柱数=数据格数**这条裁决。对照：旧 LOD（约 1 万根柱）时同一手势 33 ms。

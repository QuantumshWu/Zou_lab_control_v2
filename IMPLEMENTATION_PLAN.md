...
- Current phase：`commit 48a0444 已恢复所有 signal 的 producer + 完整 shape，Fit 仅修真实 leaf 名字。现在按正式产品反证继续定位：ROI/Fit 未进入 Logic tab、Setting Auto 行错位与字段 commit 后宽度突变、Area/viewport 未正确回写 producer ROI，以及 Camera Measurement 默认 Repeat 不是 0。`
- Last completed action：`Phase 3 commit b01aa43 保留 SelectionBridge 生命周期修复和 FitResult.parameter_names 单一名字来源；commit 48a0444 恢复所有普通/ROI/Fit signal 的 producer 分组与完整 shape 文本。Fit leaf 为 center_x/center_x_err，不再出现 FitParameterSpec(...) 或重复 fit_..._error。现有真实 raster test 又补齐 live panel -> frozen Edit 的反向 Area 同步，和已有 Edit -> live 方向共同证明两视图消费同一 canonical selector state。未新增生产文件/类/框架。`
- Last verified tests：`用户修正前的当前生产精确红：Workbench 2 failed（frame_0 与 Fit/ROI 均缺shape），Qt 1 failed（collapsed picker缺producer）；commit 48a0444 后相同测试分别2 passed和1 passed。双向 selector 新增反向断言后，既有 test_panel_editor_selection_uses_only_its_current_frozen_publication 1 passed。此前SelectionBridge 19 passed、Qt Console views 25 passed以及Refresh窄测试证据仍有效。所有进程先打印root/被测模块路径指向当前v2；未跑package/full。`
- Pending acceptance gates：`逐条完成真实 Logic tab、Setting/Edit 几何、字段 commit 稳定布局、Area/viewport -> producer ROI、Repeat=0 默认值的根因修复；随后只跑直接相关验收、阶段提交，并由用户本人做正式可见按钮验收。`
- Next action：`先追 exact SignalPlane publication 到 Logic editor projection 的过滤/刷新链，不能再以合成 source_options 测试替代正式 tab；并记录 Setting/Edit 每行真实 geometry 前后变化。`

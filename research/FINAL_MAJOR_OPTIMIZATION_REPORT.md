...
2026-09-07；性能取证版本为 `971765ba`（基于 `ec706be2` 的性能 worktree）。
本轮保留上一轮已验证的投影、Front 生命周期等修改。用户随后批准合并；与 master `d1b1d1e0` 的整合保留了其候选裁决、有效性、交付顺序及公共 bench 机制。下列时间是整合前的实测，不冒充整合后重新测量的结果。

合并交界验证：现有测试245项通过（拟合76、投影/显示159、Front/Runtime/bench 10）。保留master公共CPU采样和配置完成判据，删除重复bench计时；已删除的单图Newton路径不因冲突恢复。未新增测试、未跑全套回归或性能矩阵。Numba只报告一处strided `np.dot`的连续内存性能建议，没有数值错误。

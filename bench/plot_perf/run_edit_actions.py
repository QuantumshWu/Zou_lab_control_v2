...
def _flows(bench: ConsoleBench, actions: list, *, grid, histogram, roi_signal: str, window: int) -> None:
    ...
    Each is one call to the presenter command the Setting form and card signal
    enter, timed until the panel's picture has changed to match.  Calling the
    command directly preserves its refusal result; a Qt signal discards return
    values and let the old bench wait thirty seconds after a rejected edit.
    Every change is made and then undone, so both directions are measured.
    """
    ...
    def state_change(panel, patch: dict, holds) -> None:
        ...
        try:
            actions.append(_timed_action(
                bench,
                f"flow: {label} {short}",
                lambda: presenter.update_panel_state(
                    panel.panel_id, dict(patch)
                ),
                # A live card changes Front whenever new data arrives.  That
                # alone cannot acknowledge this edit: the earlier benchmark
                # returned after an unrelated live frame, fired the inverse
                # edit while the first configuration was still in flight,
                # and then blamed the product when the real UI's mutation
                # gate would have kept the second control disabled.  Settle
                # the panel's own transaction and representation first.
                lambda: (
                    panel.configuration is None
                    and (
                        panel.port is None
                        or panel.port.presentation_current
                    )
                    and holds(panel.state)
                    and _front_of(bench, panel) is not before
                ),
                timeout=30.0,
            ))
    ...
    state_change(
        ...
        lambda state: (
            state.display.get("window") == 3 * window
            and histogram.history_lease is not None
            and histogram.history_lease.window == 3 * window
        ),
    )
    state_change(
        histogram, {"display": {**display, "window": window}},
        lambda state: (
            state.display.get("window") == window
            and histogram.history_lease is not None
